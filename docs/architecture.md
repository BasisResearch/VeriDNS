...
`SlistFromNS.fromNsWithGlue`. `stepFindServers` also looks up cached A records
...
exposed via the manual `SlistFromNS.matchCount`/`hasServers`. Without this,
...
- **`SlistFromNS S NS`** (extends `SlistSpec S NS`): batch SLIST creation from
  an array of NS name wire bytes and a match count. Used by `stepFindServers`
  (NS walking) and `stepAnalyzeResponse` (4b delegation).
