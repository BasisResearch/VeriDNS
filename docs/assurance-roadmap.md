...
`resolveWithIO_simulates` (`Proof/Refinement.lean`) is **not total**. Impl-connected branches:
...
- **D2 (GAP 3) — keystone DONE:** credibility-granularity gap closed *on the resolver's used cred
  ...
  *unconditional* refinement; the only step left is to thread it (with `αSection_map_rrBytes_wf`) to
  delete the `hhit` argument of `resolveWithIO_cacheHit_simulates` literally. The 7-level credibility
  ranking is the sole remaining gap-3 model-completeness item.
