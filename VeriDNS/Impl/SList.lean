...
def DnsSList.addServer (s : DnsSList) (entry : ServerEntry) : DnsSList :=
  { s with servers := s.servers.push entry }

/-- Best server = least queries sent so far. -/
def DnsSList.bestServer (s : DnsSList) : Option ServerEntry :=
  s.servers.foldl (init := none) fun best e =>
    match best with
    | none => some e
    | some b => if e.queryCount < b.queryCount then some e else some b

def DnsSList.removeServer (s : DnsSList) (name : ByteArray) : DnsSList :=
  { s with servers := s.servers.filter fun e => e.name != name }

instance : SlistSpec DnsSList ServerEntry where
...
def DnsSList.mkSbelt (entries : Array (ByteArray × BitVec 32)) : DnsSList :=
  { servers := entries.map fun (name, addr) => ⟨name, some addr, 0, 0⟩
...
instance : SlistFromNS DnsSList ServerEntry where
  fromNsNames names mc :=
    { servers := names.map fun n => ⟨n, none, mc, 0⟩
      zone := ByteArray.empty
      matchCount := mc }
...
def DnsSList.pickBest (best : Option (ServerEntry × BitVec 32)) (e : ServerEntry)
    : Option (ServerEntry × BitVec 32) :=
  ...
  | some addr =>
    ...
    | none => some (e, addr)
    | some (b, baddr) =>
      if e.queryCount < b.queryCount then some (e, addr) else some (b, baddr)
...
def DnsSList.bestWithAddress (s : DnsSList) : Option (ServerEntry × BitVec 32) :=
  s.servers.foldl DnsSList.pickBest none

/-- Mark a server as queried (increment queryCount). -/
def DnsSList.markQueried (s : DnsSList) (name : ByteArray) : DnsSList :=
  { s with servers := s.servers.map fun e =>
    if e.name == name then { e with queryCount := e.queryCount + 1 } else e }
...
/-- Names of servers whose addresses are not available. RFC 1034 §5.3.3
    ...
    address knowledge is partial (`ServerEntry.address : Option`). These names
