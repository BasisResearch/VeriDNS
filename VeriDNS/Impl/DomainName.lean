    ...
    if h : pos < buf.data.size then
      ...
      if b == 0 then
        ...
        if h2 : pos + 1 < buf.data.size then
          ...
          if 12 ≤ offset ∧ offset < pos then
            let endPos := firstEndPos.getD (pos + 2)
            match decodeNameAux buf offset fuel (some endPos) with
            | .ok (labels, _) => .ok (labels, endPos)
            | .error e => .error e
          else .error "domain name: compression pointer into the header, forward, or self (RFC 1035 §4.1.4)"
...
def toggleCaseByte (b : UInt8) : UInt8 :=
  if 65 ≤ b && b ≤ 90 then b + 32
  else if 97 ≤ b && b ≤ 122 then b - 32
  else b

def caseSeedBit (seed : UInt16) (i : Nat) : Bool :=
  (seed.toNat >>> (i % 16)) % 2 == 1

def randomizeCase (seed : UInt16) (n : ByteArray) : ByteArray :=
  ⟨n.data.mapIdx fun i b => if caseSeedBit seed i then toggleCaseByte b else b⟩

def labelCount (wire : ByteArray) : Nat :=
  match wireFormatToLabels wire with
  | .ok ls => ls.size
  | .error _ => 0

def minimisedName (wire : ByteArray) (keep : Nat) : ByteArray :=
  match wireFormatToLabels wire with
  | .ok ls =>
    if keep < ls.size then labelsToWireFormat (ls.extract (ls.size - keep) ls.size)
    else wire
  | .error _ => wire
