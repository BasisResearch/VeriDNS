...
`scrubAnswer` is owner-based only: it keeps every record whose owner lies on
the CNAME chain rooted at the query name, regardless of its TYPE.  A response
carrying an entitled answer record can therefore smuggle same-owner records of
a *different* type into the delivered answer section.  RFC 1034 §3.6.2: the
answer section of a response to a QTYPE=T query consists of the CNAME chain
records plus records of type T — nothing else.  `typeScrub` is the delivery
filter enforcing exactly that: a delivered record either matches the query
type (`QType.covers`, so QTYPE=* keeps everything) or is a chase-chain CNAME. -/

/-- A record is *type-relevant* to a query type iff the query type covers it
    (RFC 1035 §3.2.3 QTYPE matching) or it is a CNAME (a chase-chain link,
    RFC 1034 §3.6.2). -/
def typeRelevant (qtype : QType) (rr : RR) : Bool :=
  qtype.covers rr.rtype || rr.rtype == RRType.cname

/-- Drop every answer record that neither matches the query type nor is a
    CNAME chain link (RFC 1034 §3.6.2). -/
def typeScrub (qtype : QType) (answer : List RR) : List RR :=
  answer.filter (typeRelevant qtype)

theorem typeScrub_subset {qtype : QType} {answer : List RR} {rr : RR}
    (h : rr ∈ typeScrub qtype answer) : rr ∈ answer :=
  (List.mem_filter.mp h).1

/-- **068 pin (model side)**: every record surviving the type scrub has the
    query type or is a CNAME. -/
theorem typeScrub_relevant {qtype : QType} {answer : List RR} {rr : RR}
    (h : rr ∈ typeScrub qtype answer) :
    qtype.covers rr.rtype = true ∨ rr.rtype = RRType.cname := by
  have hp := (List.mem_filter.mp h).2
  unfold typeRelevant at hp
  rw [Bool.or_eq_true] at hp
  rcases hp with hc | hc
  · exact Or.inl hc
  · refine Or.inr ?_
    generalize hg : rr.rtype = t at hc ⊢
    cases t <;>
      first | rfl | exact absurd hc (by decide) | exact Bool.noConfusion hc

/-- A wrong-type record (not covered, not a CNAME) never survives the type
    scrub, whatever its owner. -/
theorem typeScrub_excludes {qtype : QType} {answer : List RR} {rr : RR}
    (hty : qtype.covers rr.rtype = false) (hcn : rr.rtype ≠ RRType.cname) :
    rr ∉ typeScrub qtype answer := by
  intro h
  rcases typeScrub_relevant h with hc | hc
  · rw [hty] at hc; exact absurd hc (by simp)
  · exact hcn hc

/-- The composed delivery scrub stays authentic: `typeScrub` only removes
    records, so everything it delivers is still owned on the CNAME chain
    (strengthens `scrubAnswer_authentic` to the full delivery pipeline). -/
theorem typeScrub_scrubAnswer_authentic (qtype : QType) (qname : Name) (answer : List RR) :
    AnswerAuthenticWrt qname answer (typeScrub qtype (scrubAnswer qname answer)) :=
  fun rr hrr => scrubAnswer_authentic qname answer rr (typeScrub_subset hrr)

/-- Data preservation through the composed delivery scrub: every delivered
    record is a same-data copy of an original answer record (strengthens
    `scrubAnswer_data`). -/
theorem typeScrub_scrubAnswer_data {qtype : QType} {qname : Name} {answer : List RR} {r' : RR}
    (h : r' ∈ typeScrub qtype (scrubAnswer qname answer)) :
    ∃ r ∈ answer, r'.rdata = r.rdata ∧ r'.ttl = r.ttl ∧ r'.cls = r.cls
      ∧ nameEq r'.owner r.owner = true :=
  scrubAnswer_data (typeScrub_subset h)
