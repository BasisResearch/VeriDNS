...
/-- RFC 1034 §4.3.2.d: "if the response shows a servers failure or **other
    bizarre contents**, delete the server from the SLIST and go back to step 3"
    (retry).  A response justifies a retry when it is an explicit SERVFAIL, or
    the resolver could extract neither an answer, a proper delegation, nor a
    CNAME from it, or its RCODE is neither NOERROR nor NXDOMAIN, or it is an
    **empty-answer response that is not an NXDOMAIN** — the last disjunct is the
    key strengthening (2026-07-15, DIRECTION row): an empty NOERROR whose
    authority section carries no valid negative proof (no SOA, RFC 2308 §2.2) and
    no proper referral is *bizarre contents*, and must be re-queried, never
    synthesised into a believed NODATA.  This covers the AA=1 lame-referral (040)
    and the bare empty NOERROR (041/045).

    `entitled : Format → Bool` is the ENTITLED-answer predicate (the resolver's
    concrete `entitledAnswerB`): a response answers the query only when it
    carries a record of the query type whose owner is on the CNAME chain rooted
    at the query name.  The final disjunct is the 2026-07-15 off-owner
    strengthening (findings 036 / off-owner-A): a NON-EMPTY answer that is
    entitled in no record is *foreign* — it scrubs to empty at delivery, so it
    too is bizarre contents and must be re-queried, never accepted-then-scrubbed
    into a spurious NODATA. -/
def VeriDNS.Spec.guard_serverFailure : (VeriDNS.Spec.Format → Bool) → VeriDNS.Spec.Format → Prop :=
  fun entitled resp =>
  resp.header.rcode = VeriDNS.Spec.Rcode.serverFailure ∨
    ...
    (resp.answer.isEmpty = true ∧ resp.header.rcode ≠ VeriDNS.Spec.Rcode.nameError) ∨
    (entitled resp = false
      ∧ (resp.header.rcode ≠ VeriDNS.Spec.Rcode.nameError ∨ resp.answer.size > 0))
...
def VeriDNS.Spec.guardRefined_answerOrNameError : (VeriDNS.Spec.Format → Bool) →
  (Array ByteArray → BitVec 16 → Bool) → (VeriDNS.Spec.Format → Bool) → VeriDNS.Spec.Format → Prop :=
  fun answersQuery hasRRType handled resp =>
  answersQuery resp = Bool.true
    ∨ (resp.header.rcode = VeriDNS.Spec.Rcode.nameError ∧ resp.answer.isEmpty = Bool.true)
...
def VeriDNS.Spec.guardRefined_cname : (VeriDNS.Spec.Format → Bool) →
  (Array ByteArray → BitVec 16 → Bool) → (VeriDNS.Spec.Format → Bool) →
  (VeriDNS.Spec.Format → Option ByteArray) → VeriDNS.Spec.Format → Prop :=
  fun answersQuery hasRRType handled chase resp =>
  hasRRType resp.answer 5 = Bool.true ∧ answersQuery resp = Bool.false
    ∧ (chase resp).isSome = Bool.true
...
def VeriDNS.Spec.obligation_answerOrNameError : (VeriDNS.Spec.Format → Bool) →
  (Array ByteArray → BitVec 16 → Bool) →
    (VeriDNS.Spec.Format → Bool) →
      (VeriDNS.Spec.Format → Option ByteArray) →
      (VeriDNS.Spec.Format → Option VeriDNS.Spec.AlgorithmStep → Prop) → Prop :=
  fun answersQuery hasRRType handled chase transition =>
  ∀ (resp : VeriDNS.Spec.Format),
    VeriDNS.Spec.guardRefined_answerOrNameError answersQuery hasRRType handled resp →
      ¬VeriDNS.Spec.guardRefined_delegation answersQuery hasRRType handled resp →
        ¬VeriDNS.Spec.guardRefined_cname answersQuery hasRRType handled chase resp →
...
inductive VeriDNS.Spec.StepSpec  : VeriDNS.Spec.AlgorithmStep → VeriDNS.Spec.AlgorithmStep → Prop where
  ...
  | serverFailure : ∀ (entitled : VeriDNS.Spec.Format → Bool) (resp : VeriDNS.Spec.Format),
  VeriDNS.Spec.guard_serverFailure entitled resp →
    VeriDNS.Spec.StepSpec VeriDNS.Spec.AlgorithmStep.analyzeResponse
...
def VeriDNS.Spec.responseHandled : (VeriDNS.Spec.Format → Bool) → VeriDNS.Spec.Format → Prop :=
  fun entitled resp =>
  VeriDNS.Spec.guard_answerOrNameError resp ∨
    VeriDNS.Spec.guard_delegation resp ∨
      VeriDNS.Spec.guard_cname resp ∨ VeriDNS.Spec.guard_serverFailure entitled resp
