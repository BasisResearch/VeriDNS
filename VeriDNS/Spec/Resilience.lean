...
-- RFC 5452: resilience against forged answers (post-Kaminsky hardening).
-- §9.1: a resolver MUST match responses against the query's attributes and
-- treat mismatches as invalid — implemented by `acceptResponse` (ID, name,
-- class, type; Proof/Server.lean `acceptResponse_matches`). Source
-- address/port matching happens at the socket layer and is not yet checked.
-- §9.2: outgoing queries MUST use an unpredictable query ID over the full
-- 16-bit range — implemented by `UdpSocket.randomId` + `withRandomId`.
-- The §9.1 matching rules generate `querymatchingrules_match_obligation`
-- (one abstract matcher per bullet), instantiated by acceptResponse in
-- Proof/Server.lean.
