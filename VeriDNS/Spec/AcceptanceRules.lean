...
structure AcceptanceConditions where
  ...
  /-- The reply packet is actually a *response* (header QR bit set): a
      reflected query is never DNS data (RFC 1035 §4.1.1; finding 030). -/
  qrIsResponse : Prop
  /-- The reply packet's OPCODE echoes the outstanding question's opcode
      (always the standard QUERY opcode for the queries this resolver
      originates, RFC 1035 §4.1.1; finding 030). -/
  opcodeEchoes : Prop
...
theorem accept_requires_qr_response (c : AcceptanceConditions)
    (h : acceptResponse c) : c.qrIsResponse := h.2.2.2.2.1

theorem accept_requires_opcode_echo (c : AcceptanceConditions)
    (h : acceptResponse c) : c.opcodeEchoes := h.2.2.2.2.2
