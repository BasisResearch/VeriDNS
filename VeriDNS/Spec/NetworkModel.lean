...
/-- The "pick-one" `foldl` in `bestDeleg` only ever returns an element of the list (or the initial `acc`). -/
...
/-- **`bestDeleg` returns a delegation that is an ancestor of the query name.** Since `bestDeleg` folds
    over `z.delegations.filter (isAncestor ·.subapex qname)`, its result (if `some d`) is in that filtered
    list, so `isAncestor d.subapex qname`. The model-side extraction feeding the driver's referral `hbail`
    (`Response.inBailiwick`) — no impl `suffixMatchCount` loop needed. -/
...
def cnameRR (recs : List RR) : Option RR :=
  recs.find? (fun r => r.rdata.rtype == RRType.cname)
rfc_proves VeriDNS.Spec.Net.cnameRR [1034][1311:1318]

/-- An empty record list carries no CNAME. The `hnc` (CNAME side-condition) discharge for the
    empty-answer `ServerAnswers` constructors (`nameError`/`noData`/`exitFollowed`/`cacheMiss`) at the
    honest answer terminal — they deliver `answer := []`, so `cnameRR [] = none` satisfies the left
    disjunct of `serverAnswer_hasVerdict`'s `hnc` unconditionally. -/
@[simp] theorem cnameRR_nil : cnameRR ([] : List RR) = none := rfl

/-- Filtering preserves "no CNAME": if a record list has no CNAME, neither does any sublist of it. Used by
    the forward-simulation answer terminal to lift `ServerAnswers.answer`'s `hnc` (about `here`) to the
    served subset (`here.filter …`, the model answer section). -/
theorem cnameRR_filter_none (here : List RR) (f : RR → Bool) (h : cnameRR here = none) :
    cnameRR (here.filter f) = none := by
  unfold cnameRR at h ⊢
...
theorem cnameAlone_forces_cname {recs : List RR} {c : RR} {qt : QType} {qcls : RRClass}
    (hca : cnameAlone recs = true) (hcn : cnameRR recs = some c)
    (hqt : qt.covers RRType.cname = false) :
    recs.filter (fun r => qt.covers r.rdata.rtype && r.cls == qcls) = [] := by
  simp only [cnameRR] at hcn
  have hpc := List.find?_some hcn
...
inductive ServerAnswers (s : Server) (now : Time) : List Name → Bool → Query → List Step → Response → Prop where
  ...
  | answer {seen : List Name} {o : Bool} (q : Query) (z : Zone) (here : List RR)
      ...
      (hnc : cnameRR here = none ∨ q.qtype.covers RRType.cname = true)
  ...
  | cname {seen : List Name} {o : Bool} (q : Query) (z : Zone) (c : RR) (target : Name)
      ...
      (hz : bestZone s q.qname q.qclass = some z)
      (hd : bestDeleg z q.qname = none)
      (hc : cnameRR (recordsAt z q.qname) = some c)
...
/-- **`isAncestor` is congruent in its ancestor argument under `nameEq`** — if `a` is an ancestor of `n`
    and `b` equals `a` (as a name), then `b` is an ancestor of `n`. The model-side step turning
    `isAncestor d.subapex qname` + `nameEq nsOwner d.subapex` into `isAncestor nsOwner qname` for the
    referral `hbail` (each NS record's owner equals the delegation apex by `Zone.WF`). -/
...
/-- **`isAncestor` is congruent in its descendant argument under `nameEq`.** If `a` is an ancestor of `b`
    and `b` equals `c`, then `a` is an ancestor of `c`. The model-side step for the referral `hdesc`
    (`referralCut ref` equals the delegation apex only up to `nameEq`). -/
...
/-- **An ancestor is no longer than its descendant.** `isAncestor` gates on `Nat.ble anc.length n.length`,
    so a true result forces `anc.length ≤ n.length`. Feeds the referral `hdesc` strict-length step. -/
...
/-- **Every name has an ancestor at each depth up to its own**: dropping the leading `n.length - k`
    labels leaves the length-`k` suffix, an ancestor by `isAncestor`'s suffix reading. This is the
    frontier construction for the depth-progress refer rules: the referral cut's ancestor at exactly the
    current SLIST matchCount depth is fresh by the driver's length invariant. -/
...
/-- **An equal-length ancestor is an equal name.** If `a` is an ancestor of `b` and they have equal
    length, then `b.drop (b.length - a.length) = b.drop 0 = b`, so `nameEq a b`. Together with
    `isAncestor_length_le` this turns a strict-descendant `¬nameEq` witness into a strict length gap. -/
...
theorem answer_step_on_tree (s : Server) (now : Time) (seen : List Name) (o : Bool)
    (q : Query) (z : Zone)
    (hz : bestZone s q.qname q.qclass = some z) (hd : bestDeleg z q.qname = none)
    (hnc : cnameRR (treeRecordsAt (treeOf z) q.qname) = none ∨ q.qtype.covers RRType.cname = true)
