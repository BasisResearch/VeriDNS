...
/-- A response with a non-empty answer section is not a referral (referrals have an empty answer). Used by
    the forward-simulation answer terminal to discharge `serverAnswer_hasVerdict`'s `hnr` obligation from the
    (non-empty) server answer. -/
...
/-- An authoritative response (`aa = true`) is never a referral: `isReferral` requires `aa == false`.
    Discharges `serverAnswer_hasVerdict`'s `hnr` for the *negative* honest terminal — the authoritative
    `ServerAnswers` constructors (`answer`/`cname`/`wildcard`/`nameError`/`noData`/`exitFollowed`) all set
    `aa := true`, so even when their answer section is empty (NODATA / NXDOMAIN) they are not referrals. -/
theorem isReferral_false_of_aa (r : Response) (h : r.aa = true) :
    r.isReferral = false := by
  unfold Response.isReferral
  simp [h]

/-- A response with an empty authority section is never a referral: `isReferral` requires an NS record in
    the authority. Third `hnr` building block for the honest answer terminal — discharges the `cacheMiss`
    `ServerAnswers` constructor (`authority := []`). Together with `isReferral_false_of_aa` (the six
    `aa = true` ctors), `isReferral_false_of_answer_ne_nil` (`referralCacheAnswer`, and `fromCache` with a
    non-empty cache hit), this covers 8 of the 9 constructors; only the bare `referral` and
    `fromCache`-with-empty-hit are genuinely referral-shaped (the impl would have *followed* them, so they
    cannot reach the `.finished` answer terminal — that residue is closed by the codec/decode-validity step). -/
theorem isReferral_false_of_authority_nil (r : Response) (h : r.authority = []) :
    r.isReferral = false := by
  unfold Response.isReferral
  simp [h]

/-- **A response with the referral shape IS a referral.** The positive converse of the `isReferral_false_*`
    lemmas: empty answer, non-authoritative (`aa = false`), positive (`rcode = noError`), an NS record in
    authority, and NO SOA (a SOA marks a negative/NODATA answer, not a delegation) together give
    `isReferral = true` by direct computation. The driver's `.continue` honest-refer arm feeds these five
    facts about `αResp respA` — the impl's referral classification (empty answer + NS, from
    `stepAnalyzeResponse_goto_referral`) plus the honest referral's `aa`/`rcode`/no-SOA wire properties — to
    obtain `(αResp respA).isReferral = true`, hence `href` (via the WorldModels `hisref` conjunct). -/
...
/-- **A cache-served delegation is in-bailiwick by construction.** `cachedDelegation` filters
    `freshServerCache` by `isAncestor r.owner qname`, so every NS record it returns has an owner that is
    an ancestor of the query name — exactly `Response.inBailiwick`. This discharges the referral `hbail`
    for the `ServerAnswers.fromCache` constructor (a cache-served referral, whose `authority` is
    `cachedDelegation`), the companion to `referral_bailiwick_desc`'s zone-delegation (`referral` ctor)
    case. RFC 1034 §5.3.3 / anti-poison: a recursive server may only serve cached delegations within
    bailiwick, which the `cachedDelegation` filter enforces. -/
...
def accepts (out reply : Datagram) : Bool :=
  (out.id == reply.id)
    ...
    && nameEq out.qname reply.qname && (out.qtype == reply.qtype)
...
theorem accepts_reply (id : Nat) (srcAddr dstAddr : String) (srcPort : Nat) (ednsBuf : Nat) (q : Query)
  ...
  simp [accepts, queryDatagram, replyDatagram, nameEq_refl, hq, hcl]
...
theorem accepts_requires_match (out reply : Datagram) (h : accepts out reply = true) :
  ...
  exact ⟨h.1.1.1.1.1.1.1, h.1.1.1.1.1.2, h.1.1.1.1.2, h.1.1.1.2, h.1.1.2, h.1.2, h.2⟩
...
/-- **RFC 2181 §5.4.1 usability gate.** Data of the least-trustworthy class — the additional section,
    and the authority section of a non-authoritative answer — "should not be cached in such a way
    that they would ever be returned as answers". `Cred.additional` is exactly that class; every other
    credibility is usable as an answer. -/
...
/-- **Order-free de-duplication** (keep one per distinct `CacheRR`). Self-contained — this toolchain has no
    Mathlib `List.dedup`, so the dedup + its `Perm`-stability are hand-proved from core (`subperm_of_subset` +
    `Subperm.antisymm`). Used by `topServed` to make the model's served set match the impl's write-time-deduped
    `store` for WARM caches (RFC 2181 §5.4.1: a lookup yields the max-cred RRset, deduplicated) WITHOUT
    touching `Cache.insert` — keeping the pure-prepend write path and `MatchMaxEquiv.absorb` intact. -/
...
/-- `dedup` is `Perm`-stable: permuted inputs give permuted (de-duplicated) outputs. This is the property
    `topServed` needs so the `topOf`/bridge lemmas (which produce `topServed`-`Perm`) keep feeding
    `MatchMaxEquiv` after the dedup is wrapped in. -/
...
/-- The per-key **rank-maximal** matching records — `served` *before* the usability filter. Isolating the
    max-rank gate (RFC 2181 §5.4.1) from the `usable` gate is what makes a cache-substitution congruence
    for the forward simulation tractable: `served` (and hence `hit`) is just `topServed` filtered by
    usability, while `topServed`-equality (unlike `served`- or `hit`-equality) is **stable under
    `absorb`** — appending the same records to two caches with equal per-key argmax keeps the argmax equal,
    since records strictly below a key's max can never re-enter the max. (Two caches can `serve` the same
    records yet differ in max *rank* — a non-usable top-rank record suppresses usable lower ones — so
    `served`-equality alone is not `absorb`-stable; `topServed`-equality pins the rank and is.) -/
def Cache.topServed (c : Cache) (now : Time) (q : Query) : List CacheRR :=
  (c.matching now q).filter (fun e => (c.matching now q).all
    (fun e2 => !(e2.sameKey e.rr) || Nat.ble e2.cred.rank e.cred.rank))

/-- `served` is exactly `topServed` filtered by usability — the gate-factorisation the congruence rests
    on. (`served`'s predicate is `usable && maxRank`; `topServed`'s is `maxRank`; both `maxRank` closures
    are over the *same* `c.matching now q`, so they compose as nested filters up to `Bool.and_comm`.) -/
...
/-- `hit` is determined by `topServed`: equal `topServed` ⟹ equal `served` ⟹ equal `hit`. The cache-read
    half of the substitution congruence (one of the four read predicates the cache enters `Resolves`
    through). -/
...
/-- The **served** (per-key rank-maximal) CNAME records for a name — the *order-invariant* basis for the
    `cacheCname` premise (the capstone plan's Phase 0a: use `cn ∈ cnameServed` membership instead of the
    order-sensitive `cnameAt = some cn`, so the forward-simulation cache-substitution congruence carries it
    by permutation-preserves-membership, with no `Name` ordering needed). Defined off `served`, hence
    determined by `topServed` and covered by `MatchMaxEquiv` without a separate clause. -/
...
/-- Same `(owner, rtype, cls)` cache key (the model image of the impl `rrSameKeyB`). -/
def rrKeyEq (a b : RR) : Bool := nameEq a.owner b.owner && (a.rtype == b.rtype) && (a.cls == b.cls)

/-- The minimum `ttl` over every member of `rrs` sharing `r`'s key (seeded by `r.ttl`, so a per-key
    minimum when `r ∈ rrs`) — the model image of the impl `groupMinTtl`. -/
def rrGroupMin (rrs : List RR) (r : RR) : Nat :=
  rrs.foldl (fun acc e => if rrKeyEq e r then min acc e.ttl else acc) r.ttl

/-- **RRset TTL normalization (RFC 2181 §5.2).** Rewrite each member's `ttl` to the per-key minimum
    over the section. Preserves the member *set* (only `ttl` changes) — the model image of the impl
    `normalizeRRsetTtls`. Applied per section inside `absorb` so the model and impl cache the same
    (uniform-TTL) RRsets, keeping the `MatchMaxEquiv`/`WriteRefines` refinement tight. -/
...
/-! ### `normalizeTTL` / `foldl insert` structural helpers (used to re-establish the `absorb` lemmas
    now that each section is TTL-normalized before insertion). `normalizeTTL` is a `map` that changes
    only `ttl`, so owner/rtype/cls/rdata provenance survives it. -/
...
/-- A normalized member shares owner/rdata/cls with the section record it came from (only `ttl`
    changed). The provenance backbone for the `absorb` lemmas under normalization. -/
...
/-- A positive entry of a fold of `insert`s over `ys` is either from the base cache or has its `rr`
    among `ys`. -/
...
/-- A fold of `insert`s only ever adds — every base positive entry survives. -/
...
theorem absorb_pos_in_bailiwick (c : Cache) (now : Time) (bw : Name) (resp : Response)
  ...
  -- Every section is `normalizeTTL (section.filter keep)`; a stored `rr` has the owner of a kept
  -- (hence in-bailiwick) record.
...
/-- `absorb` only ever **inserts** records — it never removes one — so every positive entry of `c`
    survives into `c.absorb`. The forward (monotone) companion of `absorb_pos_in_bailiwick`; needed to
    show `c.matching ⊆ (c.absorb).matching` (rank-maximality over the larger matching set implies it over
    the smaller subset). -/
...
/-- **Served-record anti-poison (the form that survives cache refinement).** Every record `c.absorb`
    would *serve* (its `topServed` set, for any time/query) is either served by `c` already or is
    in-bailiwick of the absorbed response. Unlike the raw-`pos` `absorb_pos_in_bailiwick`, this is phrased
    on `topServed`, so it composes with `CacheRefines` (`topServed`-`Subperm`) in the `referForget` rule —
    a resolver that *forgets* (evicts) cached answers before recursing still never serves off-path data. -/
...
theorem absorbNeg_ttl_is_soa_minimum (c : Cache) (now : Time) (q : Query) (resp : Response)
    (m : Nat) (hrc : resp.rcode = RCode.nameError) (hm : soaNegTtl resp = some m) :
    (c.absorbNeg now q resp).neg = ⟨q.qname, none, now, m⟩ :: c.neg := by
  have hb : (RCode.nameError == RCode.nameError) = true := rfl
  simp [Cache.absorbNeg, hrc, hm, hb]
...
theorem absorbNeg_nodata_typed (c : Cache) (now : Time) (q : Query) (resp : Response)
    ...
    (hm : soaNegTtl resp = some m) :
    (c.absorbNeg now q resp).neg = ⟨q.qname, some q.qtype, now, m⟩ :: c.neg := by
...
/-- **Positive-cache eviction by a per-record predicate** (the model image of the impl's expiry-class
    `boundExpiryClasses`: `αCache_boundExpiryClasses_eq` gives `αCache c.boundExpiryClasses = c.filterPos qf`).
    Only positives are dropped — the negative cache is untouched, exactly as the impl's `boundStateCache`. -/
...
/-- `filterPos` commutes with `matching` (both are positive-cache filters): the matching records of the evicted
    cache are the matching records of the original, evicted. -/
...
/-- **Cache refinement** — `cf` serves a `Subperm` (sub-multiset) of `c'`'s positive answers per `(now, q)` and
    agrees on the negative cache. The sound model reading of bounded-cache eviction (RFC 1035 §7.4): a resolver may
    FORGET some cached positive answers — re-resolving over the network yields a model-permitted verdict — but never
    fabricates a cache hit and never alters negative caching. The refer rule's recursive cache may be any such
    refinement of the post-absorb cache; `CacheRefines.refl` recovers the no-eviction (under-capacity) case. -/
...
/-- Post-compose a `CacheRefines` with a same-time `topServed`-`Perm` + negative-cache agreement (the
    `MatchMaxEquiv` clauses): the cache-substitution retarget for the `cacheCname` eviction slot. -/
...
/-- **Time-bounded cache refinement** — `CacheRefines`, but the positive-cache `Subperm` clause is required
    only at read times `≥ t` (the negative clauses stay unconditional; eviction and RRset replacement never
    touch the negative cache). This is the refinement an RFC-faithful cache write actually satisfies: the
    implementation REPLACES a same-key RRset on store and REFUSES less-trustworthy data (RFC 2181 §5.2,
    §5.4.1), while the model `absorb` accumulates and max-gates at read time — so at read times *before* the
    write the two caches can genuinely disagree (the model still serves data the impl's write dropped), but
    from the write time onward the impl serves only what the accumulate-cache would. Since `Resolves`
    derivations read the cache only at their own (monotonically advancing) clock, a continuation started at
    time `t` never performs a pre-`t` read — `CacheRefinesFrom t` is exactly the sound obligation for the
    referral-continuation cache. -/
...
/-- Post-compose a time-bounded refinement with an unconditional one (e.g. a `MatchMaxEquiv`-derived
    `CacheRefines` retargeting the absorb cache under cache substitution). -/
...
/-- Pre-compose an unconditional refinement (e.g. the `boundStateCache` eviction) with a time-bounded one. -/
theorem CacheRefines.trans_from {t : Time} {a b c : Cache}
    (h : CacheRefines a b) (h' : CacheRefinesFrom t b c) : CacheRefinesFrom t a c :=
  (h.from t).trans h'

/-- **Write refinement** — the relation an RFC-faithful cache WRITE bears to the model's accumulate-`absorb`.
    Four clauses: (i) *read-soundness from `t`*: at read times `≥ t` (the write/continuation time) the
    written cache serves a sub-multiset of what the accumulate-cache would — replacement (RFC 2181 §5.2) and
    trust-refusal (§5.4.1) only ever *shrink* the served set going forward; (ii) *all-time provenance*: ANY
    record the written cache ever serves, the accumulate-cache serves at SOME read time (same query) — a
    replaced-in RRset is served by the accumulate-cache once its higher-trust or differently-expiring
    occluders lapse, so no serveable data is ever fabricated (this is the clause the cache-poisoning
    provenance theorems — `resolves_cache_in_bailiwick`, `resolves_cout_grounded` — walk through, keeping
    their unconditional statements); (iii)–(iv) the negative cache agrees unconditionally (positive writes
    never touch it). Pre-`t` *same-time* agreement is deliberately NOT required: the model still serves data
    the write dropped at past reads, but a `Resolves` continuation's clock starts at `t` and only advances,
    so those reads never occur. -/
...
/-- Post-compose with an unconditional refinement whose `topServed` clause is a same-time `Perm`
    (e.g. a `MatchMaxEquiv`-derived retarget of the absorb cache under cache substitution): `Perm` preserves
    membership both ways, so the provenance clause survives. Note a mere `CacheRefines` does NOT post-compose
    (its `Subperm` can drop the provenance witness); the `Perm` form is what cache substitution provides. -/
...
/-- Pre-compose an unconditional refinement (e.g. the `boundStateCache` eviction, a whole-key
    `filterPos`) with a write refinement. -/
...
/-- A `WriteRefines` transfers a cache MISS at (or after) the continuation time: the written cache
    serves only a sub-multiset of what the target serves, so a target miss is a written-cache miss.
    With `absorb_hit_nil` this discharges the referral recursion's `hit = []` precondition. -/
...
/-- `Cache.absorb` reads only the response's `aa`/`answer`/`authority`/`additional` — two responses
    agreeing on those four absorb identically. Lets the forward simulation rewrite the absorb of its
    processed impl response into the absorb of the model reply (their non-answer sections agree by the
    codec round-trip; a referral's answer sections are both empty). -/
...
/-- A referral's answer section is empty (`isReferral` requires it) — the extraction lemma. -/
...
/-- A referral is non-authoritative (`isReferral` requires `aa = false`) — the extraction lemma. -/
...
/-- Intro form of `descendsBelow_strict`: an ancestor of the cut that is strictly shorter witnesses
    `descendsBelow`. Discharges the depth-progress `hdescF` for a frontier constructed at the current
    floor depth (`isAncestor_drop_ancestor`). -/
...
/-- **An in-bailiwick referral's cut is an ancestor of the query name.** `referralCut` is the first
    authority-NS owner (or the root when there is none, an ancestor of everything); `inBailiwick`
    bounds every authority-NS owner by `q.qname`. Discharges the `hcut` premise of `trustedReferral`
    for the driver's cache-served and spoofed referral arms — the RFC 5452 poison-confinement bound. -/
...
/-- Under the `refer` rule's `descendsBelow` premise, the absorb bailiwick collapses to the referral cut
    itself: `absorbBailiwick srvApex cut = cut`. (`descendsBelow apex` is *definitionally* `absorbBailiwick`'s
    own guard.) This is what lets the model refer bailiwick `absorbBailiwick (serverBailiwick …) (referralCut
    reply)` reduce to `referralCut reply` — exactly the NS owner the impl's `referralCutRaw` extracts — so the
    impl and model bailiwick filters coincide. -/
...
/-- **In-bailiwick glue addresses.** The SLIST is seeded from referral glue, but *only* glue that is
    trustworthy: an additional `A` record qualifies iff (a) its owner is one of the NS hosts named in
    the authority section (`referredServers`) **and** (b) its owner lies within the delegated zone cut
    (`isAncestor (referralCut ref) r.owner`). Clause (b) is the **in-bailiwick-glue rule**: a parent
    delegating zone `cut` is authoritative only for names at or below `cut`, so glue for `ns.evil.com`
    accompanying a delegation of `sub.example` must be discarded even though it is referenced — exactly
    the cache-poisoning vector. This matches the implementation's `bailiwickRaws (referralCutRaw …)`
    filter on the SLIST glue (`Resolver.lean`), closing the model-side half of the in-bailiwick-glue
    gap (the cache absorb was already bailiwick-filtered via `absorbBailiwick`). -/
def glueAddresses (ref : Response) : List String :=

  (referredServers ref).filterMap (fun h =>
...
/-- **Membership characterization of the per-NS-host `glueAddresses`.** A dotted string is a glue address iff
    some referred NS host's `find?` over the additional section lands on an `A` record whose address dots to
    it. The model-side handle for the SLIST connector's glue correspondence (step 4c), pairing with the impl's
    `modelSlistOf_fromNsWithGlue` per-NS-host `findSome?` form. -/
theorem mem_glueAddresses (ref : Response) (s : String) :
    s ∈ glueAddresses ref ↔ ∃ h ∈ referredServers ref, ∃ r a,
...
/-- **NS host names for the SLIST at `nm`** — the cached NS RRset's rdata targets at `nm`, taken at per-key
    MAX credibility via `topServed` (RFC 2181 §5.4.1 ranking) but WITHOUT the `usable`/answer filter: building
    the SLIST is not *answering*, so even low-credibility (e.g. additional-section glue) NS/glue may be used to
    *reach* servers — only the per-key max-rank gate applies. A real resolver builds its SLIST from cached NS
    (RFC 1034 §5.3.3), preferring the most credible records per key; this is the model counterpart of the impl's
    `walkNs`. Using `topServed` (not `served`) keeps glue (correctly non-answerable) available for the SLIST, and
    `topServed` is exactly what `MatchMaxEquiv` pins — so this is cache-substitution-invariant. -/
...
/-- **All glue `A` addresses for host `h`** — per-key max-credibility (`topServed`), all addresses (a real
    resolver keeps and tries EVERY known address of a nameserver; RFC 1034 §5.3.3). The `filterMap` over
    `topServed` is `Perm`-stable (and order-independent, so no `findSome?`-first ambiguity). -/
...
/-- **Re-derive the referral SLIST from the (post-absorb) cache.** Walks `nm` up toward the root to the closest
    enclosing cached NS, then takes ALL cached glue `A` addresses of each host (dropping glueless hosts). This is
    what a real resolver does (RFC 1034 §5.3.3): the SLIST is built from the CACHE, not directly from the
    response — so it matches the impl's `walkNs`+ all-addresses glue under cache correspondence (`MatchMaxEquiv`),
    regardless of prior cached NS at the cut. The `flatMap` (all addresses) is `Perm`-stable. -/
...
/-- SLIST entries (address + RTT) re-derived from the cache — the `Cache.referralSlist` analogue of
    `glueEntries`, used as the model `refer` rule's recursive SLIST. -/
...
/-- **Out-of-bailiwick glue is rejected (anti-poison regression).** A referral delegating `SUB`
    (cut = `["SUB"]`) names *two* NS hosts: an in-bailiwick `ns1.sub` and an out-of-bailiwick
    `evil.com`, each with matching additional `A` glue. Even though `evil.com` is a referred server
    (so it passes the old `referredServers` test), its owner is **not** within the delegated cut, so
    the in-bailiwick-glue rule drops its address — only `7.7.7.7` seeds the SLIST, never `6.6.6.6`.
    This is the model-side analogue of the implementation's `bailiwickRaws` SLIST filter and the exact
    glue-poisoning vector the referral-cache bug exploited; with the tightened `glueAddresses` it is
    now a theorem, not a latent gap. -/
...
/-- The 32-bit numeric key of an IPv4 (big-endian octet packing) — a total, injective order used to pick
    a canonical address. -/
def ipKey (ip : IPv4) : Nat :=
  ip.o0.toNat * 16777216 + ip.o1.toNat * 65536 + ip.o2.toNat * 256 + ip.o3.toNat

/-- The running min-by-key fold step over `Option IPv4`. -/
...
/-- **Order-invariant address selection** (RFC 1034 §5.3.3 picks *an* address of a name server; the choice
    is unspecified, so we canonicalise to the minimum-key A-record). Picking the *first* record would make
    the choice depend on cache/answer order — which breaks the forward-simulation cache-substitution
    congruence at `gluelessNs`. The min-key pick is permutation-invariant (`addressOf_perm`). -/
def addressOf (resp : Response) : Option String :=
  ((resp.answer.filterMap (fun r => match r.rdata with | .a a => some a | _ => none)).foldl
...
inductive Resolves (net : Network) (ns : NetState) (resolverAddr : String) (ednsBuf : Nat) (rttOf : String → Nat) :
  ...
  /-- **Trusted-reply answer — the RFC 5452 threat model made explicit.** A datagram delivered to the
      resolver that *passes* `accepts` (matching transaction id, question, and source for the query sent to
      `addr`) is trusted and its message returned — exactly what the executable resolver does. The `origin`
      need NOT equal `addr`: an off-path attacker who guesses the 16-bit id + ephemeral source port (or
      spoofs the source IP) produces an `accepts`-passing reply the resolver cannot distinguish from the
      genuine server's answer. The delivered answer section is CACHED (RFC 1034 §5.3.3 step 4a / §7.4 —
      the impl's `stepAnalyzeResponse` positive-answer delivery writes `bailiwickRaws sname resp.answer`
      before delivering), carried in `trustedCname`'s replacing/evicting continuation-cache slot shape:
      `cf0` `WriteRefines` the answer-only, `q.qname`-bailiwick absorb image and `cf` is `cf0` after the
      RFC 1035 §7.4 capacity eviction; the rule is TERMINAL, so its conclusion cout is pinned to `cf`.
      A genuinely UNWRITTEN delivery (the impl's tc=1 truncated delivery — `cacheUnlessTruncated` is the
      identity on TC=1, RFC 1035 §4.1.1 partial data — and the negative/junk-answer deliveries) is kept
      expressible by the `cf0 = c` disjunct of `hcf0`: an unwritten cache does NOT `WriteRefines` the
      absorb image in general (a higher-credibility absorbed record may occlude what the old cache
      served, breaking both the read-soundness and the provenance clause), so the no-write case must be
      a disjunct, not a `WriteRefines.refl` instance. Because the rule now writes, it joins
      `trustedReferral`/`trustedCname` as an *in-bailiwick cache poisoning* admission, bounded by the
      `TrustedReplyCache` escape disjunct (poison confined to `q.qname`'s own subtree); it also weakens
      verdict *authoritativeness* (see the `∨ TrustedReplyAnswer` disjunct in
      `resolves_answer_grounded`/`_authoritative`). RFC 5452 §§ id/port/question matching. -/
  ...
  | refer {now now' : Time} {nseen : List Name} {seen : List Name} (addr : String)
      (rest : List String) (q : Query) (srv : Server) (tr : List Step) (ref : Response)
  ...
  /-- **Referral with a REPLACING (and possibly evicted) recursive cache** — identical to `refer`, except the
      recursion runs over the resolver's OWN continuation caches instead of the model's accumulate-`absorb`:
      `cf0` is the post-write cache image (an RFC-faithful store REPLACES a same-key RRset and REFUSES
      less-trustworthy data — RFC 2181 §5.2/§5.4.1 — so from the continuation time onward it serves a
      sub-multiset of what the accumulate-`absorb` would: `hcf0 : CacheRefinesFrom now' …`), and `cf` is `cf0`
      after the capacity-bound eviction (RFC 1035 §7.4, `hcf`). The resolver may forget cached positive
      answers — forcing re-resolution — but never fabricates a hit and never alters negative caching. The
      recursive SLIST is re-derived from `cf0` (the cache the resolver actually consulted to build it), and
      must be contained in the continuation's candidates (`hsl`, `Subperm`). `CacheRefinesFrom.refl` +
      `CacheRefines.from` recover the cold-cache/no-eviction case; the forward simulation supplies
      `cf0 = αCache(post-referral-write cache)`, `cf = αCache(boundStateCache …)`, `hcf0` from the warm-write
      refinement, and `hcf` from `cacheRefines_boundStateCache_absorb`-style eviction refinement. -/
  ...
  /-- **Trusted-referral — the RFC 5452 threat model for a *cache-writing* delegation.** The referral analogue
      of `trustedReply`: an `accepts`-passing datagram delivered from an arbitrary `origin` (an off-path attacker
      who guessed the 16-bit id + ephemeral source port, or spoofed the source IP) that the resolver classifies
      as a referral and *follows* — caching its bailiwick-filtered records and rebuilding the SLIST, exactly as
      the executable resolver does (it cannot distinguish the forgery from the genuine delegating server). Unlike
      `trustedReply` this WRITES cache, so it is the one rule that admits *in-bailiwick cache poisoning*. The
      damage is provably bounded: the write bailiwick is `absorbBailiwick frontier (referralCut reply.msg)`,
      which — under the `descendsBelow frontier` guard (`hdesc`) the impl enforces — collapses to `referralCut
      reply.msg`, an ancestor of `q.qname` (`hcut`). So the attacker can inject only records descending from the
      delegation cut on the query's own resolution path — never redirect an unrelated domain (see the weakened
      `∨ TrustedReferralCache` escape disjunct in `resolves_cout_grounded`/`resolves_cache_in_bailiwick`, and the
      unconditional anti-poison bound it still enforces). Mirrors `referForget`'s replacing/evicting continuation
      caches (`cf0`/`cf`, `hcf0`/`hcf` `WriteRefines`) and its either-source SLIST (`hsl`), with the honest
      server (`srv`/`hans`/`hwire`) replaced by the off-path `origin`/`htrans` and the server bailiwick abstracted
      to `frontier`. RFC 5452 §§ id/port/question matching; RFC 1034 §5.3.3 SLIST rebuild. -/
  | trustedReferral {now now' : Time} {nseen : List Name} {seen : List Name} (addr origin : String)
      (rest : List String) (q : Query) (frontier : Name)
      (ftr : List Step) (rpath : List String) (tEnd : Time) (final : Response)
      (id srcPort : Nat) (c cout : Cache)
      (hmiss : c.hit now q = [])
      (hnmiss : c.negHit now q = false)
  ...
  /-- **Trusted-CNAME — the RFC 5452 threat model for a *cache-writing, verdict-transforming* chase.** The
      CNAME analogue of `trustedReferral`: an `accepts`-passing datagram from an arbitrary `origin` (an
      off-path attacker who guessed id + port, or spoofed the source IP) that the resolver classifies as a
      CNAME redirection and *chases* — caching its answer section and restarting at the canonical name,
      exactly as the executable resolver does. The damage is provably bounded: the write bailiwick is
      `q.qname` itself (the impl's `bailiwickRaws sname resp.answer`), so the poison is confined to the
      queried name's own subtree; and the delivered answer prepends only the single chased CNAME record.
      Mirrors `answerCname`'s replacing/evicting continuation caches and restart-`seen`, with the honest
      server (`srv`/`hans`/`hwire`) replaced by the off-path `origin`/`htrans`. -/
  ...
  /-- **Glueless NS-target address resolution (RFC 1034 §5.3.3 "obtain the missing addresses",
      second half of the old fused `referNoGlue`).** Receipt of a glueless referral is covered by
      `refer`/`referForget` — the referral is absorbed and its NS hosts survive only as cached NS
      records. This rule matches the implementation's (and BIND/unbound's) temporal structure: with
      NO addressed SLIST candidates (conclusion slist `[]`), pick an NS host `nsHost` at an
      enclosing cut `zone` of `q.qname`, resolve its `A` address as a FRESH sub-resolution (its own
      visited sets: the impl bounds the sub-run by depth, not by the main chain's bookkeeping), and
      continue the main query on the sub-run's output cache at a SLIST containing the learned
      address.

      The NS-host provenance anchor `hns` reads from a rule-carried FROZEN cache `cprov` (the
      corner-2 `cacheCname cf`-slot mechanism), NOT from the rule's current cache `c`: real
      resolvers (BIND/unbound, RFC 1034 §5.3.2 SLIST) hold the referral's NS names IN MEMORY and
      never re-derive them from the live cache, so a later sub-run's higher-credibility
      `(zone, NS, IN)` write — which OCCLUDES the referral RRset in `topServed`-based reads — must
      not invalidate the remaining glueless targets. `cprov` is instantiated by the soundness
      driver as the genuine post-referral cache (pre-capacity-bound, which also sidesteps
      `boundExpiryClasses` eviction); it is deliberately untied to `c` — no true local refinement
      exists in either direction (c both gains sub-run absorbs and loses evictions vs `cprov`),
      and `hns` feeds no security walk (they compose only through `ihNs`/`ihRec`). `nsHostsAt`
      keeps the anti-poison pedigree: per-key max-rank `topServed` over bailiwick-gated writes. -/
  ...
  /-- **Accepted-but-useless reply ⟹ retry the next server** (RFC 1034 §5.3.3 / 4.3.2: a SERVFAIL or
      otherwise unusable response from a queried server is discarded and resolution continues with the
      remaining SLIST). The reply was *delivered* (`Transit … (some reply)`) and *passed* the RFC 5452
      anti-spoof `accepts` gate (so it is not `rejectSpoof`, which is for *rejected* replies, nor
      `timeout`, which is for *lost* datagrams), yet it carries `rcode = servFail` and so cannot be a
      `ServerAnswers` derivation (`answer`/`refer`). This is the model image of the implementation's
      `dropIfBizarre`/`afterResume_bizarre` retry branch — a branch the model previously lacked, which
      blocked the forward-simulation totality (`resolveWithIO_total`) for the bizarre-retry path. -/
  ...
  /-- **Unfollowable / out-of-bailiwick delegation ⟹ retry the next server** (the bailiwick enforcement
      the referral-poisoning fix added, as a model branch). A server returns a referral
      (`reply.msg.isReferral`) whose cut does *not* descend strictly below the server's bailiwick
      (`descendsBelow … = false` — not closer to the target, i.e. an out-of-bailiwick or lateral/upward
      delegation an attacker might inject). Such a delegation is *not followable* — `refer`/`referForget`
      require `descendsBelow = true` — so the resolver discards it and continues with the remaining SLIST,
      caching nothing from it. This is the model image of the implementation's `unfollowableDelegationB`
      drop (`run_ioResumeLoop_unfollowable`); without it the forward-simulation totality had no target for
      that branch — the *security-relevant* one. -/
  ...
  /-- **Server selection is a free choice ⟹ the verdict is invariant under SLIST reordering** (RFC 1034
      §5.3.3 / §7.2: the resolver "selects" a server from the SLIST as a *performance heuristic* — the
      reference implementation sorts by measured RTT, but the *choice* of which valid server to query is
      not correctness-relevant; any server in the candidate set yields the same authoritative answer). This
      rule lets a derivation built for one ordering `slist'` of the candidate servers be reused for any
      permutation `slist` of it. It is **output-preserving**: the trace, path, end-time, output cache, and
      final response are all unchanged — only the order in which the (same) servers would be tried differs.
      Soundness: `slist'.Perm slist` means the two SLISTs hold exactly the same servers, and resolution only
      ever queries servers drawn from the SLIST, so no new server, answer, or cache write is introduced —
      `cout` is literally identical, so cache anti-poison (`offpath_cannot_cache`) is unaffected, and the
      visited `path` is unchanged, so delegation acyclicity is preserved. This closes the forward-simulation
      *order* gap: the implementation seeds its SLIST from referral glue in NS-name/array order and queries
      by `transmissionCount` (never reading the model's abstract `rttOf`), whereas the model `refer` rule
      pins the recursive SLIST to `sortByRtt (glueEntries rttOf …)`; the two orderings cannot coincide for an
      abstract `rttOf`, but they are always *permutations* of the same glue-address set, which this rule
      bridges. -/
...
theorem resolves_time_monotone {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat} {now : Time}
  ...
  | refer addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hmsg href hbail frontier hdesc hdescF hglue hfresh hmono sl hsl hrec ih =>
      exact Nat.le_trans hmono ih
  | referForget addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hmsg href hbail frontier hdesc hdescF hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih =>
      exact Nat.le_trans hmono ih
  ...
  | rejectSpoof addr rest q ftr rpath tEnd final c cout id srcPort reply hreject hrec ih => exact ih
  | badResponse addr rest q ftr rpath tEnd final c cout id srcPort reply htrans hacc hbad hrec ih => exact ih
  | unfollowableReferral addr rest q srv tr ref id srcPort ftr rpath tEnd final c cout reply hmiss hnmiss hfind hans htrans hacc href hunfollow hrec ih => exact ih
  ...
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss reply htrans hacc href hbail hcut hdesc hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih =>
      exact Nat.le_trans hmono ih
  | _ => exact Nat.le_refl _

/-- **Depth-progress acyclicity (RFC 1034 §5.3.3 loop detection).** Each `refer`/`referForget`/
    `trustedReferral` step descends strictly below a `frontier` watermark (`hdescF`/`hdesc`) and records that
    frontier in `seen`, requiring the next frontier to be unseen. So the chain of visited frontiers is free of
    repeats (`gluelessNs` pushes NO frontier — like `timeout`/`skipMissing` it is a pass-through for the
    descent bookkeeping: its NS-address sub-run carries its own fresh visited sets and the continuation keeps
    the main chain's unchanged) — the model's loop-freedom is expressed as DEPTH PROGRESS on `q.qname`'s path (matching the impl's
    `delegationCloserB` strict matchCount-increase guard), not on zone identity (a warm-cache delegation's
    served-zone provenance is unattestable) nor on server addresses (which may legitimately repeat under
    anycast, so the old address-path `Nodup` is deliberately no longer claimed). Termination (`now ≤ tEnd`)
...
theorem resolves_rd_irrelevant {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat}
  ...
  | refer addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      ...
      exact Resolves.refer addr rest {q with rd := b} srv tr ref ftr rpath tEnd final id srcPort c cout
        hmiss hnmiss hfind (serverAnswers_rd_irrelevant b hans) reply htrans hacc hwire href hbail
        frontier hdesc hdescF hglue hfresh hmono sl hsl ih
  | referForget addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hwire href hbail frontier hdesc hdescF hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih =>
      exact Resolves.referForget addr rest {q with rd := b} srv tr ref ftr rpath tEnd final id srcPort c cout
        hmiss hnmiss hfind (serverAnswers_rd_irrelevant b hans) reply htrans hacc hwire href hbail
        frontier hdesc hdescF hfresh hmono sl cf0 hcf0 hsl cf hcf ih
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss reply htrans hacc href hbail hcut hdesc hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih =>
      exact Resolves.trustedReferral addr origin rest {q with rd := b} frontier ftr rpath tEnd final id srcPort c cout
        hmiss hnmiss reply htrans hacc href hbail hcut hdesc hfresh hmono sl cf0 hcf0 hsl cf hcf ih
  | answerCname addr rest q srv tr resp cn target id srcPort c nsl ftr rpath tEnd cout final
  ...
  | rejectSpoof addr rest q ftr rpath tEnd final c cout id srcPort reply hreject hrec ih =>
      exact Resolves.rejectSpoof addr rest {q with rd := b} ftr rpath tEnd final c cout id srcPort reply hreject ih
  | badResponse addr rest q ftr rpath tEnd final c cout id srcPort reply htrans hacc hbad hrec ih =>
      exact Resolves.badResponse addr rest {q with rd := b} ftr rpath tEnd final c cout id srcPort reply
        htrans hacc hbad ih
  | unfollowableReferral addr rest q srv tr ref id srcPort ftr rpath tEnd final c cout reply
      ...
      exact Resolves.unfollowableReferral addr rest {q with rd := b} srv tr ref id srcPort ftr rpath tEnd
        final c cout reply hmiss hnmiss hfind (serverAnswers_rd_irrelevant b hans) htrans hacc href
        hunfollow ih
...
theorem accepts_off_path_false (out reply : Datagram)
  ...
  rcases h with h | h <;> simp [accepts, h]
...
/-- **A cached record that entered via a `trustedReferral` forgery — the RFC 5452 in-bailiwick poisoning
    bound made explicit.** The grounding theorems carry this as an escape disjunct: a served record is
    grounded in a real network server UNLESS it arrived through an `accepts`-passing *referral* whose
    delegation cut is an ancestor of the query name (`isAncestor (referralCut reply.msg) q.qname`) and whose
    owner descends from that cut. Even the escape is *bounded* — the poison is confined to names descending
    from a cut on the query's own resolution path, never an unrelated domain. This is exactly the residual
    attack surface of a DNS resolver without DNSSEC (an off-path attacker guessing id+port); the anti-poison
    theorems still prove it can reach no further. -/
...
/-- **A cached record that entered via a `trustedCname` forgery — the CNAME-chase sibling of
    `TrustedReferralCache`.** An `accepts`-passing reply carrying a CNAME the resolver chased writes its
    (bailiwick-filtered) answer section; the write bailiwick is the queried name itself, so the poison is
    confined to records whose owner descends from `q.qname` — an even tighter bound than the referral
    escape's delegation cut. -/
def TrustedCnameCache (ra : String) (ednsBuf : Nat) (r : RR) : Prop :=
  ∃ (addr : String) (id srcPort : Nat) (q : Query) (reply : Datagram),
    accepts (queryDatagram id ra addr srcPort ednsBuf q) reply = true
    ∧ (cnameRR reply.msg.answer).isSome = true
    ∧ isAncestor q.qname r.owner = true

/-- **A cached record that entered via a `trustedReply` forgery — the direct-answer sibling of
    `TrustedCnameCache`.** Since the delivered answer section is cached (RFC 1034 caching of delivered
    answers, the impl-harden closing the warm-cache gap), an `accepts`-passing non-referral reply writes
    its (bailiwick-filtered) answer section; the write bailiwick is the queried name itself, so the
    poison is confined to records whose owner descends from `q.qname` — the same tight bound as the
    CNAME-chase escape. This is RFC 5452's classic answer-forgery cache-poisoning vector, made explicit
    and bounded. -/
...
/-- **Anti-poison, served-record form (RFC 5452 §5).** Every record the *output* cache would serve (its
    `topServed` set, for any time/query) is either already served by the *input* cache `c` or is
    in-bailiwick of some queried server. Phrased on `topServed` (what the resolver actually *returns*)
    rather than raw `pos`, so it is preserved by the `referForget` rule's `CacheRefines` (`topServed`-
    `Subperm`): a resolver that evicts cached answers before recursing still never serves off-path data.
    The impl's stronger storage-side property (every *stored* entry is in-bailiwick) is proved separately
    impl-side (`cacheWrite_pos_in_bailiwick`); the model, viewed through the topServed abstraction, can
    only guarantee — and only needs — the served form. -/
theorem resolves_cache_in_bailiwick {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat} {now : Time}
  ...
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      ...
      · obtain ⟨n3, h3⟩ := hcf.2.1 n2 q2 e h'
        ...
        · exact Or.inl ⟨n4, q2, h''⟩
        ·
          refine Or.inr (Or.inr (Or.inl ⟨addr, id, srcPort, q, reply, hacc, href, hcut, ?_⟩))
...
/-- The `cnameServed`-membership analogue of `cnameAt_eqData`: a CNAME drawn from the served set is
    grounded in `c.pos` (it is the aged image of a real stored entry). Keeps the answer-grounding /
    authoritativeness theorems valid after the `cacheCname` premise moves to membership (Phase 0a). -/
...
/-- **`absorb` provenance (up to TTL).** Every positive entry `absorb` writes is either from the base
    cache or `eqData`-matches (owner/rdata/cls) a record of the response — the TTL may differ because
    the RRset is normalized (RFC 2181 §5.2) before insertion, so exact membership no longer holds. -/
...
theorem absorb_no_authority_soa (c : Cache) (now : Time) (bw : Name) (resp : Response)
  ...
  · rcases step _ _ _ h2 with h1 | h1
    ...
    · -- authority: the record's rdata (= e.rr's, by eqData) is SOA, but authData excludes SOA.
      obtain ⟨r', hr', hd⟩ := h1
...
/-- `Grounded` is `eqData`-closed (both disjuncts pivot on `eqData`), so it transfers along the
    TTL-normalization `absorb` performs on cached records. -/
...
/-- A record that entered the verdict via a `trustedReply` (an `accepts`-passing, possibly off-path,
    reply). The honest-grounding theorems carry this as an escape disjunct: an answer record is grounded
    in the cache / a real server UNLESS it arrived through an `accepts`-passing forgery. -/
def TrustedReplyAnswer (ra : String) (ednsBuf : Nat) (r : RR) : Prop :=
  ∃ (addr : String) (id srcPort : Nat) (q : Query) (reply : Datagram),
    accepts (queryDatagram id ra addr srcPort ednsBuf q) reply = true
    ∧ r ∈ reply.msg.answer ++ reply.msg.authority ++ reply.msg.additional

/-- The `nxdomain`-justification analogue: an NXDOMAIN verdict that arrived via a `trustedReply` forgery. -/
...
/-- A served record (`topServed`) survives `absorb`: anything `c.absorb` serves is either already served
    by `c` or is one of the absorbed response records. The served-set companion of `absorb_pos_provenance`
    — rank-maximality of `e` over the larger `(c.absorb).matching` set carries to the `c.matching` subset
    (`absorb` only inserts, never removes). The form that crosses `CacheRefines` (`topServed`-`Subperm`). -/
...
/-- `hit`-membership refines to `topServed`-membership (the aged record is the served entry up to `ttl`,
    and `served ⊆ topServed`). The served-set analogue of `hit_eqData`. -/
...
/-- The `cnameServed` analogue of `hit_topServed_eqData`: a served CNAME is the aged image of a
    `topServed` entry. -/
...
/-- **Served-set grounding** — `Grounded` with provenance traced through `topServed` rather than raw
    `pos`. This is the form preserved by the `referForget` rule's `CacheRefines` (`topServed`-`Subperm`):
    a resolver that evicts cached answers before recursing still serves only records justified by what it
    (still) serves or by a real server response. -/
...
/-- `GroundedServed` is `eqData`-closed — transfers along the TTL-normalization `absorb` performs. -/
...
/-- **Grounded-or-forged.** The `trustedReferral`-weakened grounding predicate: a served record is grounded
    in a real server OR entered via an `accepts`-passing referral / CNAME chase / direct answer (the
    RFC 5452 in-bailiwick poisoning escapes, bounded by `TrustedReferralCache`/`TrustedCnameCache`/
    `TrustedReplyCache`). The grounding theorems conclude this weaker form; without a `trusted*` write in
    the derivation it collapses to the strong `GroundedServed`. -/
...
/-- `GroundedServed` transfers along a `CacheRefines` shrink of the reading cache: the cache-served
    disjunct's witness survives (the refined cache's `topServed` is a sub-multiset at every read
    time), and the server-grounded disjunct is cache-free. Carries the `cacheCname` eviction slot
    (`hcf`) through the provenance walks. -/
...
/-- `TrustedReferralCache` transfers across `RR.eqData` (its only record-dependence is the owner, and
    `isAncestor` respects `nameEq`) — the escape analogue of the server-answer disjunct's `eqData_trans`. -/
theorem trustedReferralCache_eqData {ra : String} {ednsBuf : Nat} {r r' : RR}
  ...
  obtain ⟨addr, id, srcPort, q, reply, hacc, href, hcut, howner⟩ := h
  exact ⟨addr, id, srcPort, q, reply, hacc, href, hcut,
    isAncestor_congr_right howner ((nameEq_symm r.owner r'.owner) ▸ he.1)⟩

/-- `TrustedCnameCache` transfers across `RR.eqData` — the CNAME-escape sibling of
    `trustedReferralCache_eqData`. -/
theorem trustedCnameCache_eqData {ra : String} {ednsBuf : Nat} {r r' : RR}
  ...
  exact ⟨addr, id, srcPort, q, reply, hacc, hcn,
    isAncestor_congr_right howner ((nameEq_symm r.owner r'.owner) ▸ he.1)⟩

/-- `TrustedReplyCache` transfers across `RR.eqData` — the direct-answer escape sibling of
    `trustedReferralCache_eqData`/`trustedCnameCache_eqData`. -/
theorem trustedReplyCache_eqData {ra : String} {ednsBuf : Nat} {r r' : RR}
  ...
  exact ⟨addr, id, srcPort, q, reply, hacc, hnr,
    isAncestor_congr_right howner ((nameEq_symm r.owner r'.owner) ▸ he.1)⟩

/-- The `GroundedServedF` companion of `groundedServed_of_cache_groundedServed`: cache-provenance carries the
    grounded-or-forged predicate (the escape disjunct is cache-independent and transfers via
    `trustedReferralCache_eqData`). -/
...
/-- `GroundedServedF` is `eqData`-closed (each disjunct is) — transfers along `absorb`'s TTL normalization. -/
...
/-- The `GroundedServedF` companion of `absorb_topServed_groundedServed`. -/
...
theorem resolves_cout_grounded {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat}
  ...
  | refer addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      ...
      exact Or.inl (Or.inr ⟨srv, serverAt_mem hfind, q, _, tr, ref, hans, r', hr', RR.eqData_refl _⟩)
  | referForget addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hwire href hbail frontier hdesc hdescF hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih =>
      intro now' q' e he
      have hmsg := onWire_accepted_honest _ _ _ hwire hacc
  ...
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      ...
      ·
        refine Or.inr (Or.inl ⟨addr, id, srcPort, q, reply, hacc, href, hcut, ?_⟩)
        rwa [absorbBailiwick_of_descendsBelow frontier reply.msg hdesc] at h'
  | answerCname addr rest q srv tr resp cn target id srcPort c nsl ftr rpath tEnd cout final
      hmiss hnmiss hfind hans reply htrans hacc hwire hcn hqt htgt hfresh hmono htc cf0 hcf0 cf hcf hrec ih =>
      intro now' q' e he
      ...
      obtain ⟨n3, h3⟩ := hcf.2.1 n q2 e0 he0
      obtain ⟨n4, h4⟩ := hcf0.2.1 n3 q2 e0 h3
      refine absorb_topServed_groundedServedF { reply.msg with authority := [], additional := [] } ?_ n4 q2 e0 h4
      intro r' hr'
...
theorem resolves_answer_grounded {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat}
  ...
  | refer addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      ...
      · refine Or.inl (groundedServed_of_cache_groundedServed ?_ hg)
        ...
        intro r' hr'; rw [hmsg] at hr'
        exact Or.inr ⟨srv, serverAt_mem hfind, q, _, tr, ref, hans, r', hr', RR.eqData_refl _⟩
      · exact Or.inr ht
  | referForget addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hwire href hbail frontier hdesc hdescF hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih =>
      intro r hr
  ...
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      ...
      · rcases hg with ⟨n, q2, e, he, hre⟩ | hsrv
        · obtain ⟨n3, h3⟩ := hcf.2.1 n q2 e he
          ...
          · refine Or.inr (Or.inr (Or.inl (trustedReferralCache_eqData ⟨addr, id, srcPort, q, reply, hacc, href, hcut, ?_⟩ hre)))
            rwa [absorbBailiwick_of_descendsBelow frontier reply.msg hdesc] at h'
  ...
  | answerCname addr rest q srv tr resp cn target id srcPort c nsl ftr rpath tEnd cout final
      ...
      · rcases ih r hrest with hg | ht
        · refine Or.inl (groundedServed_of_cache_groundedServed ?_ hg)
          ...
          obtain ⟨n3, h3⟩ := hcf.2.1 n q2 e0 he0
          obtain ⟨n4, h4⟩ := hcf0.2.1 n3 q2 e0 h3
          refine absorb_topServed_groundedServed { reply.msg with authority := [], additional := [] } ?_ n4 q2 e0 h4
          intro r' hr'
          simp only [List.append_nil] at hr'
          rw [hmsg] at hr'
...
theorem absorbNeg_neg_mem {c : Cache} {now : Time} {q : Query} {resp : Response} {e : NegRR}
  ...
  · split at he
    · rename_i hrc
      ...
      · exact Or.inr (Or.inl ⟨rcode_eq_of_beq hrc, rfl⟩)
      · exact Or.inl h
    · split at he
      · rename_i hcond
        rcases List.mem_cons.mp he with rfl | h
...
/-- A negative-cache hit after `absorbNeg` is either inherited from `c` or justified by the absorbed
    response being itself an NXDOMAIN / NODATA answer. The `negHit`-boolean companion of
    `absorbNeg_neg_mem`; the form preserved by `CacheRefines` (which equates `negHit`/`negHitNx`). -/
...
/-- **Negative-cache anti-poison (served / `negHit` form).** Any negative answer the *output* cache would
    serve (`negHit`) is either inherited from the input cache `c` or is backed by a real server's NXDOMAIN
    or NODATA response. Phrased on `negHit` (the boolean the resolver reads), so it survives the
    `referForget` rule's `CacheRefines` (which equates `negHit`/`negHitNx`). -/
theorem resolves_negcache_grounded {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat}
    ...
    ∀ now' q', cout.negHit now' q' = true → c.negHit now' q' = true ∨
      ∃ srv ∈ net.servers, ∃ (q'2 : Query) (t : Time) (tr' : List Step) (resp' : Response),
        ServerAnswers srv t [] true q'2 tr' resp' ∧
          (resp'.rcode = RCode.nameError ∨ (resp'.rcode = RCode.noError ∧ resp'.answer = [])) := by
  induction h with
  ...
  | answer addr rest q srv tr resp id srcPort c hmiss hnmiss hfind hans reply htrans hacc hwire hnr hnc htc =>
      ...
      · exact Or.inr ⟨srv, serverAt_mem hfind, q, _, tr, resp, hans, Or.inl (by rw [hfull] at h1; exact h1)⟩
      · exact Or.inr ⟨srv, serverAt_mem hfind, q, _, tr, resp, hans,
  ...
  | rejectSpoof addr rest q ftr rpath tEnd final c cout id srcPort reply hreject hrec ih => intro now' q' hn; exact ih now' q' hn
  | badResponse addr rest q ftr rpath tEnd final c cout id srcPort reply htrans hacc hbad hrec ih => intro now' q' hn; exact ih now' q' hn
  | unfollowableReferral addr rest q srv tr ref id srcPort ftr rpath tEnd final c cout reply hmiss hnmiss hfind hans htrans hacc href hunfollow hrec ih => intro now' q' hn; exact ih now' q' hn
  | cacheCname slist q cn target c nsl ftr rpath tEnd cout final
      hmiss hnmiss hcn hqt htgt hfresh cf hcf hrec ih =>
      intro now' q' hn
      rcases ih now' q' hn with h1 | h1
...
theorem resolves_response_grounded {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat}
  ...
  | rejectSpoof addr rest q ftr rpath tEnd final c cout id srcPort reply hreject hrec ih => intro r hr; exact ih r hr
  | badResponse addr rest q ftr rpath tEnd final c cout id srcPort reply htrans hacc hbad hrec ih => intro r hr; exact ih r hr
  | unfollowableReferral addr rest q srv tr ref id srcPort ftr rpath tEnd final c cout reply hmiss hnmiss hfind hans htrans hacc href hunfollow hrec ih => intro r hr; exact ih r hr
  | cacheCname slist q cn target c nsl ftr rpath tEnd cout final
      hmiss hnmiss hcn hqt htgt hfresh cf hcf hrec ih =>
      intro r hr
      simp only [List.cons_append] at hr
...
/-- A fresh NXDOMAIN negative hit (`negHitNx`) after `absorbNeg` is either inherited from `c` or the
    absorbed response was itself an NXDOMAIN. (The NODATA branch stores `qtype = some _`, which can never
    satisfy `negHitNx`'s `qtype.isNone` gate, so it cannot be the source.) -/
...
/-- **NXDOMAIN negative-cache provenance (`negHitNx` form).** Any fresh NXDOMAIN the output cache serves is
    inherited from `c` or backed by a real server NXDOMAIN. The `negHitNx`-specialised companion of
    `resolves_negcache_grounded`; needed because `nxdomain_justified` must preserve the NX-vs-NODATA
    distinction, which the general `negHit` form drops. Survives `CacheRefines` (equates `negHitNx`). -/
theorem resolves_negHitNx_justified {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat}
    ...
    ∀ now' q', cout.negHitNx now' q' = true → c.negHitNx now' q' = true ∨
      ∃ srv ∈ net.servers, ∃ (q'2 : Query) (t : Time) (tr' : List Step) (resp' : Response),
        ServerAnswers srv t [] true q'2 tr' resp' ∧ resp'.rcode = RCode.nameError := by
  induction h with
  | cacheHit c slist q here hhit hne => intro now' q' hn; exact Or.inl hn
  | negHit c slist q hneg => intro now' q' hn; exact Or.inl hn
  | trustedReply addr origin rest q id srcPort c reply hmiss hnmiss htrans hacc hnr htc cf0 hcf0 cf hcf =>
  ...
  | answer addr rest q srv tr resp id srcPort c hmiss hnmiss hfind hans reply htrans hacc hwire hnr hnc htc =>
      ...
      · exact Or.inr ⟨srv, serverAt_mem hfind, q, _, tr, resp, hans, by rw [hfull] at h1; exact h1⟩
  | refer addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hwire href hbail frontier hdesc hdescF hglue hfresh hmono sl hsl hrec ih =>
      intro now' q' hn
      rcases ih now' q' hn with h1 | h1
...
theorem resolves_nxdomain_justified {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat} {rttOf : String → Nat}
  ...
  | gluelessNs q zone nsHost nsAddr nsNseen nsSeen nsSlist nsTr nsPath nsEnd nsResp
      ...
      · obtain ⟨now2, q2, hh⟩ := hd
        ...
        rcases resolves_negHitNx_justified hnsres now2 q2 hh with h1 | h1
  ...
  | rejectSpoof addr rest q ftr rpath tEnd final c cout id srcPort reply hreject hrec ih => intro hrc; exact ih hrc
  | badResponse addr rest q ftr rpath tEnd final c cout id srcPort reply htrans hacc hbad hrec ih => intro hrc; exact ih hrc
  | unfollowableReferral addr rest q srv tr ref id srcPort ftr rpath tEnd final c cout reply hmiss hnmiss hfind hans htrans hacc href hunfollow hrec ih => intro hrc; exact ih hrc
  | cacheCname slist q cn target c nsl ftr rpath tEnd cout final
      hmiss hnmiss hcn hqt htgt hfresh cf hcf hrec ih =>
      intro hrc
...
theorem resolves_answer_untruncated {net : Network} {ns : NetState} {ra : String} {ednsBuf : Nat}
  ...
  | trustedCname addr origin rest q cn target id srcPort c nsl ftr rpath tEnd cout final
      ...
      show final.tc = false; exact ih
  | refer addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hwire href hbail frontier hdesc hdescF hglue hfresh hmono sl hsl hrec ih => exact ih
  | referForget addr rest q srv tr ref ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss hfind hans reply htrans hacc hwire href hbail frontier hdesc hdescF hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih => exact ih
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      hmiss hnmiss reply htrans hacc href hbail hcut hdesc hfresh hmono sl cf0 hcf0 hsl cf hcf hrec ih => exact ih
  | gluelessNs q zone nsHost nsAddr nsNseen nsSeen nsSlist nsTr nsPath nsEnd nsResp
      slist2 ftr rpath tEnd final c c2 cout
...
theorem serverAnswers_leaf_auth {net : Network} {srv : Server} (hmem : srv ∈ net.servers)
  ...
  | cname q z c target tr' rest hz hd hc hcov ht hfresh hrec ih =>
      ...
      · refine Or.inl ⟨srv, hmem, q, z, hz, hd, r, ?_, RR.eqData_refl r⟩
        have : cnameRR (recordsAt z q.qname) = some r := hc
  ...
  | trustedReferral addr origin rest q frontier ftr rpath tEnd final id srcPort c cout
      ...
      · rcases hg with ⟨n, q2, e, he, hre⟩ | hsrv
        · obtain ⟨n3, h3⟩ := hcf.2.1 n q2 e he
          ...
          rcases absorb_topServed_in_bailiwick _ _ _ _ _ _ _ h4 with h' | h'
          · exact Or.inr (Or.inl (Or.inl ⟨n4, q2, e, h', hre⟩))
          · refine Or.inr (Or.inr (Or.inr (Or.inl (trustedReferralCache_eqData ⟨addr, id, srcPort, q, reply, hacc, href, hcut, ?_⟩ hre))))
            rwa [absorbBailiwick_of_descendsBelow frontier reply.msg hdesc] at h'
        · exact Or.inr (Or.inl (Or.inr hsrv))
      · exact Or.inr (Or.inr ht)
  | timeout addr rest q ftr rpath tEnd final c cout d hdrop hmono hrec ih => exact ih
...
/-- Discharges a `refer`-step bailiwick freshness `serverBailiwick … ∉ seen` in the worked-example
    traces: either `seen = []` (closed by `simp`), or `seen = [bw']` with `bw'` a shallower bailiwick,
    closed by the fact that distinct-depth bailiwicks differ in `length` (`Name = List ByteArray` has no
    `DecidableEq`, so `decide` can't tackle the membership directly, but the `Nat` length comparison can). -/
...
theorem ex_631_iterative_resolution :
    ∃ tr path tEnd cout resp,
      ...
      ∧ path.Nodup := by
  refine ⟨_, _, _, _, _,
    Resolves.refer (now' := 100) "10.0.0.52" _ _ cISI _ _
      _ _ _ _ 4660 5300 _ _ rfl rfl rfl
...
theorem ex_spoof_rejected :
    ∃ tr path tEnd cout resp, Resolves scenario allUp "127.0.0.1" 512 (fun _ => 0) 100 [] [] Cache.empty
      ...
      ∧ resp.answer = [ rr ["MIL"] 86400 (.ns (N ["SRI-NIC","ARPA"])),
                        rr ["MIL"] 86400 (.ns (N ["A","ISI","EDU"])) ] := by
  refine ⟨_, _, _, _, _,
    Resolves.rejectSpoof "10.0.0.52" _ _ _ _ _ _ _ _ 1 5300
...
theorem ex_sibling_resolution :
  ...
  refine ⟨_, _, _, _, _,
    ...
    Resolves.referForget (now' := 100) "1.1.1.1" _ _ glRoot _ _ _ _ _ _ 1 5300 _ _
      rfl rfl rfl
...
theorem ex_resolve_from_root_hints :
  ...
  refine ⟨_, _, _, _, _,
    Resolves.refer (now' := 100) "198.41.0.4" _ _ rhRoot _ _ _ _ _ _ 1 5300 _ _ rfl rfl rfl
      (ServerAnswers.referral _ _ _ rfl rfl rfl)
      ...
      (Resolves.refer (now' := 100) "192.5.6.30" _ _ rhEdu _ _ _ _ _ _ 2 5300 _ _ rfl rfl rfl
        (ServerAnswers.referral _ _ _ rfl rfl rfl)
        _ (Transit.deliver _ _ _ (by decide) (by decide)) (accepts_reply _ _ _ _ _ _ _) OnWire.fromServer rfl (by decide) (N ["EDU"]) (by decide) (by decide) (by decide) (by bailiwickFresh) (by decide)
...
/-- **The RTT-sorted glue SLIST is a permutation of the raw glue addresses.** `sortByRtt` reorders by the
    resolver's `rttOf` table but neither drops nor duplicates a server (`sortedByRtt_perm` + `map`-stability),
    and `glueEntries` just pairs each address with its RTT — so the model's referral SLIST
    `sortByRtt (glueEntries rttOf ref)` is order-only-different from `glueAddresses ref`. This is the
    order-bridge half of the SLIST connector (the forward-simulation refer step): combined with `chooseServer`
    it lets a verdict on the implementation's glue SLIST reroute to the model's pinned RTT order. -/
...
/-- **The RTT-sorted cache-re-derived referral SLIST is a permutation of `referralSlist`.** Exact analogue of
    `sortByRtt_glueEntries_perm` but for the cache-re-derived SLIST: `sortByRtt` reorders by `rttOf` without
    dropping or duplicating a server, and `referralEntries` just pairs each `referralSlist` address with its RTT —
    so the model `refer` rule's recursive SLIST `sortByRtt (referralEntries rttOf c now nm fuel)` is order-only-
    different from `referralSlist c now nm fuel`. This is the order-bridge half of the SLIST connector for the
    cache-re-derived refer step (chunk 3 ctor switch): combined with the keystone correspondence
    `modelSlistOf(impl re-derived) = referralSlist` it lets a verdict on the impl's all-addresses SLIST reroute
    to the model's pinned RTT order. -/
...
/-- **In a well-formed zone, every delegation NS record's owner equals the delegation apex** (`Zone.WF`
    conjunct 4). Feeds the referral `hbail`: each NS authority record's owner `nameEq d.subapex`, and
    `bestDeleg` gives `isAncestor d.subapex qname`, so `isAncestor owner qname` (`isAncestor_congr_left`). -/
...
/-- **A well-formed delegation's NS set contains no SOA** (`Zone.WF` conjunct 4: every `d.nsSet` record has
    `rdata.rtype = NS`, and `NS ≠ SOA`). The `!authority.any(SOA)` half of showing an honest referral response
    (whose `authority = d.nsSet`) has `Response.isReferral = true` — the model side of refer Arm A's `href`. -/
theorem Zone.WF_nsSet_no_soa {z : Zone} (h : z.WF) {d : Delegation} (hd : d ∈ z.delegations) :
  ...
  obtain ⟨-, h1⟩ := hns
  revert h1 hsoa
  cases r.rdata.rtype <;> decide

/-- **In a well-formed zone, every delegation is strictly below the apex** (`Zone.WF` conjunct 3):
    `isAncestor z.apex d.subapex ∧ d.subapex ≠ z.apex`. Feeds the referral `hdesc`
    (`Response.descendsBelow (serverBailiwick …) = isAncestor apex (referralCut) && apex.length < cut.length`). -/
...
/-- **The core referral-`hbail` derivation (model-side, no impl `suffixMatchCount` loop).** A referral
    response whose authority is `d.nsSet` for the best delegation `d = bestDeleg z qname` in a well-formed
    zone is in-bailiwick for `qname`: every NS owner equals `d.subapex` (`Zone.WF_nsSet_owner`), and
    `d.subapex` is an ancestor of `qname` (`bestDeleg_isAncestor`), so `isAncestor owner qname`
    (`isAncestor_congr_left`), giving `Response.inBailiwick`. The driver's `.continue` refer case supplies
    `z.WF` (from `Network.WF` + `bestZone_spec`) and `ref.authority = d.nsSet` (from `ServerAnswers.referral`). -/
...
/-- **The core referral-`hdesc` derivation (model-side, no impl `suffixMatchCount` loop).** A referral
    response whose authority is `d.nsSet` for the best delegation `d = bestDeleg z qname` in a well-formed
    zone *descends strictly below* the zone apex: the first NS record `r₀` (all `d.nsSet` records are NS,
    `Zone.WF` conjunct 4; `d.nsSet` nonempty, conjunct 7) is the `referralCut`, its owner `nameEq d.subapex`,
    and `Zone.WF` conjunct 3 gives `isAncestor z.apex d.subapex ∧ ¬nameEq d.subapex z.apex`. So
    `isAncestor z.apex (referralCut)` (`isAncestor_congr_right`) and `z.apex.length < (referralCut).length`
    (`isAncestor_length_le` plus `isAncestor_eq_length_nameEq` refuting the equal-length case against the
    `¬nameEq` witness). Feeds the driver's `.continue` refer case with `serverBailiwick srv … = z.apex`. -/
...
/-- **The packaged honest-referral bailiwick+descent producer (Arm A of the forward-simulation refer step).**
    From a well-formed network, a server located at `addr`, and the `ServerAnswers.referral` witnesses
    (`bestZone srv … = some z`, `bestDeleg z q.qname = some d`, `ref.authority = d.nsSet`), derive BOTH
    hypotheses the refer producer `serverReferForget_hasVerdict` consumes: `ref.inBailiwick q.qname` (via
    `inBailiwick_of_bestDeleg`) and `ref.descendsBelow (serverBailiwick srv q.qname q.qclass)` (via
    `descendsBelow_of_bestDeleg`, rewriting `serverBailiwick srv q.qname q.qclass = z.apex` from `hz`). The
    zone's `Zone.WF` comes from `Network.WF net` + `serverAt_mem` + `bestZone_spec`. This is the exact form
    the driver's `.continue` honest-refer arm plugs into the producer — no imperative `suffixMatchCount` loop. -/
...
/-- **Ancestors of a common name are totally ordered by length.** If `a` and `b` are both ancestors of `n`
    and `a.length ≤ b.length`, then `a` is an ancestor of `b` (the shorter suffix of a fixed name is a suffix
    of the longer one). The chain-ordering fact behind the refer-descent structural argument. -/
...
/-- **A followable referral's server bailiwick is at least as deep as any prior delegation cut** (the
    refer-descent freshness core, from `Network.WF`'s global-tree conjunct). If `srv`'s best zone `z`
    delegates `d` for `q.qname`, and `pc` is some zone apex (a prior cut) that is an ancestor of `q.qname`
    strictly shallower than the new cut `d.subapex`, then `pc.length ≤ z.apex.length = serverBailiwick`.
    Reason: were `z.apex` strictly shallower than `pc`, then `pc` would sit strictly between `z.apex` and
    `d.subapex`, so a zone (apex `pc`) would be authoritative inside `z`'s delegated span — forbidden by the
    tree conjunct. Discharges the driver's `hFloorLe`/freshness at the ZONE refer arm. -/
...
theorem wf_cname_forces_redirection {z : Zone} (h : z.WF) {qname : Name} {c : RR}
    ...
    (hcn : cnameRR (recordsAt z qname) = some c) (hqt : qt.covers RRType.cname = false) :
