...
-- NODATA terminal: the `com` node exists but carries no A record (and no CNAME),
-- so the authoritative leaf answers noError/empty-answer with a SOA authority
-- (`childNeg` = an SOA owned by `com`, an ancestor of the query name `com`).
-- Exercises the `resolveWithIO_flatAuthoritative_nodata_adequate` delivery path
-- through the SOA-gated NODATA classifier arm.
def nodataT : Node ResourceRecord := .mk ByteArray.empty #[] #[.mk (lab "com") #[] #[]]

def flatNodataDelivered : Bool :=
  let respondFlat : ByteArray → Format → Format := fun _ q => treeRespond nodataT childNeg q
  match Prog.run 400 (Server.resolveWithIO (M := Prog) (Sock := Unit)
      pinQuery pinSbelt DnsCache.empty 100000 40 6 5)
      { w0 with oracle := mkHonestOracleAddr respondFlat } with
  | some ((.ok resp, _), w') =>
...
/-! ### Finding 061: live-clock deadline vectors

The model clock is LIVE: every network exchange advances `World.clock` by
`World.tick World.exchCtr`.  The two vectors below are the decidable halves of
the deadline bivalence:

* `deadlineFiresMidResolution` — a slow world (10 time units per exchange
  against a 5-unit budget) takes the first (root-referral) round, crosses the
  deadline, and the SECOND loop iteration returns the deadline error.  A
  mutant that deletes the `t ≥ deadline` check in `ioResumeLoop` — or answers
  instead of erroring — flips this vector (theorem twin:
  `FreeIO.run_resolveWithIO_deadline_witness`).
* `deadlineSurvivesFastWorld` — a fast-but-nonzero schedule (2 units per
  exchange) stays under the budget and the answer is delivered with the exact
  elapsed time on the final clock.  (`depth1AnswerDelivered` above is the
  `tick ≡ 0` frozen-clock twin.) -/

def deadlineFiresMidResolution : Bool :=
  match Prog.run 400 (Server.resolveWithIO (M := Prog) (Sock := Unit)
      pinQuery pinSbelt DnsCache.empty 100000 40 6 5)
      { w0 with tick := fun _ => 10 } with
  | some ((.error msg, _), w') =>
    msg == "resolveWithIO: query deadline exceeded"
      && w'.clock == 100010     -- exactly one exchange's latency elapsed
      && w'.exchCtr == 1
      && w'.idCtr == 2          -- exactly one round was taken before the cutoff
  | _ => false

#guard deadlineFiresMidResolution

def deadlineSurvivesFastWorld : Bool :=
  match Prog.run 400 (Server.resolveWithIO (M := Prog) (Sock := Unit)
      pinQuery pinSbelt DnsCache.empty 100000 40 6 5)
      { w0 with tick := fun _ => 2 } with
  | some ((.ok resp, _), w') =>
    resp.header.rcode == Rcode.noError
      && resp.answer == #[RRParse.rrBytes comARR]
      && w'.clock == 100004    -- two exchanges, two units each
      && w'.exchCtr == 2
  | _ => false

#guard deadlineSurvivesFastWorld
