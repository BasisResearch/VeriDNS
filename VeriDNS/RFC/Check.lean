...
/-- Resolve an optional `via <proof>` clause: register that `proof` discharges the
    statement `stmt`, attach a "proven by" citation to `stmt`, and return the proof's
    name together with the colour the coverage span should take (the proof's status).
    Returns `none` when there is no `via` clause, in which case the caller uses
    `stmt`'s own status and an empty `provenBy`. -/
