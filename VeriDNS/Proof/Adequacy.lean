...
theorem terminates_forwardQuery_bind {α} (query : Format) (addr : ByteArray)
    (k : Option Format → Prog α) (w : World) (h : ∀ v, Terminates (k v) w.afterExchange) :
                      ...
                      ·
                        ...
                        · split
                          · -- 055 (RFC 6891 §6.2.2): FORMERR arm — EDNS-free retry recursion
                            ...
                            exact IHfuel fuel' (by omega) _ _ _
                          · split
                            · -- 051/064: probe-NXDOMAIN arm now recurses (full-qname fallback)
                              refine terminates_log_bind _ _ _ ?_
                              exact IHfuel fuel' (by omega) _ _ _
                            · split
                              ·
...
theorem Delivers_referral_step
  ...
  obtain ⟨w', htransfer, ho, hto, hids, hclk, hectr, htick, hctr⟩ :=
    run_ioResumeLoop_referral_lift sbelt state deadline depth fuel' revealed w
      ...
      hunfollow hcont hfe
  obtain ⟨K, w'', hK⟩ := hnext w' ho hto hids hclk hectr htick hctr
...
theorem DescentChain.of_descent {sbelt : DnsSList} {deadline : UInt32} {depth : Nat}
  ...
  have key : ∀ (n : Nat)
    ...
    | _ n IH =>
      ...
      · subst hfuel
        exact hnode (fun w' ho hto hids hclk hectr htick hctr =>
          IH (μ st) (by omega) st fuel' revealed' w' rfl
            (hInv' w' ho hto hids hclk hectr htick hctr))
