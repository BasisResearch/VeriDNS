...
theorem storeProbeNegative_negWriteRefines
  ...
  | some p =>
    ...
    exact storeNegative_negWriteRefines_prepend hmme hqn hexp
      ...
      (VeriDNS.Spec.Net.absorbNeg_ttl_is_soa_minimum c now pq reply.msg negTtl.toNat hrcM hm)
                            ...
                            ·
                              ...
                              · rw [if_neg hunf] at hrun
                                ...
                                case pos =>
                                  ...
                                  have hresEq := Except.ok.inj hres
                                  subst hresEq
                                  subst hcoutEq
                                  subst hwEq
                                  obtain ⟨hmme, hsn, htm, hwm⟩ := hSM
                                  obtain ⟨hprC, hstD⟩ := (Bool.and_eq_true _ _).mp hstT
                                  obtain ⟨-, hrcI, htcI⟩ := strictDenialB_facts hstD
                                  obtain ⟨pq, hαQp, hSP⟩ := αQuery_buildSubQuery_probe hbuild hprC hsn hqm
                                  obtain ⟨origin, reply, srcPort, htrans, hacc, hragS, _hclsLink, htcR, hauthS,
                                      hvalidAnsW, hvldA, _hrefImpl⟩ :=
                                    tcpSpoofReply_of_honest net ns ra ednsBuf now hnetWF w subQuery0
                                      (w.ids w.idCtr) (w.ids (w.idCtr + 1)) (Server.ipv4ToAddr ipAddr)
                                      bytes2 raw2 tcpResp tcpRespA pq hwmTcp hOr hdec2 hsan2 hacc2 hαQp htc0
                                  ...
                                  have hrcA : (αResp tcpRespA).rcode = RCode.nameError := by
                                    rw [(αResp_components tcpRespA).1, hrcI]
                                    rfl
                                  have hcanonMin : ∃ na,
                                      αName (VeriDNS.Impl.DomainName.minimisedName state.resources.sname revealed)
                                        = some na
                                      ∧ VeriDNS.Impl.DomainName.minimisedName state.resources.sname revealed
                                        = VeriDNS.Impl.DomainName.labelsToWireFormatGo na
                                      ∧ ∀ x ∈ na, x.size ≤ 63 := by
                                    refine ⟨q.qname.drop (q.qname.length - revealed),
                                      VeriDNS.Proof.QnameMin.αName_minimisedName hsn revealed, ?_, ?_⟩
                                    · rw [hsnameCanon]
                                      exact VeriDNS.Proof.QnameMin.minimisedName_go hqvalid revealed
                                    · intro x hx
                                      exact (hqvalid x (List.mem_of_mem_drop hx)).2
                                  obtain ⟨q₀B, quB, hq0B, hquB, hsubqB⟩ := buildSubQuery_inv state subQuery0 revealed hbuild
                                  have hsubQ0 : subQuery0.question[0]?
                                      = some (Resolver.subQuestion state.resources.sname revealed quB) := by
                                    rw [hsubqB]
                                    rfl
                                  have hcoutPack :
                                      CacheWf (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now) state.now
                                      ∧ CacheNsCanon (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now)
                                      ∧ CacheCnameCanon (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now)
                                      ∧ (∀ e ∈ (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now).records,
                                          VeriDNS.Proof.NameTree.WfRR e.rr)
                                      ∧ (∀ qu : VeriDNS.Spec.Question,
                                          (∃ q₀, state.lastQuery = some q₀ ∧ q₀.question[0]? = some qu) →
                                          CacheNegWf (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now) qu.qclass)
                                      ∧ CacheNsDistinct (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now)
                                      ∧ VeriDNS.Proof.NameTree.OneExpiryPerKey (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now)
                                      ∧ (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now).records.size ≤ DnsCache.capacity := by
                                    rcases storeProbeNegative_cases state.resources.cache subQuery0 tcpRespA state.now with
                                      heq | ⟨quW, negTtlW, soaRRW, hquW, hextW, heq⟩
                                    · rw [heq]
                                      exact ⟨hCacheWf, hNsCanon, hCnCanon, hwfrr, hNegWf, hNsDistinct, hOE, hCap⟩
                                    · rw [heq]
                                      obtain ⟨hwfP, hnsP, hcnP, hwfrrP, hnsdP, hoeP⟩ :=
                                        storeNegative_records_package (name := quW.qname) (qt := quW.qtype)
                                          (qc := quW.qclass) (rc := tcpRespA.header.rcode)
                                          (soa := some { soaRRW with ttl := Server.capNegativeTtl negTtlW })
                                          (expiry := state.now + (Server.capNegativeTtl negTtlW).toNat.toUInt32)
                                          hCacheWf hNsCanon hCnCanon hwfrr hNsDistinct hOE
                                      refine ⟨hwfP, hnsP, hcnP, hwfrrP, ?_, hnsdP, hoeP, ?_⟩
                                      · intro qu hqu'
                                        obtain ⟨q₀', hlq', hqu''⟩ := hqu'
                                        obtain rfl : q₀' = q₀B :=
                                          Option.some.inj (hlq'.symm.trans hq0B)
                                        obtain rfl : qu = quB :=
                                          Option.some.inj (hqu''.symm.trans hquB)
                                        have hquWeq : quW = Resolver.subQuestion state.resources.sname revealed qu :=
                                          Option.some.inj (hquW.symm.trans hsubQ0)
                                        rw [hquWeq, subQuestion_probe hprC]
                                        exact CacheNegWf_storeNegative _ _ _ _ _ _
                                          (hNegWf qu ⟨q₀', hlq', hqu''⟩) (Or.inl hrcI) hcanonMin
                                      · rw [storeNegative_records]
                                        exact hCap
                                  obtain ⟨hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE⟩ := hcoutPack
                                  have hrcMS : reply.msg.rcode = RCode.nameError := hragS.1.symm.trans hrcA
                                  exact ⟨byteAddrToModel (Server.ipv4ToAddr ipAddr) ::
                                    ...
                                    αResp tcpRespA,
                                    αCache (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now),
                                    ancestorDenied_hasVerdictAt net ns ra ednsBuf rttOf
                                      (byteAddrToModel (Server.ipv4ToAddr ipAddr)) origin
                                      ((modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)))
                                      q pq (w.ids w.idCtr).toNat srcPort c reply
                                      hmiss hnmiss hSP htrans hacc hrcMS htcR
                                      (αResp tcpRespA) ⟨hragS.1, hragS.2⟩
                                      (c.absorbNeg now pq reply.msg) (Or.inl (WriteRefines.refl now _))
                                      (αCache (Server.storeProbeNegative state.resources.cache subQuery0 tcpRespA state.now))
                                      (storeProbeNegative_negWriteRefines hmme htm hclock hαQp
                                        hauthS hvldA hrcI hrcMS),
                                    hpermAns.symm.subperm,
                                    (finalizeAnswer_abstracts_rcode _ tcpRespA).trans (αResp_components tcpRespA).1.symm,
                                    by rw [(αResp_components _).2.1, finalizeAnswer_answer, αSection_prependChain,
                                      (αResp_components tcpRespA).2.1],
                                    CacheRefines.refl _,
                                    WorldModels_oracle net ns ra ednsBuf now rfl hwm,
                                    hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE,
                                    answerWriteWf_finalize (by exact hAW) hAWwire⟩
                                ...
                                ·
                                  ...
                                  ·
                                    ...
                                    have hcoutB : cacheR = (Resolver.cacheUnlessTruncated (RR := VeriDNS.Spec.ResourceRecord)
                                      ...
                                      (afterResume_finished_payload_pos _ entry.name tcpRespA resp hstepM hcn hnotbiz.1 hnotbiz.2 hansI hAR).2
                                    subst hcoutB
                                    ...
                                    ·
                                      ...
                                      · have hchainM : (({ state with resources := { state.resources with slist := state.resources.slist.markQueried entry.name } } : Resolver.State DnsSList DnsCache VeriDNS.Spec.SlistEntry VeriDNS.Spec.ResourceRecord)).cnameChain = #[] := hch
                                        ...
                                        exact ⟨byteAddrToModel (Server.ipv4ToAddr ipAddr) :: (modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)), (αResp resp),
                                          ...
                                          hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE, by
                                            rw [(afterResume_finished_payload_pos _ entry.name tcpRespA resp hstepM hcn hnotbiz.1 hnotbiz.2 hansI hAR).1]
                                      ...
                                      ·
                                        ...
                                        obtain ⟨out, coutP, hout, hrcP, hansP⟩ := afterResume_answer_payload
                                          ...
                                          entry.name tcpRespA hstepM hcn hnotbiz.1 hnotbiz.2 hansI
                                        ...
                                        exact ⟨byteAddrToModel (Server.ipv4ToAddr ipAddr) :: (modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)), αResp tcpRespA,
                                          ...
                                          hcrE, WorldModels_oracle net ns ra ednsBuf now rfl hwm,
                                          hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE, by
                                            rw [(afterResume_finished_payload_pos _ entry.name tcpRespA out hstepM hcn hnotbiz.1 hnotbiz.2 hansI hAR).1]
                                            exact answerWriteWf_finalize (by exact hAW) hAWwire⟩
                                  ·
                                    ...
                                    ·
                                      ...
                                      ·
                                        ...
                                        subst hcoutW
                                        have hcf0 := answer_write_WriteRefines_echo state.resources.cache tcpRespA qaR state.resources.sname
                                    ...
                                    · by_cases hbiz : (tcpRespA.header.rcode == VeriDNS.Spec.Rcode.serverFailure
                                      ...
                                      · rw [Bool.not_eq_true] at hbiz
                                        ...
                                        rcases stepAnalyzeResponse_goto_cases
                                          ...
                                          with ⟨-, hfacts⟩ | ⟨-, -, h4bL, hrefL, hnodataL⟩
                                        rotate_left
                                        ·
                                          have hansE : tcpRespA.answer.isEmpty = true := by
                                            have h := h4bL; simp only [Bool.and_eq_true] at h; exact h.1.2
                                          have hnoerr : (tcpRespA.header.rcode == VeriDNS.Spec.Rcode.noError) = false := by
                                            cases hb : (tcpRespA.header.rcode == VeriDNS.Spec.Rcode.noError) with
                                            | false => rfl
                                            | true => rw [hb, hansE] at hnodataL; exact absurd hnodataL (by decide)
                                          have hrs : Server.referralShapedB tcpRespA = false := by
                                        ...
                                        have hnsne : Resolver.extractNsNames (RR := VeriDNS.Spec.ResourceRecord) tcpRespA.authority ≠ #[] :=
                                          extractNsNames_ne_of_hasRRTypeIn tcpRespA.authority hfacts.2.2.2.2.1
                                        ...
                                        ·
                                          have hslist'' : state''.resources.slist
                                              = VeriDNS.Spec.SlistFromNameSpec.setUpAddresses (S := SList.DnsSList) (NS := VeriDNS.Spec.SlistEntry)
                                                (Resolver.extractNsNames (RR := VeriDNS.Spec.ResourceRecord) tcpRespA.authority)
                                          ...
                                          have hGlProv'' : GluelessProv state''.resources.slist := by
                                            rw [hslist'']
                                            exact GluelessProv_fromNsWithGlueAll_of_canonical _ _ _
                                              (extractNsNames_canonical tcpRespA.authority (by
                                                intro b hb
                                          ...
                                          have hgae : glueAddresses (αResp tcpRespA) = glueAddresses reply.msg := by
                                            unfold glueAddresses VeriDNS.Spec.Net.referredServers VeriDNS.Spec.Net.referralCut
                                            rw [show (αResp tcpRespA).authority = reply.msg.authority from hauthE,
                                              show (αResp tcpRespA).additional = reply.msg.additional from haddE]
                                          ...
                                          have hvalidAdd' : ∀ b ∈ tcpRespA.additional.toList, ∀ rr,
                                            ...
                                            rw [hpr'] at hpr; injection hpr with h; subst h; exact hne
                                          have hconn := glueAddresses_subperm_transient tcpRespA (Resolver.referralCutRaw (RR := VeriDNS.Spec.ResourceRecord) tcpRespA.authority) (Resolver.delegationMatchCount (RR := VeriDNS.Spec.ResourceRecord) tcpRespA.authority state.resources.sname) hcutRef hvalidAuth'
                                            hvalidAdd' hnsCanonG
                          ...
                          ·
                            ...
                            case pos =>
                              ...
                              have hwmApp := hwm subQuery0 (w.ids w.idCtr) (w.ids (w.idCtr + 1))
                                (Server.ipv4ToAddr ipAddr) d bytes resp0 respS respA pq hO ha hdec hsani haccR hαQp
                              ...
                              have hrcA : (αResp respA).rcode = RCode.nameError := by
                                rw [(αResp_components respA).1, hrcI]
                                rfl
                              have hcanonMin : ∃ na,
                                  αName (VeriDNS.Impl.DomainName.minimisedName state.resources.sname revealed)
                                    = some na
                              ...
                              have hsubQ0 : subQuery0.question[0]?
                                ...
                                rfl
                              have hcoutPack :
                                  CacheWf (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now) state.now
                                  ∧ CacheNsCanon (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now)
                                  ∧ CacheCnameCanon (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now)
                                  ∧ (∀ e ∈ (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now).records,
                                      VeriDNS.Proof.NameTree.WfRR e.rr)
                                  ∧ (∀ qu : VeriDNS.Spec.Question,
                                      (∃ q₀, state.lastQuery = some q₀ ∧ q₀.question[0]? = some qu) →
                                      CacheNegWf (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now) qu.qclass)
                                  ∧ CacheNsDistinct (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now)
                                  ∧ VeriDNS.Proof.NameTree.OneExpiryPerKey (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now)
                                  ∧ (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now).records.size ≤ DnsCache.capacity := by
                                rcases storeProbeNegative_cases state.resources.cache subQuery0 respA state.now with
                                  heq | ⟨quW, negTtlW, soaRRW, hquW, hextW, heq⟩
                                · rw [heq]
                                  exact ⟨hCacheWf, hNsCanon, hCnCanon, hwfrr, hNegWf, hNsDistinct, hOE, hCap⟩
                                · rw [heq]
                                  obtain ⟨hwfP, hnsP, hcnP, hwfrrP, hnsdP, hoeP⟩ :=
                                    storeNegative_records_package (name := quW.qname) (qt := quW.qtype)
                                      (qc := quW.qclass) (rc := respA.header.rcode)
                                      (soa := some { soaRRW with ttl := Server.capNegativeTtl negTtlW })
                                      (expiry := state.now + (Server.capNegativeTtl negTtlW).toNat.toUInt32)
                                      hCacheWf hNsCanon hCnCanon hwfrr hNsDistinct hOE
                                  refine ⟨hwfP, hnsP, hcnP, hwfrrP, ?_, hnsdP, hoeP, ?_⟩
                                  ...
                                  · rw [storeNegative_records]
                                    exact hCap
                              obtain ⟨hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE⟩ := hcoutPack
                              rcases hwmApp with ⟨srv, tr, ref, hfind, hans, hragA, hreachA, hfit, hisref, hcnbi,
                                _hcncorr, hvalid, hvalidAuth, -, hnstail⟩ |
                                ⟨origin, reply, srcPort, htrans, hacc, hragS, _hclsLink, htcR, hauthS, _hvld, hvldA, _hrefImpl⟩
                              ·
                                have hrcMH : ref.rcode = RCode.nameError := hragA.1.symm.trans hrcA
                                have htcMH : ref.tc = false := by
                                  rw [← hnstail.2.2.2.2.2.2]
                                  exact htcI
                                obtain ⟨htransH, haccH, -⟩ := honest_wire_premises net ns ra
                                  (byteAddrToModel (Server.ipv4ToAddr ipAddr)) (w.ids w.idCtr).toNat 0 ednsBuf pq ref
                                  hreachA (VeriDNS.Proof.WorldNetwork.reach_self net ns ra)
                                exact ⟨byteAddrToModel (Server.ipv4ToAddr ipAddr) ::
                                    (modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)),
                                  αResp respA,
                                  αCache (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now),
                                  ancestorDenied_hasVerdictAt net ns ra ednsBuf rttOf
                                    (byteAddrToModel (Server.ipv4ToAddr ipAddr)) (byteAddrToModel (Server.ipv4ToAddr ipAddr))
                                    ((modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)))
                                    q pq (w.ids w.idCtr).toNat 0 c
                                    (replyDatagram (queryDatagram (w.ids w.idCtr).toNat ra
                                      (byteAddrToModel (Server.ipv4ToAddr ipAddr)) 0 ednsBuf pq) ref)
                                    hmiss hnmiss hSP htransH haccH hrcMH htcMH
                                    (αResp respA) ⟨hragA.1, hragA.2⟩
                                    (c.absorbNeg now pq ref) (Or.inl (WriteRefines.refl now _))
                                    (αCache (Server.storeProbeNegative state.resources.cache subQuery0 respA state.now))
                                    (storeProbeNegative_negWriteRefines
                                      (reply := replyDatagram (queryDatagram (w.ids w.idCtr).toNat ra
                                        (byteAddrToModel (Server.ipv4ToAddr ipAddr)) 0 ednsBuf pq) ref)
                                      hmme htm hclock hαQp hnstail.2.2.1 hvalidAuth hrcI hrcMH),
                                  hpermAns.symm.subperm,
                                  (finalizeAnswer_abstracts_rcode _ respA).trans (αResp_components respA).1.symm,
                                  by rw [(αResp_components _).2.1, finalizeAnswer_answer, αSection_prependChain,
                                    (αResp_components respA).2.1],
                                  CacheRefines.refl _,
                                  WorldModels_oracle net ns ra ednsBuf now rfl hwm,
                                  hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE,
                                  answerWriteWf_finalize (by exact hAW) hAWwire⟩
                                    ...
                                    · have hbridge : RespAgree (αResp resp) { ref with aa := false } :=
                                      ...
                                      exact ⟨byteAddrToModel (Server.ipv4ToAddr ipAddr) :: (modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)), (αResp resp), c,
                                        VeriDNS.Proof.WorldNetwork.serverAnswer_hasVerdictAt net ns ra ednsBuf rttOf
                                          ...
                                          hcrE, WorldModels_oracle net ns ra ednsBuf now rfl hwm,
                                          hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE, by
                                            rw [(afterResume_finished_payload_pos _ entry.name respA resp hstepM hcn hnotbiz.1 hnotbiz.2 hansI hAR).1]
                                            exact answerWriteWf_finalize (by exact hAW) hAWwire⟩
                                  ...
                                  ·
                                    ...
                                    · obtain ⟨r, hrmem, hrcov⟩ := positive_answer_covered hαQ hqm2 hansI hvalid hragA
                                      exact ⟨byteAddrToModel (Server.ipv4ToAddr ipAddr) :: (modelSlistOf state.resources.slist).erase (byteAddrToModel (Server.ipv4ToAddr ipAddr)), αResp respA, c,
                                        ...
                                        hcrE, WorldModels_oracle net ns ra ednsBuf now rfl hwm,
                                        hwfE, hnsE, hcnE, hwfrrE, hnegE, hnsdE, hoeE, hcapE, by
                                          rw [(afterResume_finished_payload_pos _ entry.name respA out hstepM hcn hnotbiz.1 hnotbiz.2 hansI hAR).1]
                                          exact answerWriteWf_finalize (by exact hAW) hAWwire⟩
                                    · obtain ⟨origin, reply, srcPort, htrans, hacc, hragS, _hclsLink, htcR, _hauthS, hvld, _hvldA, _hrefImpl⟩ := hspoof
                              ...
                              ·
                                ...
                                ·
                                  ...
                                  ·
                                    ...
                                    subst hcoutW
                                    have hcf0 := answer_write_WriteRefines_echo state.resources.cache respA qaR state.resources.sname
                                      ...
                                      (by
                                        ...
                                        ·
                                          have habs : c.absorb now q.qname
                                              (({ aa := (respA.header.aa == 1), rcode := RCode.noError, answer := αSection respA.answer,
                                                  authority := [], additional := [], ra := false, tc := false } : Response).answerOwned q.qname)
                                              = c.absorb now q.qname (ref.answerOwned q.qname) :=
                                            absorb_resp_congr _ _ _ (by simpa using haaEq)
                                              (by simp only [Response.answerOwned_answer]; rw [hansEq]) rfl rfl
                              ...
                              case pos =>
                                ...
                                · by_cases hbiz : (respA.header.rcode == VeriDNS.Spec.Rcode.serverFailure
                                  ...
                                  · rw [Bool.not_eq_true] at hbiz
                                    ...
                                    rotate_left
                                    ·
                                      have hansE : respA.answer.isEmpty = true := by
                                        have h := h4bL; simp only [Bool.and_eq_true] at h; exact h.1.2
                                      have hnoerr : (respA.header.rcode == VeriDNS.Spec.Rcode.noError) = false := by
                                        cases hb : (respA.header.rcode == VeriDNS.Spec.Rcode.noError) with
                                        | false => rfl
                                        | true => rw [hb, hansE] at hnodataL; exact absurd hnodataL (by decide)
                                      have hrs : Server.referralShapedB respA = false := by
                                        unfold Server.referralShapedB
                                        rw [hnoerr]
                                        simp
                                  ...
                                  ·
                                    have habs : c.absorb now q.qname
                                      ...
                                      absorb_resp_congr _ _ _ (by simpa using hnstail2.2.2.2.2.1)
                                        (by simp only [Response.answerOwned_answer]; rw [hcncorr]) rfl rfl
