    ...
    Server.serveTcpDatagram (M := Prog) (Sock := Unit) connSock acl sbelt cache queryBytes clientAddr
      = (do
        ...
        VeriDNS.Spec.UdpSocket.tcpSend (M := Prog) (Sock := Unit) (Addr := ByteArray) connSock
          (VeriDNS.Impl.TcpFraming.frameTcp (VeriDNS.Impl.Message.encode
            (VeriDNS.Impl.Edns.withReplyOpt query response)))
...
theorem serveTcpDatagram_verdict_sound
    ...
    ∃ (m : Nat) (rr : Except String VeriDNS.Spec.Format) (cache' : DnsCache) (w₂ : World),
      ...
      ∧ ((∃ msg, rr = .error msg
            ...
            ∧ w'.tcpSent = w.tcpSent
                ++ [VeriDNS.Impl.TcpFraming.frameTcp (VeriDNS.Impl.Message.encode
                    (VeriDNS.Impl.Edns.withReplyOpt query (Server.finalizeForClient
            ...
            ∧ w'.tcpSent = w.tcpSent
                ++ [VeriDNS.Impl.TcpFraming.frameTcp (VeriDNS.Impl.Message.encode
                    (VeriDNS.Impl.Edns.withReplyOpt query
                      (Server.deliveredResponse query resp)))])) := by
  ...
  | error msg =>
    ...
    have htcp : w₄.tcpSent = w.tcpSent
        ++ [VeriDNS.Impl.TcpFraming.frameTcp (VeriDNS.Impl.Message.encode
            (VeriDNS.Impl.Edns.withReplyOpt query (Server.finalizeForClient
              (Server.buildErrorResponse query Rcode.serverFailure))))] := by
      have h4 := run_tcpSend_inv hrunSend
      have h3 := (VeriDNS.Proof.SentMinimised.replyForResolution_sends_frame
        query (.error msg) cache' w.clock hrunReply).2
      have h2 := (VeriDNS.Proof.SentMinimised.resolveWithIO_sends_frame
        query sbelt cache w.clock hrunRes).2
      rw [h4]
      simp only [hrespE, h3, h2]
  ...
  | ok resp =>
    ...
    have htcp : w₄.tcpSent = w.tcpSent
      ...
      have h4 := run_tcpSend_inv hrunSend
      have h3 := (VeriDNS.Proof.SentMinimised.replyForResolution_sends_frame
        query (.ok resp) cache' w.clock hrunReply).2
      have h2 := (VeriDNS.Proof.SentMinimised.resolveWithIO_sends_frame
        query sbelt cache w.clock hrunRes).2
      rw [h4]
      simp only [hrespD, h3, h2]
    have hrunPure' : Prog.run m₆ (pure (cache''.boundLru
          (Server.serveTouches query sbelt cache w.clock) w.clock) : Prog DnsCache) w₄
        = some (cacheOut, w') := hrunPure
    rw [run_pure'] at hrunPure'
    ...
    refine Or.inr ⟨resp, slist, v, cOut, coutM, rfl, hq0, hcOutR, hHV,
      ...
      hCR, hWM, hwf1, hwf2, hwf3, hwf4, hwf5, hwf6, hwf7, hwf8.1, ?_, ?_, htcp⟩
