...
theorem mem_store_preserve (c : DnsCache) (rr : ResourceRecord) (now : UInt32) (cred : Trustworthiness)
    ...
    (hkeep : (nameEqCI e.rr.name rr.name && e.rr.type == rr.type && e.rr.class == rr.class
        && (e.expiry != now + rr.ttl.toNat.toUInt32 || rdataEqCI rr.type e.rr.rdata rr.rdata)) = false) :
...
theorem negativelyCacheable_nodata (resp : Format)
  ...
  rcases h with ⟨_, ⟨hcase, _⟩ | ⟨hno, hempty⟩⟩
...
/-- **Finding 053 pin (0x20-cased rdata must not defeat cache dedup).**
Inserting the same logical RR twice under different rdata-name casing yields
ONE member: starting from a cache with no entry under the RRset key, storing
`rr` and then a case-variant `rr'` (same key modulo 0x20 case, same TTL,
rdata equal modulo case per RFC 4343 §3) leaves the second entry as the ONLY
member under that key — the CI dedup filter removes the first push instead of
letting a client accumulate case-variant twins without bound. -/
theorem store_ci_variant_dedups (c : DnsCache) (rr rr' : ResourceRecord)
    (now : UInt32) (cred cred' : Trustworthiness)
    (hclean : ∀ e ∈ c.records,
      (nameEqCI e.rr.name rr'.name && e.rr.type == rr'.type
        && e.rr.class == rr'.class) = false)
    (hname : nameEqCI rr.name rr'.name = true)
    (htype : rr.type = rr'.type) (hclass : rr.class = rr'.class)
    (httl : rr.ttl = rr'.ttl)
    (hci : rdataEqCI rr'.type rr.rdata rr'.rdata = true) :
    ∀ e ∈ ((c.store rr now cred).store rr' now cred').records,
      (nameEqCI e.rr.name rr'.name && e.rr.type == rr'.type
        && e.rr.class == rr'.class) = true →
  ...
  · exfalso
    obtain ⟨hm1, hkeep⟩ := Array.mem_filter.mp hf
    rcases Array.mem_push.mp hm1 with hf1 | hp1
    · -- e came from the base cache: contradicts key-cleanliness
      have hc := hclean e (Array.mem_filter.mp hf1).1
      rw [hkey] at hc
      exact absurd hc (by decide)
    · -- e is the first push ⟨rr, …⟩: the CI dedup predicate is TRUE on it,
      -- so it cannot have survived the second store's filter
      rw [hp1] at hkeep
      simp only [hname, htype, hclass, httl, hci, beq_self_eq_true,
        Bool.and_true, Bool.true_and, bne_self_eq_false, Bool.false_or,
        Bool.or_true, Bool.and_self, Bool.not_true] at hkeep
