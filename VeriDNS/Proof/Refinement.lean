...
theorem mem_cacheRRs_live_of_split (c : Cache.DnsCache) (pre post : Array ByteArray) (nsRaw : ByteArray)
    ...
    (hpost : ∀ b ∈ post, ∀ rr,
      ...
      (DomainName.nameEqCI nsRR.name rr.name && nsRR.type == rr.type && nsRR.class == rr.class
        && (now + nsRR.ttl.toNat.toUInt32 != now + rr.ttl.toNat.toUInt32 || Cache.rdataEqCI rr.type nsRR.rdata rr.rdata)) = false) :
...
theorem store_fresh_records (c : Cache.DnsCache) (rr : VeriDNS.Spec.ResourceRecord) (now : UInt32)
  ...
  have hf : Array.filter (fun e => !(VeriDNS.Impl.DomainName.nameEqCI e.rr.name rr.name
        ...
        && (e.expiry != (now + rr.ttl.toNat.toUInt32) || Cache.rdataEqCI rr.type e.rr.rdata rr.rdata))) c.records = c.records := by
...
theorem store_push_records (c : Cache.DnsCache) (rr : VeriDNS.Spec.ResourceRecord) (now : UInt32)
    ...
    (h : ∀ e ∈ c.records, (VeriDNS.Impl.DomainName.nameEqCI e.rr.name rr.name
        && e.rr.type == rr.type && e.rr.class == rr.class
        && (e.expiry != (now + rr.ttl.toNat.toUInt32) || Cache.rdataEqCI rr.type e.rr.rdata rr.rdata)) = false) :
    (Cache.DnsCache.store c rr now cred).records
      = c.records.push ⟨rr, now + rr.ttl.toNat.toUInt32, false, cred, now⟩ := by
  have hf : Array.filter (fun e => !(VeriDNS.Impl.DomainName.nameEqCI e.rr.name rr.name
        && e.rr.type == rr.type && e.rr.class == rr.class
...
theorem nsName_mem_extractNsNames_of_rederived (c : Cache.DnsCache) (cut : ByteArray)
    ...
    nm ∈ Resolver.extractNsNames (RR := VeriDNS.Spec.ResourceRecord)
      (Resolver.ownerRaws (RR := VeriDNS.Spec.ResourceRecord) cut authority) := by
  ...
  · next htype =>
    ...
    · have hbauth : b ∈ authority.toList :=
      ...
      have hlivename : DomainName.nameEqCI e.rr.name cut = true := by
        unfold Cache.liveEntry at hlive
        simp only [Bool.and_eq_true] at hlive
        exact hlive.1.1.1
      have hbowner : b ∈ Resolver.ownerRaws (RR := VeriDNS.Spec.ResourceRecord) cut authority := by
        unfold Resolver.ownerRaws
        rw [Array.mem_filter]
        refine ⟨Array.mem_def.mpr hbauth, ?_⟩
        rw [hpb]
        exact hlivename
  ...
  · rw [hc]
    have hrec : (c.store rr now cred).records
        = (c.records.filter fun e => !(VeriDNS.Impl.DomainName.nameEqCI e.rr.name rr.name
            && e.rr.type == rr.type && e.rr.class == rr.class
            && (e.expiry != now + rr.ttl.toNat.toUInt32 || Cache.rdataEqCI rr.type e.rr.rdata rr.rdata))).push
...
theorem CacheNsDistinct_storeChecked (c : Cache.DnsCache) (rr : VeriDNS.Spec.ResourceRecord)
  ...
  · rw [hc]
    ...
    · intro a ha b hb
      ...
      simp at hap
      have hself : Cache.rdataEqCI rr.type rr.rdata rr.rdata = true :=
        Cache.rdataEqCI_refl _ _
      rw [hself] at hap
    ...
    Resolver.stepAnalyzeResponse s = .goto .checkAnswer { s with
      resources := { s.resources with
        ...
        cache := Resolver.cacheUnlessTruncated (RR := RR) s.resources.cache resp
          (Resolver.cnameRaws (RR := RR) (Resolver.echoedQname resp) resp.answer)
...
theorem stepAnalyzeResponse_lame {S C NS RR : Type}
    ...
    (hnodata : ((resp.header.rcode == VeriDNS.Spec.Rcode.noError)
              && resp.answer.isEmpty
              && Resolver.hasSoaAuthorityFor (RR := RR)
                  (Resolver.echoedQname resp) resp.authority) = false) :
    Resolver.stepAnalyzeResponse s = .goto .sendQueries { s with lastResponse := none } := by
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname]
...
/-- The G-branch retry: an empty-answer NOERROR response with **empty authority**
    (RFC 2308 §2.2 NODATA TYPE 3, ambiguous) carries no negative proof, so the
    resolver re-queries (RFC 1034 §4.3.2.d bizarre contents) rather than
    synthesise a NODATA.  Its authority is empty ⇒ it is not the B branch. -/
theorem stepAnalyzeResponse_retryEmpty {S C NS RR : Type}
    [VeriDNS.Spec.SlistSpec S NS] [VeriDNS.Spec.SlistFromNameSpec S NS]
    [VeriDNS.Spec.CacheSpec C RR] [VeriDNS.Spec.TrustworthinessSpec C RR]
    [VeriDNS.Spec.NegativeAuthoritySpec C RR] [VeriDNS.Spec.NegativeCacheSpec C]
    [VeriDNS.Spec.RRParse RR] [Inhabited S] [Inhabited C]
    ...
    (hne : (resp.header.rcode == VeriDNS.Spec.Rcode.nameError) = false)
    (hemp : resp.answer.isEmpty = true)
    (hauth : resp.authority.isEmpty = true)
    (hnoerr : (resp.header.rcode == VeriDNS.Spec.Rcode.noError) = true)
    (htc : (resp.header.tc == 1) = false) :
    Resolver.stepAnalyzeResponse s = .goto .sendQueries { s with lastResponse := none } := by
  have hsoa : Resolver.hasSoaAuthorityFor (RR := RR)
      (Resolver.echoedQname resp) resp.authority = false := by
    have hnil : resp.authority = #[] := Array.isEmpty_iff.mp hauth
    simp [Resolver.hasSoaAuthorityFor, hnil]
  have hent : Resolver.entitledAnswerB (RR := RR) resp = false := by
    cases hc : Resolver.entitledAnswerB (RR := RR) resp
    · rfl
    · rw [Resolver.answersQueryB_of_entitled (RR := RR) resp hc] at hans; simp at hans
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname]
  rw [if_neg (by simp [hbiz]),
    if_neg (by rw [hans, hne, hemp, hauth]; simp),
    if_neg (by rw [hent]; simp),
    if_neg (by rw [hne]; simp),
    if_neg (by rw [hnoerr, hemp, hsoa]; simp),
    if_neg (by rw [htc]; simp)]
...
/-- **Off-owner retry (findings 036 / off-owner-A).**  A response with a
    NON-EMPTY answer the resolver does NOT recognise as entitled
    (`entitledAnswerB = false`) — a *foreign* answer whose records scrub to
    empty — that is not an NXDOMAIN and not truncated drives a retry rather than
    accepting-then-scrubbing-to-NODATA.  (The empty-answer B-block cannot fire:
    the answer is non-empty.) -/
theorem stepAnalyzeResponse_retryForeign {S C NS RR : Type}
    [VeriDNS.Spec.SlistSpec S NS] [VeriDNS.Spec.SlistFromNameSpec S NS]
    [VeriDNS.Spec.CacheSpec C RR] [VeriDNS.Spec.TrustworthinessSpec C RR]
    [VeriDNS.Spec.NegativeAuthoritySpec C RR] [VeriDNS.Spec.NegativeCacheSpec C]
    [VeriDNS.Spec.RRParse RR] [Inhabited S] [Inhabited C]
    ...
    (hbiz : (resp.header.rcode == VeriDNS.Spec.Rcode.serverFailure
              || !Resolver.classifiableB resp) = false)
    (hent : Resolver.entitledAnswerB (RR := RR) resp = false)
    (hnem : resp.answer.isEmpty = false)
    (hne : (resp.header.rcode == VeriDNS.Spec.Rcode.nameError) = false)
    (htc : (resp.header.tc == 1) = false) :
    Resolver.stepAnalyzeResponse s = .goto .sendQueries { s with lastResponse := none } := by
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname]
  rw [if_neg (by simp [hbiz]),
    if_neg (by rw [hnem]; simp),
    if_neg (by rw [hent]; simp),
    if_neg (by rw [hnem]; simp),
    if_neg (by rw [hnem]; simp),
    if_neg (by rw [htc]; simp)]

/-- **The `¬ NXDOMAIN`-free retry variant.**  A type-matching (`answersQueryB`,
    hence non-empty), non-entitled, non-bizarre, non-truncated response RETRIES —
    *regardless of rcode*.  Because the NXDOMAIN answer arm is now guarded on
    `answer.isEmpty` (a `nameError` carrying an answer is contradictory and no
    longer delivers), a non-empty response skips both the entitled arm (not
    entitled) and the NXDOMAIN arm (non-empty), so it falls through to the retry.
    This drops the `hne` premise the spoof arm cannot supply. -/
theorem stepAnalyzeResponse_retryForeign_ans {S C NS RR : Type}
    [VeriDNS.Spec.SlistSpec S NS] [VeriDNS.Spec.SlistFromNameSpec S NS]
    [VeriDNS.Spec.CacheSpec C RR] [VeriDNS.Spec.TrustworthinessSpec C RR]
    [VeriDNS.Spec.NegativeAuthoritySpec C RR] [VeriDNS.Spec.NegativeCacheSpec C]
    [VeriDNS.Spec.RRParse RR] [Inhabited S] [Inhabited C]
    ...
    (htc : (resp.header.tc == 1) = false) :
    Resolver.stepAnalyzeResponse s = .goto .sendQueries { s with lastResponse := none } := by
  have hnem : resp.answer.isEmpty = false := answersQueryB_nonempty resp hans
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname]
  rw [if_neg (by simp [hbiz]),
    if_neg (by rw [hnem]; simp),
    if_neg (by rw [hent]; simp),
...
/-- If the referral 4-group (NS-authority ∧ aa=0 ∧ NOERROR ∧ no-SOA) is false,
    the response is not `referralShapedB` — the group is a sub-conjunction. -/
theorem referralShapedB_false_of_group {resp : VeriDNS.Spec.Format}
    (h : (Resolver.hasRRTypeIn (RR := VeriDNS.Spec.ResourceRecord) resp.authority 2
            && (resp.header.aa == 0)
            && (resp.header.rcode == VeriDNS.Spec.Rcode.noError)
            && !Resolver.hasRRTypeIn (RR := VeriDNS.Spec.ResourceRecord) resp.authority 6) = false) :
    Server.referralShapedB resp = false := by
  unfold Server.referralShapedB
  cases hNS : Resolver.hasRRTypeIn (RR := VeriDNS.Spec.ResourceRecord) resp.authority 2 <;>
    cases haa : (resp.header.aa == 0) <;>
    cases hnoerr : (resp.header.rcode == VeriDNS.Spec.Rcode.noError) <;>
    cases hSOA : Resolver.hasRRTypeIn (RR := VeriDNS.Spec.ResourceRecord) resp.authority 6 <;>
    simp_all

/-- An empty authority section rules out `referralShapedB` (it needs a non-empty
    authority carrying the NS delegation). -/
theorem referralShapedB_false_of_authEmpty {resp : VeriDNS.Spec.Format}
    (h : resp.authority.isEmpty = true) : Server.referralShapedB resp = false := by
  unfold Server.referralShapedB
  rw [h]; simp
...
theorem stepAnalyzeResponse_answer {S C NS RR : Type}
    ...
    (hans : Resolver.entitledAnswerB (RR := RR) resp = true) :
  ...
  have haq : Resolver.answersQueryB (RR := RR) resp = true :=
    Resolver.answersQueryB_of_entitled (RR := RR) resp hans
  have hne : resp.answer.isEmpty = false := answersQueryB_nonempty resp haq
  -- the empty-answer B-block fails (non-empty answer), so we reach the
  -- entitled-answer arm directly.
  have h4b : (!Resolver.answersQueryB (RR := RR) resp
      && !(resp.header.rcode == VeriDNS.Spec.Rcode.nameError)
      && resp.answer.isEmpty
      && !resp.authority.isEmpty) = false := by rw [haq]; rfl
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname]
  rw [if_neg (by simp [hsf, hcls]), if_neg (by rw [h4b]; simp), if_pos hans]
...
theorem stepAnalyzeResponse_nameError {S C NS RR : Type}
  ...
  have hent : Resolver.entitledAnswerB (RR := RR) resp = false := by
    ...
    · rfl
    · rw [Resolver.answersQueryB_of_entitled (RR := RR) resp hc] at hans; simp at hans
  simp [Resolver.stepAnalyzeResponse, hresp, hcname, hsf, hcls, hnerr, hans, hent, hemp]

/-- The truncated-answer arm: a type-matching (`answersQueryB`) but non-entitled,
    non-NXDOMAIN, non-bizarre, `tc = 1` response delivers `finalizeAnswer s resp`
    with the state unchanged (RFC 1035 §4.1.1: a truncated positive answer is
    still handed to the client, but not cached). -/
theorem stepAnalyzeResponse_answer_tc {S C NS RR : Type}
    [VeriDNS.Spec.SlistSpec S NS] [VeriDNS.Spec.SlistFromNameSpec S NS]
    [VeriDNS.Spec.CacheSpec C RR] [VeriDNS.Spec.TrustworthinessSpec C RR]
    [VeriDNS.Spec.NegativeAuthoritySpec C RR] [VeriDNS.Spec.NegativeCacheSpec C]
    [VeriDNS.Spec.RRParse RR] [Inhabited S] [Inhabited C]
  ...
  have h4b : (!Resolver.answersQueryB (RR := RR) resp
      && !(resp.header.rcode == VeriDNS.Spec.Rcode.nameError)
      && resp.answer.isEmpty
      && !resp.authority.isEmpty) = false := by rw [hans]; rfl
  have hnonempty : resp.answer.isEmpty = false := answersQueryB_nonempty resp hans
  have htc' : resp.header.tc = 1 := by simpa using htc
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname, hsf, hcls, hent, hnerr, hnonempty,
    htc', h4b, Bool.false_or, Bool.not_false, Bool.not_true, Bool.and_false, Bool.false_and,
    Bool.and_true, if_false, if_true, beq_self_eq_true, reduceIte, BEq.rfl,
    Bool.false_eq_true, ↓reduceIte]

/-- The truncated arm with a (non-entitled) type-matching answer present: an
    `answersQueryB`, non-entitled, `tc = 1` response delivers `finalizeAnswer`
    with the state unchanged.  Because the NXDOMAIN answer arm is now guarded on
    `answer.isEmpty` (a `nameError` carrying ANY answer is contradictory and no
    longer delivers — see `stepAnalyzeResponse`), the only way such a non-empty
    response reaches an `.answer` terminal is the truncation arm (`tc = 1`), which
    still hands the truncated reply to the client (junk scrubbed there). -/
theorem stepAnalyzeResponse_nameError_ans {S C NS RR : Type}
    [VeriDNS.Spec.SlistSpec S NS] [VeriDNS.Spec.SlistFromNameSpec S NS]
    [VeriDNS.Spec.CacheSpec C RR] [VeriDNS.Spec.TrustworthinessSpec C RR]
    [VeriDNS.Spec.NegativeAuthoritySpec C RR] [VeriDNS.Spec.NegativeCacheSpec C]
    [VeriDNS.Spec.RRParse RR] [Inhabited S] [Inhabited C]
  ...
  have hnem : resp.answer.isEmpty = false := answersQueryB_nonempty resp hans
  simp only [Resolver.stepAnalyzeResponse, hresp, hcname]
  rw [if_neg (by simp [hsf, hcls]), if_neg (by rw [h4b]; simp), if_neg (by simp [hent]),
    if_neg (by rw [hnem]; simp), if_neg (by rw [hnem]; simp), if_pos (by simpa using htc)]
      ...
      { s with
        resources := { s.resources with
          slist := VeriDNS.Spec.SlistFromNameSpec.setUpAddresses (NS := NS)
            (Resolver.extractNsNames (RR := RR) (Resolver.ownerRaws (RR := RR) (Resolver.referralCutRaw (RR := RR) resp.authority) resp.authority))
...
theorem stepAnalyzeResponse_answer_payload_neg {S C NS RR : Type}
  ...
  rw [hresp] at hsa
  simp only [hcname, hans, hent, Bool.false_eq_true, if_false] at hsa
  repeat' split at hsa
  all_goals simp_all
  all_goals (obtain ⟨h1, h2⟩ := hsa; subst h2; exact h1.symm)
    ...
    · rename_i hcond
      ...
      · rename_i hhns
        ...
        · obtain ⟨hns, hs'⟩ := Resolver.StepResult.goto.inj hsa
          refine Or.inr (Or.inl ⟨hns.symm, hs'.symm, hcond,
            Bool.eq_false_iff.mpr hhns, Bool.eq_false_iff.mpr hnodata⟩)
    · rename_i hcond
      -- ¬B tail: entitled / nameError / SOA / tc are answers; the final else
      -- is the retry goto.  It fires for an empty-no-SOA response (041/045) OR
      -- a NON-EMPTY FOREIGN answer (036 / off-owner-A) — in both cases the
      -- response is not `referralShaped` (empty-with-empty-authority, or
      -- non-empty answer), which is what the consumer needs.
      split at hsa
      · rename_i hent; exact absurd hsa (by simp)  -- entitled answer
      · rename_i hent
        split at hsa
        · exact absurd hsa (by simp)        -- nameError ⇒ answer
        · rename_i hd
          split at hsa
          · exact absurd hsa (by simp)      -- SOA-nodata ⇒ answer
          · split at hsa
            · exact absurd hsa (by simp)    -- tc==1 ⇒ answer
            · -- else: foreign / bizarre ⇒ retry (goto sendQueries)
              obtain ⟨hns, hs'⟩ := Resolver.StepResult.goto.inj hsa
              refine Or.inr (Or.inr ⟨hns.symm, hs'.symm, ?_⟩)
              -- referralShapedB = false: either the answer is non-empty, or (if
              -- empty) the authority is empty (else the B-block would have fired).
              -- The NXDOMAIN answer arm is now guarded on `answer.isEmpty`, so `hd`
              -- rules out `nameError ∧ answer.isEmpty`; in the empty branch it
              -- gives `¬ nameError`.
              unfold Server.referralShapedB
              by_cases hemp : resp.answer.isEmpty = true
              · have haq : Resolver.answersQueryB (RR := RR) resp = false := by
                  unfold Resolver.answersQueryB
                  have hnil : resp.answer = #[] := Array.isEmpty_iff.mp hemp
                  split
                  · rename_i qu _; unfold Resolver.hasRRTypeIn; rw [hnil]; simp
                  · rfl
                have hdF : (resp.header.rcode == VeriDNS.Spec.Rcode.nameError) = false := by
                  by_contra hc
                  rw [Bool.not_eq_false] at hc
                  rw [hc, hemp] at hd; simp at hd
                have hauthE : resp.authority.isEmpty = true := by
                  by_contra hne
                  simp only [Bool.not_eq_true] at hne
                  apply absurd hcond
                  simp only [Bool.not_eq_true]
                  rw [haq, hdF, hemp, hne]; simp
                rw [hauthE]; simp
...
/-- Core: whenever `stepAnalyzeResponse` (on the response-primed analyze state)
    retries into `sendQueries`, the shim's `afterResume` yields `.continue` with
    the round-touched, sendQueries-primed state.  Both the lame-referral (B.3)
    and the ambiguous-empty (G) retry funnel through here. -/
theorem afterResume_of_stepGoto
    (state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord)
    (entryName : ByteArray) (resp : VeriDNS.Spec.Format)
    (hstep : state.currentStep = .sendQueries)
    (hbiz : (resp.header.rcode == VeriDNS.Spec.Rcode.serverFailure
              || !Resolver.classifiableB resp) = false)
    (hsa : Resolver.stepAnalyzeResponse
        { state with lastResponse := some resp, currentStep := .analyzeResponse }
        = .goto .sendQueries
            { { state with lastResponse := some resp, currentStep := .analyzeResponse } with
              lastResponse := none }) :
...
/-- **Off-owner bridge (findings 036 / off-owner-A).**  If `afterResume`
    FINISHED with a positive answer on a type-matching (`answersQueryB`),
    non-truncated, non-NXDOMAIN, non-bizarre response, that answer was ENTITLED.
    A foreign (`entitledAnswerB = false`) answer would have retried
    (`stepAnalyzeResponse ⇒ goto sendQueries`, hence `afterResume ⇒ .continue`),
    contradicting `.finished`.  This lets the positive-answer delivery lemmas be
    stated with the stronger `entitledAnswerB` guard while callers supply only
    `answersQueryB` + the terminal facts. -/
theorem entitled_of_afterResume_finished_ok
    {state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord}
    {entryName : ByteArray} {resp out : VeriDNS.Spec.Format} {cout : Cache.DnsCache}
    (hstep : state.currentStep = .sendQueries)
    (hcname : Resolver.cnameToChase (RR := VeriDNS.Spec.ResourceRecord) resp = none)
    (hsf : (resp.header.rcode == VeriDNS.Spec.Rcode.serverFailure) = false)
    (hcls : Resolver.classifiableB resp = true)
    ...
    (hAR : Server.afterResume state entryName resp = .finished (.ok out) cout) :
    Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) resp = true := by
  by_contra hcon
  have hent : Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) resp = false := by
    simpa using hcon
  have hbiz : (resp.header.rcode == VeriDNS.Spec.Rcode.serverFailure
      || !Resolver.classifiableB resp) = false := by rw [hsf, hcls]; rfl
  have hsaGoto : Resolver.stepAnalyzeResponse
      ({ state with lastResponse := some resp, currentStep := .analyzeResponse } :
      ...
      = .goto .sendQueries
          { { state with lastResponse := some resp, currentStep := .analyzeResponse } with
            lastResponse := none } :=
    stepAnalyzeResponse_retryForeign_ans _ resp rfl hcname hbiz hent hans htc
  rw [afterResume_of_stepGoto state entryName resp hstep hbiz hsaGoto] at hAR
  exact absurd hAR (by simp)

/-- The `.finished`-inversion form of `afterResume_answer` under the acceptance
    guard: a type-matching (`answersQueryB`), non-truncated, non-NXDOMAIN,
    non-bizarre response that FINISHED with a positive answer was entitled, so
    its delivered output and cache write are the positive-answer shape. -/
theorem afterResume_answer_payload_pos_ent
    (state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord)
    ...
    (hsf : (respA.header.rcode == VeriDNS.Spec.Rcode.serverFailure) = false)
    (hcls : Resolver.classifiableB respA = true)
    (hent : Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) respA = true)
    {cout : Cache.DnsCache}
  ...
  rw [afterResume_answer state entryName respA hstep hcname hsf hcls hent] at hAR
  injection hAR with h hcout
  injection h with h
  exact ⟨h.symm, hcout.symm⟩

/-- Under `tc = 0` and `¬ NXDOMAIN`, a type-matching (`answersQueryB`)
    non-bizarre response that FINISHED with a positive answer is ENTITLED: a
    foreign (non-entitled) such answer retries (`⇒ .continue`), contradicting
    `.finished`.  The caller supplies `¬ NXDOMAIN` from the honest reply (an
    NXDOMAIN never carries a type-matching answer). -/
theorem entitled_of_afterResume_finished_okTc
    {state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord}
    {entryName : ByteArray} {resp out : VeriDNS.Spec.Format} {cout : Cache.DnsCache}
    (hstep : state.currentStep = .sendQueries)
    ...
    (hans : Resolver.answersQueryB (RR := VeriDNS.Spec.ResourceRecord) resp = true)
    (hAR : Server.afterResume state entryName resp = .finished (.ok out) cout) :
    Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) resp = true :=
  entitled_of_afterResume_finished_ok hstep hcname hsf hcls htc hans hAR
...
theorem afterResume_lame
    ...
    Server.afterResume state entryName resp
      = .continue (Server.boundStateCache
          (Server.roundTouches state resp)
          { state with lastResponse := none, currentStep := .sendQueries }) := by
  exact afterResume_of_stepGoto state entryName resp hstep hbiz
    (stepAnalyzeResponse_lame _ resp rfl hcname hbiz h4b href hnodata)

/-- The G-branch analogue of `afterResume_lame`: an ambiguous empty NOERROR
    (empty answer + empty authority) drives a retry, resuming into sendQueries. -/
theorem afterResume_retryEmpty
    (state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord)
    (entryName : ByteArray) (resp : VeriDNS.Spec.Format)
    (hstep : state.currentStep = .sendQueries)
    ...
    Server.afterResume state entryName resp
      = .continue (Server.boundStateCache
          ...
          { state with lastResponse := none, currentStep := .sendQueries }) := by
  exact afterResume_of_stepGoto state entryName resp hstep hbiz
    (stepAnalyzeResponse_retryEmpty _ resp rfl hcname hbiz hans hne hemp hauth hnoerr htc)
...
theorem resume_cname_answerHit
    ...
    (hhit : Resolver.localAnswer (C := Cache.DnsCache) (RR := VeriDNS.Spec.ResourceRecord)
        (Resolver.cacheUnlessTruncated (RR := VeriDNS.Spec.ResourceRecord) state.resources.cache resp
          (Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) (Resolver.echoedQname resp) resp.answer)
...
theorem afterResume_finished_payload_pos
    ...
    cout = ((Resolver.cacheUnlessTruncated (RR := VeriDNS.Spec.ResourceRecord)
      ...
      (Resolver.credAnswer (respA.header.aa == 1)) state.now).boundLru
      (Server.roundTouches state respA) state.now) := by
  -- the entitled arm delivers the positive answer + its cache write.
  rw [afterResume_answer state entryName respA hstep hcname hsf hcls hent] at hAR
  injection hAR with h hcout
  injection h with h
  exact ⟨h.symm, hcout.symm⟩

/-- The truncated-answer arm reduced through `afterResume`: a `tc = 1`,
    type-matching, non-entitled, non-NXDOMAIN answer delivers `finalizeAnswer`
    with the cache UNCHANGED (only round-boundary LRU eviction applies). -/
theorem afterResume_answer_tc
    (state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord)
    (entryName : ByteArray) (resp : VeriDNS.Spec.Format)
    (hstep : state.currentStep = .sendQueries)
    ...
    (hcls : Resolver.classifiableB resp = true)
    (hans : Resolver.answersQueryB (RR := VeriDNS.Spec.ResourceRecord) resp = true)
    (hent : Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) resp = false)
    (hnerr : (resp.header.rcode == VeriDNS.Spec.Rcode.nameError) = false)
    (htc : (resp.header.tc == 1) = true) :
    Server.afterResume state entryName resp
      = .finished (.ok (Resolver.finalizeAnswer
  ...
  have hsa : Resolver.stepAnalyzeResponse
      ...
      = .answer (Resolver.finalizeAnswer
          { state with lastResponse := some resp, currentStep := .analyzeResponse } resp)
          { state with lastResponse := some resp, currentStep := .analyzeResponse } := by
    apply stepAnalyzeResponse_answer_tc <;> first | rfl | assumption
  unfold Server.afterResume
  rw [hdrop, Resolver.resume]
  rw [show (64 : Nat) = 63 + 1 from rfl, Resolver.resolve.loop]
  simp only [Resolver.step, hstep, Resolver.stepSendQueries]
...
/-- **Positive-answer cache write, from `answersQueryB` + `¬ NXDOMAIN`.**  A
    type-matching, non-NXDOMAIN, non-bizarre response that FINISHED positively
    writes the entitled-answer cache shape `(cacheUnlessTruncated …).boundLru …`
    — whether or not it was entitled.  When entitled, the entitled arm fires
    directly; when not entitled (`answersQueryB` but off-owner), the only
    remaining finishing arm (given `¬ NXDOMAIN`) is truncation (`tc = 1`), under
    which `cacheUnlessTruncated` collapses to the (unchanged) cache — the same
    write.  This lets the answer terminal establish the cache shape without a
    `tc` case split, and covers the spoofed off-owner NXDOMAIN-with-data case
    (which never reaches here at `tc = 0` because `¬ NXDOMAIN` is supplied). -/
theorem afterResume_finished_pos_cout
    (state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord)
    (entryName : ByteArray) (respA out : VeriDNS.Spec.Format)
    (hstep : state.currentStep = .sendQueries)
    ...
    (hnerr : (respA.header.rcode == VeriDNS.Spec.Rcode.nameError) = false)
    {cout : Cache.DnsCache}
    (hAR : Server.afterResume state entryName respA = .finished (.ok out) cout) :
    cout = ((Resolver.cacheUnlessTruncated (RR := VeriDNS.Spec.ResourceRecord)
      state.resources.cache respA
      ...
      (Resolver.credAnswer (respA.header.aa == 1)) state.now).boundLru
      (Server.roundTouches state respA) state.now) := by
  by_cases hent : Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) respA = true
  · exact (afterResume_answer_payload_pos_ent state entryName respA out hstep hcname hsf hcls
      hent hAR).2
  · rw [Bool.not_eq_true] at hent
    -- not entitled + answersQueryB + finished + ¬ NXDOMAIN  ⟹  it truncated (tc = 1).
    by_cases htc : (respA.header.tc == 1) = true
    · rw [afterResume_answer_tc state entryName respA hstep hcname hsf hcls hans hent hnerr htc]
        at hAR
      injection hAR with h hcout
      -- under tc = 1, cacheUnlessTruncated collapses to the cache; roundTouches match.
      have hcollapse : (Resolver.cacheUnlessTruncated (RR := VeriDNS.Spec.ResourceRecord)
          state.resources.cache respA
          (Resolver.ownerRaws (RR := VeriDNS.Spec.ResourceRecord)
            (Resolver.echoedQname respA) respA.answer)
          (Resolver.credAnswer (respA.header.aa == 1)) state.now)
          = state.resources.cache := by
        simp only [Resolver.cacheUnlessTruncated, htc, if_true]
      rw [← hcout, hcollapse]
      rfl
    · -- ¬ entitled, ¬ NXDOMAIN, ¬ tc, answersQueryB, finished: the impl retried,
      -- contradicting `.finished`.
      rw [Bool.not_eq_true] at htc
      have hbiz : (respA.header.rcode == VeriDNS.Spec.Rcode.serverFailure
          || !Resolver.classifiableB respA) = false := by rw [hsf, hcls]; rfl
      have hnem : respA.answer.isEmpty = false := answersQueryB_nonempty respA hans
      have hsaGoto : Resolver.stepAnalyzeResponse
          ({ state with lastResponse := some respA, currentStep := .analyzeResponse } :
            Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
              VeriDNS.Spec.ResourceRecord)
          = .goto .sendQueries
              { { state with lastResponse := some respA, currentStep := .analyzeResponse } with
                lastResponse := none } :=
        stepAnalyzeResponse_retryForeign _ respA rfl hcname hbiz hent hnem hnerr htc
      rw [afterResume_of_stepGoto state entryName respA hstep hbiz hsaGoto] at hAR
      exact absurd hAR (by simp)

/-- The `tc = 1` specialisation of `afterResume_finished_pos_cout`: a truncated,
    type-matching, finished response writes the (collapsed) cache regardless of
    entitlement — `cacheUnlessTruncated` is the identity under `tc = 1`. -/
theorem afterResume_finished_pos_cout_tc
    (state : Resolver.State SList.DnsSList Cache.DnsCache VeriDNS.Spec.SlistEntry
      VeriDNS.Spec.ResourceRecord)
    (entryName : ByteArray) (respA out : VeriDNS.Spec.Format)
    (hstep : state.currentStep = .sendQueries)
  ...
  have hcollapse : (Resolver.cacheUnlessTruncated (RR := VeriDNS.Spec.ResourceRecord)
      ...
      (Resolver.credAnswer (respA.header.aa == 1)) state.now)
      = state.resources.cache := by
    have htc' : (respA.header.tc == 1) = true := htc
    simp only [Resolver.cacheUnlessTruncated, htc', if_true]
  rw [hcollapse]
  by_cases hent : Resolver.entitledAnswerB (RR := VeriDNS.Spec.ResourceRecord) respA = true
  · have h := (afterResume_answer_payload_pos_ent state entryName respA out hstep hcname hsf hcls
      hent hAR).2
    rw [h, hcollapse]
  · rw [Bool.not_eq_true] at hent
    by_cases hnerr : (respA.header.rcode == VeriDNS.Spec.Rcode.nameError) = true
    · have hbiz : (respA.header.rcode == VeriDNS.Spec.Rcode.serverFailure
          || !Resolver.classifiableB respA) = false := by rw [hsf, hcls]; rfl
      have hdrop : Server.dropIfBizarre state entryName respA = state := by
        unfold Server.dropIfBizarre; rw [if_neg (by simp [hbiz])]
      have hsa : Resolver.stepAnalyzeResponse
          ...
          = .answer (Resolver.finalizeAnswer
              { state with lastResponse := some respA, currentStep := .analyzeResponse } respA)
              { state with lastResponse := some respA, currentStep := .analyzeResponse } := by
        apply stepAnalyzeResponse_nameError_ans <;> first | rfl | assumption
      rw [show Server.afterResume state entryName respA
          = .finished (.ok (Resolver.finalizeAnswer
              { state with lastResponse := some respA, currentStep := .analyzeResponse } respA))
              (Server.boundStateCache (Server.roundTouches state respA)
                { state with lastResponse := some respA, currentStep := .analyzeResponse }).resources.cache from by
        unfold Server.afterResume
        rw [hdrop, Resolver.resume]
        rw [show (64 : Nat) = 63 + 1 from rfl, Resolver.resolve.loop]
        simp only [Resolver.step, hstep, Resolver.stepSendQueries]
        rw [show (63 : Nat) = 62 + 1 from rfl, Resolver.resolve.loop]
        simp only [Resolver.step, hsa]] at hAR
      injection hAR with h hcout
      rw [← hcout]; rfl
    · rw [Bool.not_eq_true] at hnerr
      rw [afterResume_answer_tc state entryName respA hstep hcname hsf hcls hans hent hnerr htc]
        at hAR
      injection hAR with h hcout
      rw [← hcout]; rfl
theorem stepAnalyzeResponse_answer_output_finalize {S C NS RR : Type}
    [VeriDNS.Spec.SlistSpec S NS] [VeriDNS.Spec.SlistFromNameSpec S NS]
    [VeriDNS.Spec.CacheSpec C RR] [VeriDNS.Spec.TrustworthinessSpec C RR]
    [VeriDNS.Spec.NegativeAuthoritySpec C RR] [VeriDNS.Spec.NegativeCacheSpec C]
    [VeriDNS.Spec.RRParse RR] [Inhabited S] [Inhabited C]
    ...
    (hresp : s.lastResponse = some resp)
    (hcname : Resolver.cnameToChase (RR := RR) resp = none) :
    ∀ r st, Resolver.stepAnalyzeResponse s = .answer r st →
      r = Resolver.finalizeAnswer s resp := by
  intro r st hsa
  unfold Resolver.stepAnalyzeResponse at hsa
  rw [hresp] at hsa
  simp only [hcname] at hsa
  repeat' split at hsa
  all_goals (first | (injection hsa with h1 h2; exact h1.symm) | (exfalso; exact absurd hsa (by simp)))
...
theorem afterResume_finished_payload_out
  ...
  -- every `.answer` arm of `stepAnalyzeResponse` delivers `finalizeAnswer respA`
  -- (only the STATE differs across arms), so the delivered output is uniform.
  have hbiz : (respA.header.rcode == VeriDNS.Spec.Rcode.serverFailure
      || !Resolver.classifiableB respA) = false := by rw [hsf, hcls]; rfl
  have hdrop : Server.dropIfBizarre state entryName respA = state := by
    unfold Server.dropIfBizarre; rw [if_neg (by simp [hbiz])]
  ...
  · rename_i X Xst heq
    ...
    split at heq
    · rename_i r' st' heq2
      have hfin := stepAnalyzeResponse_answer_output_finalize
          ({ state with lastResponse := some respA, currentStep := .analyzeResponse })
          respA hresp1 hcname r' st' heq2
      injection heq with he
      injection he with he hst
      rw [← h, ← he]; exact hfin
    · rename_i ns s' heq2
...
theorem respAgree_answer_bridge
  ...
  -- any finished-ok answer is `finalizeAnswer respA` (entitled / NXDOMAIN / tc
  -- arms all deliver it); its rcode/answer abstractions match respA's.
  have hout : resp = Resolver.finalizeAnswer
      { state with lastResponse := some respA, currentStep := .analyzeResponse } respA :=
    afterResume_finished_payload_out state entryName respA resp hstep hcname hsf hcls hAR
  have hrc : (αResp resp).rcode = αRCode respA.header.rcode := by
    rw [hout]; exact finalizeAnswer_abstracts_rcode _ respA
  have han : (αResp resp).answer
      = αSection state.cnameChain ++ αSection respA.answer := by
...
theorem extractNsNames_ownerRaws_cut_ne_of_hasRRTypeIn {RR : Type} [VeriDNS.Spec.RRParse RR]
    (authority : Array ByteArray)
    (h : Resolver.hasRRTypeIn (RR := RR) authority 2 = true) :
    Resolver.extractNsNames (RR := RR)
      (Resolver.ownerRaws (RR := RR)
        (Resolver.referralCutRaw (RR := RR) authority) authority) ≠ #[] := by
  unfold Resolver.referralCutRaw
  split
  · rename_i owner heq
    obtain ⟨b, hb, hfb⟩ := Array.exists_of_findSome?_eq_some heq
    cases hpr : VeriDNS.Spec.RRParse.parseRaw (RR := RR) b with
    | none =>
      rw [hpr] at hfb
      dsimp only [] at hfb
    ...
    | some rr =>
      rw [hpr] at hfb
      dsimp only [] at hfb
      by_cases hty : (VeriDNS.Spec.RRParse.rrType (RR := RR) rr == (2 : BitVec 16)) = true
      · rw [if_pos hty] at hfb
        have howner : VeriDNS.Spec.RRParse.rrName (RR := RR) rr = owner := Option.some.inj hfb
        have hbmem : b ∈ Resolver.ownerRaws (RR := RR) owner authority := by
          unfold Resolver.ownerRaws
          rw [Array.mem_filter]
          refine ⟨hb, ?_⟩
          rw [hpr]
          dsimp only []
          rw [howner]
          exact VeriDNS.Proof.NameTree.nameEqCI_refl _
        intro hempty
        have hmemNs : VeriDNS.Spec.RRParse.rrRdata (RR := RR) rr
            ∈ Resolver.extractNsNames (RR := RR)
              (Resolver.ownerRaws (RR := RR) owner authority) := by
          unfold Resolver.extractNsNames
          rw [Array.mem_filterMap]
          refine ⟨b, hbmem, ?_⟩
          simp only [hpr]
...
theorem extractNsNames_ownerRaws_nodup {RR : Type} [VeriDNS.Spec.RRParse RR]
    (cut : ByteArray) (authority : Array ByteArray)
    (hnd : (Resolver.extractNsNames (RR := RR) authority).toList.Nodup) :
    (Resolver.extractNsNames (RR := RR)
      (Resolver.ownerRaws (RR := RR) cut authority)).toList.Nodup := by
  have hsub : (Resolver.extractNsNames (RR := RR)
        (Resolver.ownerRaws (RR := RR) cut authority)).toList.Sublist
      (Resolver.extractNsNames (RR := RR) authority).toList := by
    unfold Resolver.extractNsNames Resolver.ownerRaws
    rw [Array.toList_filterMap, Array.toList_filterMap, Array.toList_filter]
    exact List.Sublist.filterMap _ List.filter_sublist
...
theorem currentCloser_false_referral (resp : VeriDNS.Spec.Format) (walkMc : Nat) (sname : ByteArray)
    ...
    (!VeriDNS.Spec.SlistFromNameSpec.searchFails (S := SList.DnsSList) (NS := VeriDNS.Spec.SlistEntry)
        (VeriDNS.Spec.SlistFromNameSpec.setUpAddresses (S := SList.DnsSList) (NS := VeriDNS.Spec.SlistEntry)
          (Resolver.extractNsNames (RR := VeriDNS.Spec.ResourceRecord) (Resolver.ownerRaws (RR := VeriDNS.Spec.ResourceRecord) (Resolver.referralCutRaw (RR := VeriDNS.Spec.ResourceRecord) resp.authority) resp.authority)) glue
  ...
  · have hne := extractNsNames_ownerRaws_cut_ne_of_hasRRTypeIn (RR := VeriDNS.Spec.ResourceRecord) resp.authority hns
    simpa using hne
  · exact hge
...
theorem modelSlistOf_referral_eq_glueAddresses
    ...
    (hhosts : names.toList.filterMap αName = VeriDNS.Spec.Net.cutServers ref)
