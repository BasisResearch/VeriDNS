...
theorem deliveredResponse_answer_exact {query resp : VeriDNS.Spec.Format}
    {qu : VeriDNS.Spec.Question} {mq : VeriDNS.Spec.Net.Name} {now : UInt32}
    (hqu : query.question[0]? = some qu)
  ...
  rw [(αResp_components _).2.1, (αResp_components _).2.1, hda, hclient]
  exact VeriDNS.Proof.Refinement.αSection_scrubAnswerB_eq
                ...
                | some resp₂ =>
                    ...
                    · dsimp only [] at hrun
                      ...
                      · rw [if_neg hunf] at hrun
                        ...
                        · rw [if_pos hstT] at hrun
                          ...
                          obtain ⟨⟨hres', -⟩, -⟩ := hrun
                          obtain rfl : _ = resp := Except.ok.inj hres'
                          exact finalizeAnswer_question (by exact hlq) tcpRespA
                  ...
                  · rw [if_neg hunf] at hrun
                    ...
                    ·
                      ...
                      obtain ⟨⟨hres', -⟩, -⟩ := hrun
                      obtain rfl : _ = resp := Except.ok.inj hres'
                      exact finalizeAnswer_question (by exact hlq) resp₂
                    rw [if_neg hstT] at hrun
                    by_cases hpg : (Resolver.probeRoundB state.resources.sname revealed
                            ...
                            · dsimp only [] at hrun
                              ...
                              · rw [if_neg hunf] at hrun
                                ...
                                · rw [if_pos hstT] at hrun
                                  ...
                                  obtain ⟨hrecS, hnegS, hchainS, hlrS⟩ := hss
                                  refine ⟨?_, ?_, ?_⟩
                                  · exact respSections_finalizeAnswer (by exact hlq) hsz
                                      (by exact hchainS) hwfA
                                  · rcases storeProbeNegative_cases state.resources.cache subQuery₀
                                        tcpRespA state.now with heq | ⟨quW, negTtlW, soaRRW, hquW, hextW, heq⟩
                                    · rw [heq]
                                      exact hrecS
                                    · rw [heq]
                                      exact cacheRecCanon_storeNegative _ _ _ _ _ _ _ _ hrecS
                                  · rcases storeProbeNegative_cases state.resources.cache subQuery₀
                                        tcpRespA state.now with heq | ⟨quW, negTtlW, soaRRW, hquW, hextW, heq⟩
                                    · rw [heq]
                                      exact hnegS
                                    · rw [heq]
                                      refine cacheNegSoaCanon_storeNegative _ _ _ _ _ _ _ hnegS ?_
                                      intro rr hrr
                                      obtain rfl : { soaRRW with ttl := Server.capNegativeTtl negTtlW } = rr :=
                                        Option.some.inj hrr
                                      exact rrWireCanon_set_ttl _
                                        (extractSoaNegative_rrWireCanon hwfA.2.1 hextW)
                                rw [if_neg hstT] at hrun
                                by_cases hpg : (Resolver.probeRoundB state.resources.sname revealed
                                    && !Server.probePassableB tcpRespA) = true
                                · rw [if_pos hpg] at hrun
                          ...
                          · rw [if_neg hunf] at hrun
                            ...
                            ·
                              ...
                              · exact respSections_finalizeAnswer (by exact hlq) hsz
                                  (by exact hchainS) hwfA
                              · rcases storeProbeNegative_cases state.resources.cache subQuery₀
                                    respA state.now with heq | ⟨quW, negTtlW, soaRRW, hquW, hextW, heq⟩
                                · rw [heq]
                                  exact hrecS
                                · rw [heq]
                                  exact cacheRecCanon_storeNegative _ _ _ _ _ _ _ _ hrecS
...
theorem serveDatagram_verdict_sound
  ...
  | error msg =>
    ...
    rcases htr : Server.truncateUdp (VeriDNS.Impl.Message.encode response) response
        (VeriDNS.Impl.Edns.clientCap query)
      with ⟨tb, tf⟩
    rw [htr] at hrunTail
    dsimp only at hrunTail
