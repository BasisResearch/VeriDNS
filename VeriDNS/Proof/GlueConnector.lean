...
/-- The per-element agreement: impl glue per-element fn = model glue per-element fn, for a single raw `b`. -/
...
/-- The full per-host agreement (hpoint), matching `modelSlistOf_referral_eq_glueAddresses`'s hypothesis. -/
...
/-- **Referral SLIST equals the model glue (honest referral).** The impl's post-referral model SLIST is exactly
    `glueAddresses (αResp respA)` — composing `glue_per_host_eq` (per-host) + `extractNsNames_referredServers_αResp`
    (hosts) into `modelSlistOf_referral_eq_glueAddresses`. Equality ⟹ the `Perm` that `refer_hasVerdict_perm`
    consumes, with NO oracle premise. The well-formedness hyps (NS RRs/glue abstract; NS rdata canonical) are
    supplied by the honest `WorldModels` disjunct, dischargeable at the lift from the codec round-trip. -/
theorem referral_slist_eq (respA : VeriDNS.Spec.Format) (cut : ByteArray) (mc : Nat)
    (hcut : αName cut = some (referralCut (αResp respA)))
    ...
    modelSlistOf (VeriDNS.Impl.SList.DnsSList.fromNsWithGlue
        (Resolver.extractNsNames (RR := ResourceRecord) respA.authority)
        (Resolver.extractGlueRecords (Resolver.bailiwickRaws (RR := ResourceRecord) cut respA.additional)) mc)
      = glueAddresses (αResp respA) := by
  apply modelSlistOf_referral_eq_glueAddresses
  · exact extractNsNames_referredServers_αResp respA hvalidAuth
  · intro n hnmem
    obtain ⟨raw, hraw, rr, hpr, htype, hrd⟩ :=
      mem_extractNsNames respA.authority n (Array.mem_def.mpr hnmem)
    obtain ⟨na, hαna, hcanon, hnaval⟩ := hnscanon raw (Array.mem_def.mp hraw) rr hpr htype
    have hrd' : rr.rdata = n := hrd
    rw [hrd'] at hαna hcanon
    exact glue_per_host_eq respA cut n na hcut hαna hcanon hnaval hvalidAdd

/-- **The response-transient (ttl-0) SLIST refines the model `glueAddresses`.** The impl's transient SLIST
    (installed when a delegation is followed without caching it — a zero-TTL NS, RFC 2181) is
    `fromNsWithGlueAll (extractNsNames …) (extractGlueRecords …) mc`, whose model image CONTAINS the model
    `glueAddresses` (one address per host): `glueAddresses = modelSlistOf(fromNsWithGlue) ⊆
    modelSlistOf(fromNsWithGlueAll)`. So the model `refer`/`referForget` rule's `glueAddresses`-disjunct SLIST
    lower bound is a `Subperm` of what the resolver actually carries — the case-fold fix (`fromNsWithGlueAll`
    matches glue CI) is exactly what makes both use the same `nameEqCI` match. Composes `referral_slist_eq`
    (response glue = `glueAddresses`) with `modelSlistOf_fromNsWithGlue_subperm_all` (one-per-host ⊆ all). -/
theorem glueAddresses_subperm_transient (respA : VeriDNS.Spec.Format) (cut : ByteArray) (mc : Nat)
    (hcut : αName cut = some (referralCut (αResp respA)))
    (hvalidAuth : ∀ b ∈ respA.authority.toList, ∀ rr,
      RRParse.parseRaw (RR := ResourceRecord) b = some rr → αRR rr ≠ none)
    (hvalidAdd : ∀ b ∈ respA.additional.toList, ∀ rr,
    ...
    (glueAddresses (αResp respA)).Subperm
      (modelSlistOf (VeriDNS.Impl.SList.DnsSList.fromNsWithGlueAll
        (Resolver.extractNsNames (RR := ResourceRecord) respA.authority)
        (Resolver.extractGlueRecords (Resolver.bailiwickRaws (RR := ResourceRecord) cut respA.additional)) mc)) := by
  rw [← referral_slist_eq respA cut mc hcut hvalidAuth hvalidAdd hnscanon]
