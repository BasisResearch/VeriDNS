...
theorem reachableNamesB_canonical {qname : ByteArray} {answer : Array ByteArray}
    ...
    ∀ m ∈ Resolver.reachableNamesB (RR := VeriDNS.Spec.ResourceRecord) qname answer,
      CanonicalName m := by
  intro m hm
  rw [Resolver.reachableNamesB_eq_iter] at hm
  refine reachIterB_canonical hca _ _ ?_ m hm
  intro m' hm'
...
theorem scrubAdditionalB_subset {qname : ByteArray} {additional : Array ByteArray}
    {bytes : ByteArray} (h : bytes ∈ Server.scrubAdditionalB qname additional) :
    bytes ∈ additional :=
  (Array.mem_filter.mp h).1

theorem canonicalSection_scrubAdditionalB {qname : ByteArray} {additional : Array ByteArray}
    (h : CanonicalSection additional) :
    CanonicalSection (Server.scrubAdditionalB qname additional) :=
  fun b hb => h b (scrubAdditionalB_subset hb)

theorem scrubAdditionalB_size_le (qname : ByteArray) (additional : Array ByteArray) :
    (Server.scrubAdditionalB qname additional).size ≤ additional.size :=
  Array.size_filter_le
...
/-- 068: the qtype-relevance filter preserves canonicality (it only removes
    records). -/
theorem canonicalSection_typeScrubB {qtype : BitVec 16} {answer : Array ByteArray}
    (h : CanonicalSection answer) :
    CanonicalSection (Resolver.typeScrubB (RR := VeriDNS.Spec.ResourceRecord) qtype answer) :=
  fun b hb => h b (Resolver.typeScrubB_subset hb)
...
theorem deliveredResponse_decode_encode (query resp : Format)
    ...
    (hsz : (Resolver.typeScrubB (RR := VeriDNS.Spec.ResourceRecord) (Server.clientQtype query)
  ...
  · show resp.header.qdcount.toNat = resp.question.size
    exact hqd
  · show (BitVec.ofNat 16 (Resolver.typeScrubB (RR := VeriDNS.Spec.ResourceRecord)
        (Server.clientQtype query)
        (Resolver.scrubAnswerB (RR := VeriDNS.Spec.ResourceRecord)
          (Server.clientQname query) resp.answer)).size).toNat
      = (Resolver.typeScrubB (RR := VeriDNS.Spec.ResourceRecord) (Server.clientQtype query)
        (Resolver.scrubAnswerB (RR := VeriDNS.Spec.ResourceRecord)
  ...
  · show (BitVec.ofNat 16 (Server.scrubAuthorityB (Server.clientQname query)
    ...
    have h216 : 2 ^ 16 = 65536 := rfl
    omega
  · show (BitVec.ofNat 16 (Server.scrubAdditionalB (Server.clientQname query)
        resp.additional).size).toNat
      = (Server.scrubAdditionalB (Server.clientQname query) resp.additional).size
    rw [BitVec.toNat_ofNat]
    have hle := scrubAdditionalB_size_le (Server.clientQname query) resp.additional
    have hlt : resp.header.arcount.toNat < 65536 := resp.header.arcount.isLt
    have h216 : 2 ^ 16 = 65536 := rfl
    omega
  · exact validQuestionsOfForall hq
  · exact canonicalSection_validRRBytes
      (canonicalSection_typeScrubB (canonicalSection_scrubAnswerB hca hqn))
  · exact canonicalSection_validRRBytes (canonicalSection_scrubAuthorityB hcn)
  · exact canonicalSection_validRRBytes (canonicalSection_scrubAdditionalB hcd)
...
theorem errorResponse_decode_encode (query : Format) (rc : Rcode)
  ...
  · show (BitVec.ofNat 16 (#[] : Array ByteArray).size).toNat
      = (#[] : Array ByteArray).size
    decide
  · refine validQuestionsOfForall (qs := query.question.extract 0 1) ?_
    intro ⟨i, hi⟩
    have hsz : (query.question.extract 0 1).size = min 1 query.question.size := by
      rw [Array.size_extract]; omega
    have hi' : i < query.question.size := by omega
    have h := hq ⟨i, hi'⟩
    simpa [Array.getElem_extract] using h
