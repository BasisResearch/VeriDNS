...
/-- **`capTtlRR` preserves `CanonicalRR`.** The TTL-cap either returns the RR-blob unchanged (already
    canonical) or re-encodes it with a clamped TTL: `ResourceRecord.encode` writes exactly the `rrWire`
    layout (name, type, class, ttl, rdlength, rdata), and the parsed record's name is `labelsToWireFormat`
    of the original blob's labels with `rdlength = rdata.size`, so the re-encoding is `rrWire ls t c ttl' rdata`
    — the SAME structure with only the TTL field changed, hence still `CanonicalRR` (rdata canonicity, valid
    labels, ≤255 name all inherited). This is what threads `CanonicalRR` through `sanitizeTtlsCap` for the
    referral cache write's `CacheNsCanon`. -/
...
theorem sanitizeTtlsCap_limit_ttls :
  ...
  have hr : resp' = Server.capTtls resp := by
