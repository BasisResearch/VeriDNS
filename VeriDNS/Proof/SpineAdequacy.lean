...
theorem treeProbeRound_node
  ...
  exact delegatingProbeConsumeRound_node respond sbelt state deadline depth fuel' revealed w
    ...
    (treeRespond_qr_opcode T negAuth _ _ hqSent).2
    hqSent hsanEqP hguards.1 hguards.2.1 hguards.2.2.1 hguards.2.2.2.1
    hguards.2.2.2.2 hnext
...
theorem treeAnswerRound_delivers_pinned
    ...
    (hown : ∀ rr ∈ rrs.toList, VeriDNS.Impl.DomainName.nameEqCI
        ...
        (DomainName.randomizeCase (I (ctr + 1)) state.resources.sname) = true) :
  ...
  obtain ⟨htcT, hunfT, hcnameT, hsfT, hclsT, hansT, hfeT⟩ :=
    treeRespond_answer_classified T negAuth
      ...
      hqSent hlk hmem hrtRR (hown rrs[0] (Array.mem_def.mp hmem)) htcS hsfS hfeS
  ...
  refine ⟨_, _, fun w hwo hwids hwclk hwctr =>
    delegatingAnswerRound_delivers respond sbelt state deadline depth fuel' revealed w
      ...
      hbest hegress hbuild hprobeF hrt
      (treeRespond_header_id T negAuth _) (treeRespond_question T negAuth _)
      (treeRespond_qr_opcode T negAuth _ _ hqSent).1
      (treeRespond_qr_opcode T negAuth _ _ hqSent).2
      hqSent hsanEq htcT hunfT hcnameT hsfT hclsT hansT hfeT, ?_, ?_⟩
...
theorem spineDelegation_chain
    ...
    (q : VeriDNS.Spec.Format) (qu : VeriDNS.Spec.Question)
    (rrs : Array VeriDNS.Spec.ResourceRecord)
    (I : Nat → UInt16) (TK : Nat → UInt32) (nowS : UInt32)
    (hqu : q.question[0]? = some qu)
    (htcq : (q.header.tc == 1) = false)
    (hrcq : q.header.rcode = VeriDNS.Spec.Rcode.noError)
    ...
    (hown : ∀ (seed : UInt16), ∀ rr ∈ rrs.toList, VeriDNS.Impl.DomainName.nameEqCI
        (VeriDNS.Spec.RRParse.rrName rr)
        (DomainName.randomizeCase seed qu.qname) = true) :
    ∀ (spine : List SpineHop) (revealed prevCut : Nat) (ip : BitVec 32)
      (used : Array ByteArray) (nsName : ByteArray) (fuel ctr ectr : Nat) (cl : UInt32)
      ...
      ∃ (resp : VeriDNS.Spec.Format) (cout : DnsCache),
        ...
        ∧ resp.answer = rrs.map VeriDNS.Spec.RRParse.rrBytes
        ∧ resp.question = q.question := by
  have hqsf : (q.header.rcode == VeriDNS.Spec.Rcode.serverFailure) = false := by
    rw [hrcq]; decide
  have hqfe : (q.header.rcode == VeriDNS.Spec.Rcode.formatError) = false := by
    rw [hrcq]; decide
  have key : ∀ (μ : Nat) (spine : List SpineHop) (revealed prevCut : Nat) (ip : BitVec 32)
    ...
    | _ μ IH =>
    intro spine revealed prevCut ip used nsName fuel ctr ectr cl state hμ hok htimely hfuel
      hrev hstep hsn hnow hlq hchain hshape hegressC hcinv hginv
    ...
    | nil =>
      ...
      ·
        obtain ⟨resp, cout, hterm, hpinA, hpinQ⟩ := treeAnswerRound_delivers_pinned respond
          leafT negAuth sbelt state deadline depth fuel' revealed entry ip q qu rrs I cl ctr
          hdl hleaf.resp_eq hstep hbest hegressC hlq hqu htcq hqsf hqfe hcanS
          (by rw [hsn]; exact hfull) hchain
          (by rw [hsn]; exact hflk (I (ctr + 1))) hsz hwfRR
          (by rw [hsn]; exact hown (I (ctr + 1)))
        exact ⟨resp, cout, fun w hwo hwids hwclk _hwtick _hwectr hwctr =>
      ...
      ·
        ...
        obtain ⟨resp, cout, htail, hpinA, hpinQ⟩ := IH
          ...
          fuel' (ctr + 2) (ectr + 1) (cl + TK ectr)
          ({ state with resources := { state.resources with
              slist := state.resources.slist.markQueried entry.name } })
          rfl hok htimely.step (by simp only [List.length_nil] at hμ ⊢; omega) hbrev hstep
          hsn hnow hlq hchain (hshape.markQueried entry.name) hegressC hcinv hginv
        ...
        exact treeProbeRound_node respond leafT negAuth sbelt state deadline depth fuel'
          ...
          (fun w' ho hto hids hclk hectr htick hctr => htail w' (ho.trans hwo)
            (hids.trans hwids)
    ...
    | cons hop rest =>
      ...
      ·
        ...
        have hneNs : (Resolver.extractNsNames (RR := VeriDNS.Spec.ResourceRecord)
            (Resolver.ownerRaws (RR := VeriDNS.Spec.ResourceRecord) (Resolver.referralCutRaw (RR := VeriDNS.Spec.ResourceRecord) (referralReply (Server.withSecrets subQuery₀ (I ctr) (I (ctr + 1))) hop.nsAuth hop.glue).authority) (referralReply (Server.withSecrets subQuery₀ (I ctr) (I (ctr + 1))) hop.nsAuth hop.glue).authority)).isEmpty = false := by
        ...
        have hclose := VeriDNS.Proof.Refinement.currentCloser_false_of_ge
          (Resolver.extractNsNames (RR := VeriDNS.Spec.ResourceRecord)
            (Resolver.ownerRaws (RR := VeriDNS.Spec.ResourceRecord) (Resolver.referralCutRaw (RR := VeriDNS.Spec.ResourceRecord) (referralReply (Server.withSecrets subQuery₀ (I ctr) (I (ctr + 1))) hop.nsAuth hop.glue).authority) (referralReply (Server.withSecrets subQuery₀ (I ctr) (I (ctr + 1))) hop.nsAuth hop.glue).authority))
        ...
        obtain ⟨resp, cout, htail, hpinA, hpinQ⟩ := IH
          ...
          (VeriDNS.Spec.RRParse.rrRdata (RR := VeriDNS.Spec.ResourceRecord) hop.nsrr)
          fuel' (ctr + 2) (ectr + 1) (cl + TK ectr) st rfl hrest htimely.step hfuel' hrev'
          hcsP (hsnSt.trans hsn) (hnowP.trans hnow)
        ...
        have hst : st = st' := by
          ...
          injection h
        subst hst
        exact htail w' (ho.trans hwo) (hids.trans hwids)
          (by rw [hclk, hwclk, hwtick, hwectr])
          (htick.trans hwtick)
          (by rw [hectr, hwectr])
          (by rw [hctr, hwctr])
      ·
        ...
        obtain ⟨resp, cout, htail, hpinA, hpinQ⟩ := IH
          ...
          nsName fuel' (ctr + 2) (ectr + 1) (cl + TK ectr)
          ({ state with resources := { state.resources with
              slist := state.resources.slist.markQueried entry.name } })
          rfl hok htimely.step (by omega) hbrev hstep hsn hnow hlq hchain
  ...
  intro spine revealed prevCut ip used nsName fuel ctr ectr cl state hok htimely hfuel hrev
    hstep hsn hnow hlq hchain hshape hegressC hcinv hginv
  exact key (spine.length * 128 + (DomainName.labelCount qu.qname - revealed)) spine revealed
    prevCut ip used nsName fuel ctr ectr cl state rfl hok htimely hfuel hrev hstep hsn hnow hlq
...
theorem resolveWithIO_spine_adequate_warm
  ...
  obtain ⟨resp, cout, hch, hpinA, hpinQ⟩ := spineDelegation_chain respond leafT negAuth sbelt
    (now + budget) depth q qu rrs w.ids w.tick state.now hqu htcq hrcq hcanon h127
    hnoNs hnsz hcanNeg hnegCap hflk hsz hwfRR hown
    spine (Server.seedRevealed state) prevCut ip used₀ nsName fuel w.idCtr w.exchCtr w.clock
    state hok htimely hfuel hrevS hstep hsn rfl hlq hchain0 hshape hegress hcinv hginv
...
timely spine capstone collapses to the pre-061 frozen-clock statement — the
old `hdl` premise alone suffices, and the conclusion is unchanged.  Dual:
`FreeIO.run_resolveWithIO_deadline_witness` shows slow worlds get the deadline
error instead, so `TimelyWorld` is a genuine (bivalent) premise. -/
theorem resolveWithIO_spine_adequate_warm_tick0
    (respond : ByteArray → VeriDNS.Spec.Format → VeriDNS.Spec.Format)
    (leafT : VeriDNS.Spec.Node VeriDNS.Spec.ResourceRecord) (negAuth : Array ByteArray)
    (query : VeriDNS.Spec.Format) (sbelt : DnsSList) (cache : DnsCache) (now : UInt32)
    (fuel depth : Nat) (budget : UInt32) (w : World)
  ...
  resolveWithIO_spine_adequate_warm respond leafT negAuth query sbelt cache now fuel depth
    budget w state spine prevCut ip nsName used₀ q qu rrs hcoop hpause
    (TimelyWorld.of_tick0 htick fuel hdl) hok hfuel hstep hsn hlq hchain0 hshape hegress
    hcinv hginv hqu htcq hrcq hcanon h127 hnoNs hnsz hcanNeg hnegCap hflk hsz hwfRR hown
...
theorem resolveWithIO_spine_adequate
    ...
    ∃ (K : Nat) (w' : World) (resp : VeriDNS.Spec.Format) (cout : DnsCache),
      ...
      ∧ resp.question = q.question :=
  resolveWithIO_spine_adequate_warm respond leafT negAuth query sbelt cache now fuel depth
    budget w state spine prevCut ip nsName #[] q qu rrs hcoop hpause htimely hok hfuel hstep hsn
    hlq hchain0 hshape hegress (DescentCacheInv.of_empty hcold _)
    (DescentGlueInv.of_empty hcold _) hqu htcq hrcq hcanon h127 hnoNs hnsz hcanNeg
    hnegCap hflk hsz hwfRR hown
...
theorem resolveWithIO_within_bound
  ...
  refine resolveWithIO_spine_adequate respond leafT negAuth query sbelt cache now fuel depth
    budget w state spine prevCut ip nsName q qu rrs hcoop hpause htimely hok ?_ hstep hsn hlq
    hchain0 hshape hegress hcold hqu htcq hrcq hcanon h127 hnoNs hnsz hcanNeg hnegCap
    hflk hsz hwfRR hown
...
theorem resolveWithIO_spine_no_starvation
  ...
  obtain ⟨K', w', resp, cout, hrun', hpinA, hpinQ⟩ := resolveWithIO_within_bound respond leafT
    ...
    hcoop hpause htimely hok hfb hstep hsn hlq hchain0 hshape hegress hcold hqu htcq hrcq
    hcanon h127 hnoNs hnsz hcanNeg hnegCap hflk hsz hwfRR hown
