...
/-! ### `normRaws` membership + generic invariant transfer (RFC 2181 §5.2 normalization)

    Upstream of the cache-invariant proofs so `CacheAgrees`/`CacheWf`/`OneExpiryPerKey`/… preservation
    through the now-normalizing `cacheUnlessTruncated` can transfer ttl-insensitive hypotheses across
    `normRaws`. -/
...
/-- The re-serialized normalized member round-trips back to itself. -/
...
/-- Every parsed record of `normRaws raws` is a TTL-variant of a parsed record of the original section. -/
...
/-- **Generic invariant transfer across `normRaws`** for a ttl-insensitive per-record predicate. -/
...
/-- `SectionAgrees` transfers across `normRaws` (`RRInTree` is ttl-insensitive). -/
...
theorem afterResume_sound (T : Node ResourceRecord)
  ...
  · exact ⟨fun f hf => by cases hf; exact hr.1, cacheAgrees_bound hr.2.cache⟩
  · exact ⟨cacheAgrees_bound hr.cache, hr.chain⟩
  · exact ⟨(fun f hf => nomatch hf), hs.cache⟩

/-- The RFC 1034 §5.3.3 cache-first re-check (`gluelessRecheck`) only ever delivers
    tree-derived data: a `some hit` outcome is a `finalizeAnswer` around either a negative
    response (empty answer section) or a typed cache hit whose RRs the cache-agreement
    invariant traces to the DNS tree. Mirrors `stepCheckLocal_sound`'s `.answer` arm minus
    the CNAME peeling (the re-check does none, so the chain hypothesis is `state`'s own). -/
      ...
      ·
        ...
        ·
          ...
          ·
            refine SatisfiesM.bind (satisfiesM_true _) ?_
            intro _ _
            exact ioResumeLoop_sound T hnet sbelt depth' fuel' _ _ ⟨hs.cache, hs.chain⟩
          ·
            refine SatisfiesM.bind (satisfiesM_true _) ?_
            intro _ _
            exact ioResumeLoop_sound T hnet sbelt depth' fuel' _ _ ⟨hs.cache, hs.chain⟩
          ·
            ...
            refine SatisfiesM.bind
              (ioResumeLoop_sound T hnet sbelt depth' fuel' _ deadline hrs) ?_
            intro y hy
            refine SatisfiesM.bind (satisfiesM_true _) ?_
            intro _ _
            split
                ...
                · next hit hre =>
                  exact SatisfiesM.pure
                    ⟨fun f hf => by cases hf; exact gluelessRecheck_sound hs.chain hy.2 hit hre,
                      hy.2⟩
                · exact ioResumeLoop_sound T hnet sbelt depth' fuel' _ _ ⟨hy.2, hs.chain⟩
              ·
                exact ioResumeLoop_sound T hnet sbelt depth' fuel' _ _ ⟨hs.cache, hs.chain⟩
            ·
              exact ioResumeLoop_sound T hnet sbelt depth' fuel' _ _ ⟨hs.cache, hs.chain⟩
          ...
          ·
            ...
            ·
              ...
              ·
                ...
                ·
                  exact ioResumeLoop_sound T hnet sbelt depth fuel' _ _ ha
            ·
              refine SatisfiesM.bind (satisfiesM_true _) ?_
              intro _ _
              exact ioResumeLoop_sound T hnet sbelt depth fuel' _ _
