...
theorem serveDatagram_delivers_of_resolve
  ...
  rw [serveDatagram_served clientSock acl sbelt cache queryBytes clientAddr query
    hperm hdec hqr hqp hedns hnany]
  obtain ⟨m, out, w₃, hreply⟩ := replyForResolution_runs query rr cache' w.clock w₂
  obtain ⟨response, c''⟩ := out
  exact ⟨(K + (m + (0 + 1))) + 1, _, _,
    run_now_bind _ w (run_bind hres _ (run_bind hreply _
      (run_sendTo_bind _ _ _ _ w₃ (run_pure' 0 _ _))))⟩

/-- **Finding 061, serve-level SERVFAIL witness.** When the embedded resolution
returns an error — in particular the deadline error of
`FreeIO.run_resolveWithIO_deadline_witness` (a slow world crossing the query
deadline mid-resolution) — `serveDatagram` still serves, and the datagram it
SENDS to the client is exactly the (cap-truncated, OPT-echoing) encoding of
`buildErrorResponse query .serverFailure`: the deadline error is
client-visible as SERVFAIL, pinned on the delivery log. -/
theorem serveDatagram_error_sends_servfail
    (clientSock : Unit) (acl : Server.ClientAcl) (sbelt : DnsSList) (cache : DnsCache)
    (queryBytes clientAddr : ByteArray) (query : VeriDNS.Spec.Format)
    (K : Nat) (msg : String) (cache' : DnsCache) (w w₂ : World)
    (hperm : Server.permitted acl clientAddr = true)
    (hdec : VeriDNS.Impl.Message.decode queryBytes = .ok query)
    (hqr : (query.header.qr == 1) = false)
    (hqp : Server.queryProblem query = none)
    ...
    (hnany : Server.isAnyQuery query = false)
    (hres : Prog.run K (Server.resolveWithIO (M := Prog) (Sock := Unit)
        query sbelt cache w.clock) w = some ((.error msg, cache'), w₂)) :
    ∃ (n : Nat) (cacheOut : DnsCache) (w' : World),
      Prog.run n (Server.serveDatagram (M := Prog) (Sock := Unit)
      ...
      ∧ w'.sent = w₂.sent ++
          [((Server.truncateUdp
              (VeriDNS.Impl.Message.encode (VeriDNS.Impl.Edns.withReplyOpt query
                (Server.finalizeForClient (Server.buildErrorResponse query .serverFailure))))
              (VeriDNS.Impl.Edns.withReplyOpt query
                (Server.finalizeForClient (Server.buildErrorResponse query .serverFailure)))
              (VeriDNS.Impl.Edns.clientCap query)).1, clientAddr)] := by
  rw [serveDatagram_served clientSock acl sbelt cache queryBytes clientAddr query
    hperm hdec hqr hqp hedns hnany]
  have hreply : Prog.run 1 (Server.replyForResolution (M := Prog) (Sock := Unit)
      query (.error msg) cache' w.clock) w₂
      = some ((Server.finalizeForClient (Server.buildErrorResponse query .serverFailure),
          cache'), { w₂ with trace := w₂.trace ++ [s!"SERVFAIL: {msg}"] }) := by
    unfold Server.replyForResolution
    exact run_log_bind _ _ w₂ (run_pure' 0 _ _)
  exact ⟨(K + (1 + (0 + 1))) + 1, _, _,
    run_now_bind _ w (run_bind hres _ (run_bind hreply _
      (run_sendTo_bind _ _ _ _ _ (run_pure' 0 _ _)))), rfl⟩
...
theorem serveTcpDatagram_delivers_of_resolve
  ...
  exact ⟨(K + (m + (0 + 1))) + 1, _, _,
    run_now_bind _ w (run_bind hres _ (run_bind hreply _
      (run_tcpSend_bind _ _ _ w₃ (run_pure' 0 _ _))))⟩
...
theorem serveDatagram_depth1_adequate
    ...
    (hdl₂ : ¬ (w.clock + w.tick w.exchCtr ≥ w.clock + 5))
  ...
  obtain ⟨resp, cout, K, w₂, hrun, hansEq, hquesEq⟩ := resolveWithIO_depth1_adequate respond
    ...
    hcoop hroot hchild hegressChild hpause hsentEq hsendq hdl hdl₂ hbest hegress hbuild hrev hcanon
    hsn hlq hqu hqtc hqsf hqfe hcold hchain0 hnsz hgsz hcanNs hcanGlue hoptG hnA hdG hns hsoa hclq
    hbaiq hauthN hpns hnsName hnsType hnsClass hnzNs hfreshNs haddN hpg hgKey hgType hgClass
    hnzG hfreshG hgSize hlk hsz hwfRR hown
  ...
  obtain ⟨n, cacheOut, w', hserve⟩ := serveDatagram_delivers_of_resolve clientSock acl sbelt
    cache queryBytes clientAddr query K (.ok resp) cout w w₂ hperm hdec hqr hqp hedns hnany hrun'
