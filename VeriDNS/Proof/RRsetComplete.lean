...
theorem storeStep_foldl_survivor (cred : Trustworthiness) (now : UInt32)
    ...
    ∃ e ∈ (L.foldl (storeStep cred now) c).records,
      SameKey e.rr trr ∧ rdataEqCI trr.type e.rr.rdata trr.rdata = true := by
  ...
  have hdata : sameData r trr = true := sameData_iff.mpr ⟨hkr.1, hkr.2.1, hkr.2.2, rdataEqCI_of_eq _ hrd⟩
...
theorem cacheRRsNorm_complete (cache : DnsCache) (raws : Array ByteArray)
    ...
    ∃ e ∈ (cacheRRs (C := DnsCache) (RR := ResourceRecord) cache
      ...
      SameKey e.rr rr ∧ rdataEqCI rr.type e.rr.rdata rr.rdata = true := by
  rw [cacheRRs_eq]
