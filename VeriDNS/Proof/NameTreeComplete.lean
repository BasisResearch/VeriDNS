            ...
            · next hansOuter =>
              ...
              ·
                ...
                ·
                  ...
                  have hncname : ¬ HasOwnedCname qu_r.qname resp.answer := by
              ...
              | some resp =>
                ...
                ·
                  ...
                  ·
                    ...
                    have hrc : resp.header.rcode = Rcode.nameError :=
                      strictDenialB_rcode hguard.2
                    have habsent := probeAbsent_of_strictDenial hacc hcons hrc
                    have hq0 := buildSubQuery_question_probe hs.query hs.question
                      hguard.1 hbsq
                    have hsnameNone : nodeAtName T state.resources.sname = none :=
                      nodeAtName_none_of_minimisedName_none (habsent _ hq0)
                    have hlook : treeLookup T state.resources.sname qu₀.qtype
                        = .nameError :=
                      (treeLookup_nameError_iff ..).mpr hsnameNone
                    refine SatisfiesM.pure ⟨?_,
                      cacheAgrees_storeProbeNegative hs.agrees.cache habsent,
                      lookupComplete_storeProbeNegative hs.complete,
                      oneExpiry_storeProbeNegative hs.oneExp,
                      negativesFaithful_storeProbeNegative hs.negs habsent hrc⟩
                    intro f hf htc
                    cases hf
                    exact answersFromTree_of_terminal hs.reach hlook
                      (fun _ _ hcon => nomatch hcon)
                      (finalizeAnswer_answer_agrees hs.agrees.chain hcons.answer)
                      (by rw [finalizeAnswer_rcode]; exact hrc)
                  ·
                    split
                    ·
                      refine SatisfiesM.bind (satisfiesM_true' _) ?_
