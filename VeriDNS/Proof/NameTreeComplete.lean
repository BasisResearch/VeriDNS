...
theorem sameData_iff {a b : ResourceRecord} :
    sameData a b = true ↔
      ...
      a.class = b.class ∧ a.rdata = b.rdata := by
  unfold sameData
...
theorem lookupComplete_storeNegative {T : Node ResourceRecord} {c : DnsCache}
    ...
    LookupComplete T (c.storeNegative name qt qc rc soa expiry) := h
...
theorem mem_store_iff {c : DnsCache} {rr : ResourceRecord} {now : UInt32}
    ...
    e ∈ (c.store rr now cred).records ↔
      ...
      e = ⟨rr, now + rr.ttl.toNat.toUInt32, false, cred⟩ := by
...
theorem mem_foldl_store :
    ...
    e ∈ c.records ∨
      ∃ b ∈ L, ∃ r, RRParse.parseRaw (RR := ResourceRecord) b = some r ∧
        e = ⟨r, now + r.ttl.toNat.toUInt32, false, cred⟩ ∧ r.ttl ≠ 0 ∧
        ...
        · rw [heq]
          ...
          ·
            ...
            exact sameData_iff.mpr ⟨nameEqCI_trans hk'.1 hn',
              hk'.2.1.trans ht', hk'.2.2.trans hc', hrd ▸ hd'⟩
      ...
      · rw [if_pos hempty]
        ...
        · rw [if_neg hq5]
          ...
          | some crr =>
            ...
            have hrdc : RRParse.rrRdata (RR := ResourceRecord) crr
                = trrC.rdata := by
              rw [rrRdata_eq, hceq]
              exact ((sameData_iff.mp hdataC).2.2.2).symm
...
private theorem extractCname_some {rrs : Array ByteArray}
    {canonical : ByteArray}
    (h : extractCname (RR := ResourceRecord) rrs = some canonical) :
    ∃ b ∈ rrs.toList, ∃ rr,
      RRParse.parseRaw (RR := ResourceRecord) b = some rr ∧
  ...
  | some rr =>
    ...
    · next hty =>
      cases hcond
      exact ⟨b, by simpa using hb, rr, hp, bv16_beq_iff.mp hty, rfl⟩
    · cases hcond

private theorem extractCname_none {rrs : Array ByteArray}
    (h : extractCname (RR := ResourceRecord) rrs = none) :
    ¬ HasType rrs cnameType := by
  rintro ⟨b, hb, rr, hp, hty⟩
  unfold extractCname at h
  have hnone := Array.findSome?_eq_none_iff.mp h b (by simpa using hb)
  rw [hp] at hnone
  dsimp only [] at hnone
  rw [if_pos (show (RRParse.rrType (RR := ResourceRecord) rr
...
theorem stepAnalyzeResponse_complete {T : Node ResourceRecord} {q₀ : Format}
  ...
  · intro r rst hr htc
    ...
    · next resp heq =>
      have hcons := hresp resp heq
      obtain ⟨qu_r, hqur, hqci, hqty, hqcl⟩ := hs.respMatch resp heq
    ...
    · next resp heq =>
      ...
      ·
        ...
        have hansT : ¬ (answersQueryB (RR := ResourceRecord) resp = true) := by
          rw [hansF]
          simp
        have hext : extractCname (RR := ResourceRecord) resp.answer
...
private theorem questionMatches_facts {a b : Array VeriDNS.Spec.Question}
  ...
  · next qa qb ha hb =>
    ...
    exact ⟨qa, qb, ha, hb, h.1.1, bv16_beq_iff.mp h.1.2,
      bv16_beq_iff.mp h.2⟩
  · cases h
...
private theorem buildSubQuery_question
    ...
    (hqu : q₀.question[0]? = some qu₀) {sub : Format}
    (hsub : buildSubQuery state = some sub) :
    sub.question[0]? = some
        ...
        ·
          ...
          ·
            ...
            ·
              ...
              have hq0 := buildSubQuery_question hs.query hs.question hbsq
              rw [show (withRandomId subQuery₀ rid).question
                  = subQuery₀.question from rfl, hq0] at hqb
              cases hqb
              have hmatch : ∃ qu, resp.question[0]? = some qu ∧
                  nameEqCI qu.qname state.resources.sname = true ∧
                  qu.qtype = qu₀.qtype ∧ qu.qclass = qu₀.qclass :=
                ⟨qa, hqa, hci, hty, hcl⟩
              refine SatisfiesM.bind (satisfiesM_true' _) ?_
