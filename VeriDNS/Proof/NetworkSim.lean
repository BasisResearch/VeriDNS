...
/-- `αRCode` sends only `nameError` to the model `nameError`, so an impl rcode
    whose abstraction is not `nameError` is itself not `nameError`. -/
theorem rcode_ne_nameError_of_αRCode_ne {rc : VeriDNS.Spec.Rcode}
    (h : αRCode rc ≠ VeriDNS.Spec.Net.RCode.nameError) :
    (rc == VeriDNS.Spec.Rcode.nameError) = false := by
  cases rc <;> first | rfl | exact absurd rfl h

/-- **Honest ¬NXDOMAIN witness through the TCP oracle.**  A TCP honest reply that
    carries a type-matching answer record (`answersQueryB`) is not NXDOMAIN: the
    honest model reply covers the query type, and a covered honest reply is never
    `nameError` (an NXDOMAIN carries only CNAME chain links).  This supplies the
    `¬ nameError` premise of the off-owner entitlement bridge (findings 036 /
    off-owner-A) for the TCP answer terminal (which has no spoof disjunct). -/
theorem answersQuery_honest_not_nameError (net : Network) (ns : NetState) (ra : String)
    (ednsBuf : Nat) (now : Time) (w : World)
    (q : VeriDNS.Spec.Format) (id cid : UInt16) (ab bytes : ByteArray)
    (resp0 resp₀ resp : VeriDNS.Spec.Format) (qm : Query)
    (hwm : WorldModelsTcp net ns ra ednsBuf now w)
    (hO : w.tcpOracle (VeriDNS.Impl.Message.encode
        (VeriDNS.Impl.Server.withSecrets q id cid)) ab = some bytes)
    (hdec : VeriDNS.Impl.Message.decode bytes = .ok resp0)
    (hsan : VeriDNS.Impl.Server.sanitizeTtlsCap resp0 = some resp₀)
    (hacc : VeriDNS.Impl.Server.acceptResponse
        (VeriDNS.Impl.Server.withSecrets q id cid) resp₀ = some resp)
    (hαQ : αQuery q = some qm)
    (hansI : VeriDNS.Impl.Resolver.answersQueryB (RR := VeriDNS.Spec.ResourceRecord) resp = true) :
    (resp.header.rcode == VeriDNS.Spec.Rcode.nameError) = false := by
  obtain ⟨srv, tr, ref, hfind, hans, hrag, hreach, hisref, hcnbi, hansSec, hvalid,
    hvalidAuth, hbail, hnsr, hnd, hauthSec, haddSec, haaeq, hadditional, htceq⟩ :=
    hwm q id cid ab bytes resp0 resp₀ resp qm hO hdec hsan hacc hαQ
  have hqm : VeriDNS.Impl.Server.questionMatches resp.question
      (VeriDNS.Impl.Server.withSecrets q id cid).question = true :=
    VeriDNS.Proof.Refinement.acceptResponse_questionMatches hacc
  have hcov : ∃ r ∈ ref.answer, qm.qtype.covers r.rdata.rtype = true :=
    positive_answer_covered hαQ hqm hansI hvalid hrag
  have hne : ref.rcode ≠ VeriDNS.Spec.Net.RCode.nameError :=
    serverAnswers_covered_not_nameError_uncond hans hcov
  apply rcode_ne_nameError_of_αRCode_ne
  rw [← (αResp_components resp).1, hrag.1]
  exact hne
...
theorem answerCname_hasVerdict_hv (net : Network) (ns : NetState) (ra : String) (ednsBuf : Nat)
    ...
    (hcf0 : VeriDNS.Spec.Net.WriteRefines now' cf0 (c.absorb now q.qname (reply.msg.cnameOwned q.qname)))
...
theorem cname_classifier (net : Network) (ns : NetState) (ra : String) (ednsBuf : Nat)
    ...
    (cf0 : VeriDNS.Spec.Net.Cache)
    (hcf0 : VeriDNS.Spec.Net.WriteRefines now' cf0 (c.absorb now q.qname (ref.cnameOwned q.qname)))
    (cf : VeriDNS.Spec.Net.Cache) (hcf : VeriDNS.Spec.Net.WriteRefines now' cf cf0)
    (vsub v : Response) (hrc : v.rcode = vsub.rcode) (hva : v.answer = cn :: vsub.answer)
    (hrec : HasVerdict net ns ra ednsBuf rttOf now' (q.qname :: nseen) []
        cf nsl { q with qname := target } vsub) :
...
/-- Finding 019 twin of `section_owner_extra_perm` for the chased-CNAME slice:
the impl `cnameRaws` push image is a permutation of the model `cnameOwned`
filter image. -/
theorem section_cnameOwner_extra_perm (sec : Array ByteArray) (sname : ByteArray)
    (qnN : VeriDNS.Spec.Net.Name)
    (cred : VeriDNS.Spec.Trustworthiness) (now : UInt32)
    (hsq : αName sname = some qnN)
    (hsc : sname = VeriDNS.Impl.DomainName.labelsToWireFormatGo qnN)
    (hsv : ∀ x ∈ qnN, x.size ≤ 63)
    (hno : ∀ b ∈ (VeriDNS.Impl.Cache.normRaws
        (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname sec)).toList,
        ∀ rr, VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b = some rr →
        (now + rr.ttl.toNat.toUInt32).toNat = now.toNat + rr.ttl.toNat)
    (hcanmap : ∀ e ∈ VeriDNS.Impl.Cache.rrsOf
        (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname sec),
        RRCanonMappable e) :
    (((VeriDNS.Impl.Cache.normRaws
        (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname sec)).toList.flatMap
        (pushOf cred now)).filterMap αCacheRR).Perm
      ((VeriDNS.Spec.Net.normalizeTTL
          ((αSection sec).filter (fun r => r.rdata.rtype == RRType.cname
            && VeriDNS.Spec.Net.nameEq r.owner qnN))).reverse.flatMap
        (modelPushOf now.toNat (αCred cred))) := by
  rw [flatMap_pushOf_filterMap_eq _ cred now hno]
  have hb : (VeriDNS.Impl.Cache.normRaws
        (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname sec)).toList.filterMap
      (fun b => (VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b).bind αRR)
      = VeriDNS.Spec.Net.normalizeTTL
          ((αSection sec).filter (fun r => r.rdata.rtype == RRType.cname
    ...
    have hαs : (VeriDNS.Impl.Cache.normRaws
        ...
        (fun b => (VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b).bind αRR)
        = αSection (VeriDNS.Impl.Cache.normRaws
          (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname sec)) := by
      unfold αSection
      congr 1
      funext b
      cases VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b <;> rfl
    rw [hαs, αSection_normRaws _ hcanmap, αSection_cnameRaws_eq sname qnN sec hsq hsc hsv]
  rw [hb]
  exact (List.reverse_perm _).symm.flatMap_right _
...
theorem absorb_cnameOwned_pos (c : VeriDNS.Spec.Net.Cache) (now : VeriDNS.Spec.Net.Time)
    (qn : VeriDNS.Spec.Net.Name) (resp : VeriDNS.Spec.Net.Response) :
    (c.absorb now qn (resp.cnameOwned qn)).pos
      = ((VeriDNS.Spec.Net.normalizeTTL
            (resp.answer.filter (fun r => r.rdata.rtype == RRType.cname
              && VeriDNS.Spec.Net.nameEq r.owner qn))).reverse.flatMap
            (modelPushOf now (if resp.aa then VeriDNS.Spec.Net.Cred.authoritative
                                          else VeriDNS.Spec.Net.Cred.glue)))
        ++ c.pos := by
  have hsub : (resp.answer.filter (fun r => r.rdata.rtype == RRType.cname
        && VeriDNS.Spec.Net.nameEq r.owner qn)).filter
      (fun r => VeriDNS.Spec.Net.isAncestor qn r.owner)
      = resp.answer.filter (fun r => r.rdata.rtype == RRType.cname
        && VeriDNS.Spec.Net.nameEq r.owner qn) :=
    List.filter_eq_self.mpr (fun r hr =>
      VeriDNS.Spec.Net.isAncestor_of_nameEq
        (Bool.and_eq_true _ _ |>.mp (List.mem_filter.mp hr).2).2)
  unfold VeriDNS.Spec.Net.Cache.absorb
  simp only [VeriDNS.Spec.Net.Response.cnameOwned_answer,
    VeriDNS.Spec.Net.Response.cnameOwned_authority,
    VeriDNS.Spec.Net.Response.cnameOwned_additional,
    VeriDNS.Spec.Net.Response.cnameOwned_aa, List.filter_nil,
    show VeriDNS.Spec.Net.normalizeTTL ([] : List VeriDNS.Spec.Net.RR) = [] from rfl,
    List.foldl_nil]
  rw [hsub, foldl_insert_concrete]
  rfl
...
theorem absorbNeg_neg_append (now : VeriDNS.Spec.Net.Time) (q : VeriDNS.Spec.Net.Query)
  ...
  | some ttl =>
    by_cases h1 : (resp.rcode == VeriDNS.Spec.Net.RCode.nameError && resp.answer.isEmpty) = true
...
theorem reachAny_perm {owner : VeriDNS.Spec.Net.Name} {a b : List VeriDNS.Spec.Net.RR}
    (h : a.Perm b) (o : VeriDNS.Spec.Net.Name)
    (ha : (VeriDNS.Spec.Net.reachableNames owner a).any
      (fun n => VeriDNS.Spec.Net.nameEq o n) = true) :
    (VeriDNS.Spec.Net.reachableNames owner b).any
      (fun n => VeriDNS.Spec.Net.nameEq o n) = true := by
  rw [List.any_eq_true] at ha ⊢
  obtain ⟨n, hn, ho⟩ := ha
  exact ⟨n, VeriDNS.Spec.Net.reachableNames_complete
    (VeriDNS.Spec.Net.cnameReachable_of_subset (fun r hr => h.subset hr)
      (VeriDNS.Spec.Net.reachableNames_sound owner a n hn)), ho⟩

theorem addressOf_perm {owner : VeriDNS.Spec.Net.Name} {a b : VeriDNS.Spec.Net.Response}
    (h : a.answer.Perm b.answer) :
    VeriDNS.Spec.Net.addressOf owner a = VeriDNS.Spec.Net.addressOf owner b := by
  unfold VeriDNS.Spec.Net.addressOf
  have hfun : VeriDNS.Spec.Net.aRecordOf (VeriDNS.Spec.Net.reachableNames owner a.answer)
      = VeriDNS.Spec.Net.aRecordOf (VeriDNS.Spec.Net.reachableNames owner b.answer) := by
    funext r
    unfold VeriDNS.Spec.Net.aRecordOf
    cases hbv : (VeriDNS.Spec.Net.reachableNames owner b.answer).any
        (fun n => VeriDNS.Spec.Net.nameEq r.owner n) with
    | true =>
      rw [reachAny_perm h.symm r.owner hbv]
    | false =>
      cases hav : (VeriDNS.Spec.Net.reachableNames owner a.answer).any
          (fun n => VeriDNS.Spec.Net.nameEq r.owner n) with
      | false => rfl
      | true =>
        rw [reachAny_perm h r.owner hav] at hbv
        exact Bool.noConfusion hbv
  rw [hfun, perm_foldl_eq ipMinOpt_lcomm (h.filterMap _)]
...
theorem resolves_cache_congr {net : VeriDNS.Spec.Net.Network} {ns : VeriDNS.Spec.Net.NetState}
  ...
  | gaveUp c slist q hw =>
    intro cc hmm
    refine ⟨cc, _, Resolves.gaveUp cc slist q ?_, RespAgree.refl _, hmm⟩
    cases hw with
    | serversExhausted hmiss hnmiss hns =>
      exact .serversExhausted (hmm.hit_nil hmiss) ((hmm.2.1 _ _).symm.trans hnmiss) hns
    | outOfBudget hmiss hnmiss =>
      exact .outOfBudget (hmm.hit_nil hmiss) ((hmm.2.1 _ _).symm.trans hnmiss)
  ...
  | answerCname addr rest q srv tr resp cn target id srcPort c nsl ftr rpath tEnd cout final
    ...
    exact ⟨cout, _, Resolves.answerCname addr rest q srv tr resp cn target id srcPort cc nsl ftr rpath tEnd cout final
      ...
      cf0 (hcf0.trans_perm (hmm.absorb _ q.qname (reply.msg.cnameOwned q.qname)).1
        (hmm.absorb _ q.qname (reply.msg.cnameOwned q.qname)).2.1
        (hmm.absorb _ q.qname (reply.msg.cnameOwned q.qname)).2.2)
      cf hcf hrec,
      RespAgree.refl _, MatchMaxEquiv.refl _⟩
...
theorem answerCname_hop_MatchMaxEquiv (c : VeriDNS.Impl.Cache.DnsCache) (resp : VeriDNS.Spec.Format) (sname : ByteArray)
    ...
    (h1 : ∀ (acc : VeriDNS.Impl.Cache.DnsCache) (b : ByteArray) (rr : VeriDNS.Spec.ResourceRecord),
        b ∈ (VeriDNS.Impl.Cache.normRaws (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer)).toList →
        VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b = some rr → (rr.ttl == 0) = false →
    ...
    (hnoA : ∀ b ∈ (VeriDNS.Impl.Cache.normRaws (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer)).toList,
        ∀ rr, VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b = some rr →
        (now + rr.ttl.toNat.toUInt32).toNat = now.toNat + rr.ttl.toNat)
    (hcanmap : ∀ e ∈ VeriDNS.Impl.Cache.rrsOf (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer), RRCanonMappable e) :
    MatchMaxEquiv
      (αCache (VeriDNS.Impl.Resolver.cacheUnlessTruncated (C := VeriDNS.Impl.Cache.DnsCache) (RR := VeriDNS.Spec.ResourceRecord)
        c resp (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer)
          (VeriDNS.Impl.Resolver.credAnswer (resp.header.aa == 1)) now))
      ((αCache c).absorb now.toNat bwN ((αResp resp).cnameOwned bwN)) := by
  have hI := cacheRRs_αCache_pos (VeriDNS.Impl.Cache.normRaws (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer))
    (VeriDNS.Impl.Resolver.credAnswer (resp.header.aa == 1)) now c h1
  rw [cacheUnlessTruncated_untruncated _ _ _ _ _ htc]
  have hM := absorb_cnameOwned_pos (αCache c) now.toNat bwN (αResp resp)
  have hsec := section_cnameOwner_extra_perm resp.answer sname bwN (VeriDNS.Impl.Resolver.credAnswer (resp.header.aa == 1)) now hcut hsc hsv hnoA hcanmap
  rw [αCred_credAnswer] at hsec
  refine ⟨fun nowT q' => ?_, fun nowT q' => ?_, fun nowT q' => ?_⟩
  · refine topServed_bridge_pos (αCache c) _ _ _ _ nowT q' hI ?_ hsec
    show ((αCache c).absorb now.toNat bwN ((αResp resp).cnameOwned bwN)).pos = _
    rw [hM]; rfl
  · show VeriDNS.Spec.Net.Cache.negHit _ nowT q' = _
    unfold αCache VeriDNS.Spec.Net.Cache.negHit
        ...
        ·
          ...
          have hrec : (c.store rr now cred).records
              = (c.records.filter fun e => !(entKeyB e rr
                  && (e.expiry != now + rr.ttl.toNat.toUInt32 || VeriDNS.Impl.Cache.rdataEqCI rr.type e.rr.rdata rr.rdata))).push
          ...
          have hlist : (c.store rr now cred).records.toList
              = c.records.toList.filter (fun e => !(entKeyB e rr
                  && (e.expiry != now + rr.ttl.toNat.toUInt32 || VeriDNS.Impl.Cache.rdataEqCI rr.type e.rr.rdata rr.rdata)))
          ...
          ·
            ...
            have hkeep : (!(entKeyB e rr
                && (e.expiry != now + rr.ttl.toNat.toUInt32 || VeriDNS.Impl.Cache.rdataEqCI rr.type e.rr.rdata rr.rdata))) = true := by
              rw [hek]; rfl
            ...
            · show (Q e && !(entKeyB e rr
                  && (e.expiry != now + rr.ttl.toNat.toUInt32 || VeriDNS.Impl.Cache.rdataEqCI rr.type e.rr.rdata rr.rdata))) = true
              rw [hkeep, hQ]
              rfl
            · intro p hp
          ...
          ·
            ...
            have hq2 : (Q e && !(entKeyB e rr
                && (e.expiry != now + rr.ttl.toNat.toUInt32 || VeriDNS.Impl.Cache.rdataEqCI rr.type e.rr.rdata rr.rdata))) = false := hq
...
/-- Finding 019: the chase-arm cache write refines the model `cnameOwned`
absorb — only the qname-owned CNAME records enter the cache through the
chase arm. -/
theorem chasedCname_write_WriteRefines (c : VeriDNS.Impl.Cache.DnsCache) (resp : VeriDNS.Spec.Format)
    (sname : ByteArray) (bwN : VeriDNS.Spec.Net.Name) (now : UInt32)
    (hcut : αName sname = some bwN)
    (hsc : sname = VeriDNS.Impl.DomainName.labelsToWireFormatGo bwN)
    (hsv : ∀ x ∈ bwN, x.size ≤ 63)
    ...
    (hwf : CacheWf c now)
    (hoe : VeriDNS.Proof.NameTree.OneExpiryPerKey c)
    (hval : ∀ b ∈ (VeriDNS.Impl.Cache.normRaws (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer)).toList,
        ∀ rr, VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b = some rr → (αRR rr).isSome = true)
    (hno : ∀ b ∈ (VeriDNS.Impl.Cache.normRaws (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer)).toList,
        ∀ rr, VeriDNS.Spec.RRParse.parseRaw (RR := VeriDNS.Spec.ResourceRecord) b = some rr →
        (now + rr.ttl.toNat.toUInt32).toNat = now.toNat + rr.ttl.toNat)
    (hcanmap : ∀ e ∈ VeriDNS.Impl.Cache.rrsOf (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer), RRCanonMappable e) :
    WriteRefines now.toNat
      (αCache (VeriDNS.Impl.Resolver.cacheUnlessTruncated (C := VeriDNS.Impl.Cache.DnsCache) (RR := VeriDNS.Spec.ResourceRecord)
        ...
        (VeriDNS.Impl.Resolver.credAnswer (resp.header.aa == 1)) now))
      ((αCache c).absorb now.toNat bwN ((αResp resp).cnameOwned bwN)) := by
  have hsec := section_cnameOwner_extra_perm resp.answer sname bwN
    (VeriDNS.Impl.Resolver.credAnswer (resp.header.aa == 1)) now hcut hsc hsv hno hcanmap
  rw [αCred_credAnswer] at hsec
  have hMpos : ((αCache c).absorb now.toNat bwN ((αResp resp).cnameOwned bwN)).pos
      = ((VeriDNS.Spec.Net.normalizeTTL ((αSection resp.answer).filter
            (fun r => r.rdata.rtype == RRType.cname && VeriDNS.Spec.Net.nameEq r.owner bwN))).reverse.flatMap
          (modelPushOf now.toNat
            (if (resp.header.aa == 1) then VeriDNS.Spec.Net.Cred.authoritative else VeriDNS.Spec.Net.Cred.glue)))
        ++ (αCache c).pos :=
    absorb_cnameOwned_pos (αCache c) now.toNat bwN (αResp resp)
  refine single_cred_write_WriteRefines c resp _ _ now _ _
    htc (cred_used_credAnswer _) hwf hoe hval hno hMpos hsec ?_ ?_ ?_
  · intro x hx
    obtain ⟨r, -, hxr⟩ := List.mem_flatMap.mp hx
...
theorem chasedCname_write_WriteRefines_ref (c : VeriDNS.Impl.Cache.DnsCache) (resp : VeriDNS.Spec.Format)
    (sname : ByteArray) (bwN : VeriDNS.Spec.Net.Name) (now : UInt32)
    (hcut : αName sname = some bwN)
    (hsc : sname = VeriDNS.Impl.DomainName.labelsToWireFormatGo bwN)
    (hsv : ∀ x ∈ bwN, x.size ≤ 63)
    ...
    WriteRefines now.toNat
      (αCache (VeriDNS.Impl.Resolver.cacheUnlessTruncated (C := VeriDNS.Impl.Cache.DnsCache) (RR := VeriDNS.Spec.ResourceRecord)
        c resp (VeriDNS.Impl.Resolver.cnameRaws (RR := VeriDNS.Spec.ResourceRecord) sname resp.answer)
        (VeriDNS.Impl.Resolver.credAnswer (resp.header.aa == 1)) now))
      (cm.absorb now.toNat bwN (ref.cnameOwned bwN)) := by
  have h0 := chasedCname_write_WriteRefines c resp sname bwN now hcut hsc hsv htc hwf hoe hval hno hcanmap
  have hcongr : (αCache c).absorb now.toNat bwN ((αResp resp).cnameOwned bwN)
      = (αCache c).absorb now.toNat bwN (ref.cnameOwned bwN) :=
    VeriDNS.Spec.Net.absorb_resp_congr _ _ _
      (by simp only [VeriDNS.Spec.Net.Response.cnameOwned_aa]; exact haaEq)
      (by simp only [VeriDNS.Spec.Net.Response.cnameOwned_answer]
          rw [show (αResp resp).answer = αSection resp.answer from rfl, hansEq]) rfl rfl
  rw [hcongr] at h0
  have hmabs := hmme.absorb now.toNat bwN (ref.cnameOwned bwN)
  exact h0.trans_perm hmabs.1 hmabs.2.1 hmabs.2.2
...
theorem StateModels_answerCname_preserve
    ...
    (old : StateModels net ns ra ednsBuf rttOf now q state c w)
    (hbridge : MatchMaxEquiv (αCache stateB.resources.cache)
        ((αCache state.resources.cache).absorb now bwN ((αResp resp).cnameOwned bwN)))
    (hsnameNew : αName stateB.resources.sname = some target)
    (hnow : stateB.now = state.now) :
    StateModels net ns ra ednsBuf rttOf now { q with qname := target } stateB
      (c.absorb now bwN ((αResp resp).cnameOwned bwN)) w := by
