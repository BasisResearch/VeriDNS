...
theorem namesCorr_reachableNames {qname : ByteArray} {answer : Array ByteArray} {mqname : Name}
  ...
  rw [VeriDNS.Impl.Resolver.reachableNamesB_eq_iter]
  unfold VeriDNS.Spec.Net.reachableNames
...
/-! ### 068: α-bridge for the qtype-relevance delivery filter -/

/-- Pointwise coherence of the byte-level and model-level type-relevance
    tests: on a record that parses and abstracts, `typeRelevantB` computes
    exactly `typeRelevant` of its model image. -/
theorem typeRelevantB_model_eq {qtype : BitVec 16} {qtq : VeriDNS.Spec.Net.QType}
    (hq : αQType qtype = some qtq) {b : ByteArray} {rr : ResourceRecord}
    {r : VeriDNS.Spec.Net.RR}
    (hpr : RRParse.parseRaw (RR := ResourceRecord) b = some rr)
    (hαr : αRR rr = some r) :
    VeriDNS.Impl.Resolver.typeRelevantB (RR := ResourceRecord) qtype b
      = VeriDNS.Spec.Net.typeRelevant qtq r := by
  have hrt : αType rr.type = some r.rtype := αRR_rtype rr r hαr
  have hrty : RRParse.rrType (RR := ResourceRecord) rr = rr.type := rfl
  unfold VeriDNS.Impl.Resolver.typeRelevantB VeriDNS.Spec.Net.typeRelevant
  simp only [hpr, hrty]
  have hcn : (rr.type == (5 : BitVec 16)) = (r.rtype == VeriDNS.Spec.RRType.cname) := by
    have h5 : αType (5 : BitVec 16) = some VeriDNS.Spec.RRType.cname := rfl
    by_cases he : rr.type = (5 : BitVec 16)
    · have hce : r.rtype = VeriDNS.Spec.RRType.cname := by
        rw [he] at hrt
        exact Option.some.inj (hrt.symm.trans h5)
      rw [he, hce]
      simp only [beq_self_eq_true]
      exact (VeriDNS.Proof.Refinement.rrtype_beq_self _).symm
    · have hne : (r.rtype == VeriDNS.Spec.RRType.cname) = false := by
        by_contra hcontra
        rw [Bool.not_eq_false] at hcontra
        exact he (αType_injective hrt (VeriDNS.Proof.Refinement.rrtype_eq_of_beq hcontra ▸ h5))
      have heb : (rr.type == (5 : BitVec 16)) = false := by
        simp only [beq_eq_false_iff_ne]; exact he
      rw [heb, hne]
  by_cases h255 : qtype.toNat = 255
  · have hstar : qtq = VeriDNS.Spec.Net.QType.star := by
      unfold αQType at hq
      rw [if_pos h255] at hq
      exact (Option.some.inj hq).symm
    subst hstar
    simp [h255, VeriDNS.Spec.Net.QType.covers]
  · obtain ⟨t, hαt⟩ := αType_total qtype
    have hrr : qtq = VeriDNS.Spec.Net.QType.rr t := by
      unfold αQType at hq
      rw [if_neg h255, hαt] at hq
      exact (Option.some.inj hq).symm
    subst hrr
    have hqt : (rr.type == qtype) = (t == r.rtype) := by
      by_cases he : rr.type = qtype
      · have hte : t = r.rtype := by
          rw [he] at hrt
          exact Option.some.inj (hαt.symm.trans hrt)
        rw [he, hte]
        simp only [beq_self_eq_true]
        exact (VeriDNS.Proof.Refinement.rrtype_beq_self _).symm
      · have hne : (t == r.rtype) = false := by
          by_contra hcontra
          rw [Bool.not_eq_false] at hcontra
          exact he (αType_injective hrt
            ((VeriDNS.Proof.Refinement.rrtype_eq_of_beq hcontra).symm ▸ hαt))
        have heb : (rr.type == qtype) = false := by
          simp only [beq_eq_false_iff_ne]; exact he
        rw [heb, hne]
    simp only [VeriDNS.Spec.Net.QType.covers, h255, decide_false, Bool.or_false, hqt, hcn]

/-- The qtype-relevance filter commutes with section abstraction: filtering
    the wire bytes and filtering the model image agree.  No canonicality
    premises — a record that fails to parse or abstract is invisible to both
    sides. -/
theorem αSection_typeScrubB_eq {qtype : BitVec 16} {qtq : VeriDNS.Spec.Net.QType}
    (hq : αQType qtype = some qtq) (answer : Array ByteArray) :
    αSection (VeriDNS.Impl.Resolver.typeScrubB (RR := ResourceRecord) qtype answer)
      = VeriDNS.Spec.Net.typeScrub qtq (αSection answer) := by
  unfold VeriDNS.Impl.Resolver.typeScrubB VeriDNS.Spec.Net.typeScrub αSection
  rw [Array.toList_filter]
  induction answer.toList with
  | nil => rfl
  | cons b l ih =>
    cases hpr : RRParse.parseRaw (RR := ResourceRecord) b with
    | none =>
      have hpB : VeriDNS.Impl.Resolver.typeRelevantB (RR := ResourceRecord) qtype b = false := by
        unfold VeriDNS.Impl.Resolver.typeRelevantB
        simp only [hpr]
      rw [List.filter_cons_of_neg (by rw [hpB]; exact Bool.false_ne_true),
        List.filterMap_cons_none (by simp only [hpr])]
      exact ih
    | some rr =>
      ...
      | none =>
        rw [List.filterMap_cons]
        simp only [hpr, hαr]
        by_cases hpB : VeriDNS.Impl.Resolver.typeRelevantB (RR := ResourceRecord) qtype b = true
        · rw [List.filter_cons_of_pos hpB, List.filterMap_cons_none (by simp only [hpr, hαr])]
          exact ih
        · rw [List.filter_cons_of_neg hpB]
          exact ih
      | some r =>
        have hpt := typeRelevantB_model_eq hq hpr hαr
        rw [List.filterMap_cons]
        simp only [hpr, hαr]
        by_cases hrel : VeriDNS.Spec.Net.typeRelevant qtq r = true
        · rw [List.filter_cons_of_pos (by rw [hpt]; exact hrel),
            List.filterMap_cons_some (by simp only [hpr]; exact hαr),
            List.filter_cons_of_pos hrel]
          rw [ih]
        · rw [List.filter_cons_of_neg (by rw [hpt]; exact hrel),
            List.filter_cons_of_neg hrel]
          exact ih
