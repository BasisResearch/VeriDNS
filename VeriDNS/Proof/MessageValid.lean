...
/-- Every `CanonicalRdata` payload fits the 16-bit `rdlength` field: name-typed and SOA rdata are bounded by the
    ≤255 name cap (RFC 1035 §2.3.4), and the `other` case carries the bound directly. The size hypothesis the
    `rrWire` round-trip needs, available uniformly from `CanonicalRR`. -/
...
/-- **`parseRaw` (`ResourceRecord.decode`) round-trips an `rrWire` blob.** Unlike `rrWire_frame` (the
    `decodeRRCanonical` analogue), `ResourceRecord.decode` reads rdata as RAW bytes (`readBytes rdlength`), so it
    returns the stored `rdata` VERBATIM — provided `rdata.size < 65536` so the 16-bit `rdlength` field round-trips.
    This is the key fact that lifts the codec's `CanonicalRR` (every decoded RR-blob has canonical NS rdata, since
    `decodeRRCanonical` decompressed+re-encoded it) into a per-record rdata-canonicity the keystone's `hhost`/`hnd`
    need — a stronger, ADVERSARIAL-safe replacement for the honest-disjunct NS-canonicity (works for warm caches). -/
...
/-- **Every answer RR of a decoded message is `parseRaw`-able** (codec consistency for the abstraction
    layer). Each `decode`d answer RR is a `decodeRRCanonical` output = `encode` of a canonical RR, on which
    `ResourceRecord.decode` (= `parseRaw`) succeeds (`decode_succeeds_of_encode`). This is what makes
    `αSection` FAITHFUL on a decoded response (no RR is silently dropped), closing the malformed-input
    impl/model classification gap at the honest answer terminal. -/
...
/-- Authority-section analogue of `decode_answer_parseRaw` (same proof; authority is also
    `decodeMany decodeRRCanonical`). -/
...
/-- Additional-section analogue of `decode_answer_parseRaw`. -/
...
/-- The answer section of a decoded message is `CanonicalRR` (each RR-blob is a `decodeRRCanonical`
    output). The provenance the CNAME-chase cache write needs for its canonicity obligations. -/
...
/-- The authority section of a decoded message is `CanonicalRR` (each RR-blob is a `decodeRRCanonical`
    output). The provenance the referral cache write needs for `CacheNsCanon`. -/
...
/-- The additional section of a decoded message is `CanonicalRR`. -/
...
/-- **A decoded message's answer count matches its header's `ancount`** (`run_decodeMany_size`). Lets the
    lift relate `respA.answer.isEmpty` to `ancount = 0`, hence to the server's `ref.answer = []` (referral)
    through the codec — the answer-emptiness half of the honest referral-exclusion. -/
